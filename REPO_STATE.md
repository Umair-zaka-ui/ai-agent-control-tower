...
> **Stale, and flagged rather than fully regenerated (Phase 4.6/4.7):** this tree predates Milestone 4's later sub-phases and does not show the sibling packages `app/behavior/` (4.5), `app/finops/` (4.4), `app/observability/` (4.1), `app/telemetry_export/` (4.6) or `app/slo/` (4.7). Each is a top-level package under `app/`, described in §4 and in the per-phase narrative above; the authoritative module inventory is §4, not this tree. Regenerating the tree in full is deferred to the next full REPO_STATE pass.
