...
### Milestone 2 — Enterprise Integration Framework (`ACT-SRS-M2`) — **IN PROGRESS (6/9)**

SRS approved 2026-07-28 (per the document's own §12 revision note). First sub-phase shipped 2026-08-01; second shipped 2026-08-04; third shipped 2026-08-05; fourth (2.1.4, completing the connector framework in full) shipped the same day; fifth (2.2.1, the framework's first real connector) shipped the same day again; sixth (2.2.2, the second real connector — the database connector carrying `ACT-INT-FR-121`/`FR-122`'s "the model never writes SQL" rule) shipped 2026-08-06. What remains: 2.2.3-2.2.4 (storage/queue connectors) and 2.3.1 (identity federation). Nine sub-phases, recommended build order per the SRS's own §9: framework (2.1.1→2.1.4) before any generic connector, REST (2.2.1) first among generics as the substrate later vendor connectors will reuse, database (2.2.2) second per the same §9 order, identity federation (2.3.1) independent and reorderable earlier if a design partner needs SSO before data connectors.
...
| **2.2.3-2.3.1** | **NOT STARTED** | No code references them beyond forward-looking doc comments in 2.1.1-2.2.2's own files (e.g. the deferred-scope table in `docs/integration/connectors.md` naming exactly what 2.2.3-2.2.4/2.3.1 each still owe). |
...
Output of `git branch --sort=-committerdate`, with committer dates added via `--format`. Names and dates only — 44 local branches, re-verified this session (was 43 at the last full count); each mirrored by an `origin/*` remote-tracking branch at the same commit/date (shown once below; the `origin/<name>` counterpart is identical). 1 new branch since the last full count: `feat/2.2.2-database-connector` (merged into `main` via a `--no-ff` merge, per the git-log evidence below).

| Branch | Committer date |
|---|---|
| `main` | 2026-08-06 17:15:27 +0500 |
...
Current branch: `main`, at `f29d1cd` ("Merge Phase 2.2.2: Generic Database Connector"). Working tree clean once this document's own update (and the accompanying `CHANGELOG.md`/`ROADMAP.md` updates) are committed.
