...
**107 tables**, extracted via live `sqlalchemy.inspect()` against the local Postgres database after running every migration through `0035_connector_health` (head). This reflects exactly what the migration chain produces. `alembic_version` (Alembic's own bookkeeping table, one column, no app data) is included below for completeness since it is a real table in the database.
...
### Milestone 2 — Enterprise Integration Framework (`ACT-SRS-M2`) — **IN PROGRESS (4/9)**

SRS approved 2026-07-28 (per the document's own §12 revision note). First sub-phase shipped 2026-08-01; second shipped 2026-08-04; third shipped 2026-08-05; fourth (2.1.4) shipped the same day — **this completes the connector framework (Phase 2.1) in full**; what remains is *using* it (2.2.x, 2.3.1). Nine sub-phases, recommended build order per the SRS's own §9: framework (2.1.1→2.1.4) before any generic connector, REST (2.2.1) first among generics as the substrate later vendor connectors will reuse, identity federation (2.3.1) independent and reorderable earlier if a design partner needs SSO before data connectors.
...
| **2.2.1-2.3.1** | **NOT STARTED** | No code references them beyond forward-looking doc comments in 2.1.1-2.1.4's own files (e.g. the deferred-scope table in `docs/integration/connectors.md` naming exactly what 2.2.x/2.3.1 each still owe). |
...
Output of `git branch --sort=-committerdate`, with committer dates added via `--format`. Names and dates only — 42 local branches, re-verified this session (was 41 at the last full count); each mirrored by an `origin/*` remote-tracking branch at the same commit/date (shown once below; the `origin/<name>` counterpart is identical). 1 new branch since the last full count: `feat/2.1.4-connector-sdk` (pushed to origin, merged into `main` via a `--no-ff` merge, per the git-log evidence below).

| Branch | Committer date |
|---|---|
| `main` | 2026-08-05 16:16:50 +0500 |
...
Current branch: `main`, at `5802bb2` ("Merge Phase 2.1.4: Connector SDK"). Working tree clean once this document's own update (and the accompanying `CHANGELOG.md`/`ROADMAP.md` updates) are committed.
