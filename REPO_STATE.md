...
113. **`docs/architecture/data/erd.md` is stale and known to be** (recorded Phase 3.10). It declares **24 tables** against a live schema of **124**, and its diagrams cover only the Phase 4 identity and authorization bounded contexts — nothing from Milestone 1's runtime tables, Milestone 2's connectors, or any of Milestone 3 (`deployment_traffic_allocations`, `rollout_plans`, `rollback_events`, `job_definitions`, `job_runs`, `worker_registrations`). It has been stale since at least Phase 3.4 and predates this milestone entirely. Deliberately **not** regenerated as a side effect of Phase 3.10's documentation sweep: a hundred tables of ERD is its own scoped piece of work, and silently expanding it during a docs pass would produce a diagram nobody reviewed. The file's own header carries the command that re-derives the count (`len(Base.metadata.tables)`), which is how the drift was caught. `docs/architecture/data/erd.md`.
