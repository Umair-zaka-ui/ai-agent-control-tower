...
**What changed.** New package `app/security/` (8 modules: `errors`, `encryption_provider`, `install_mode`, `canary`, `key_integrity`, `bootstrap`, `keys` (CLI), `backup`). One new model `app/models/security.py::KeyMaterialCanary`. `credential_crypto.py` and `LocalKeyProvider` **no longer generate a key when one is absent** — generation moved behind an explicit `bootstrap()` / `python -m app.security.keys bootstrap` that refuses to run on an EXISTING install. The Fernet key is now obtained through an `EncryptionKeyProvider` seam mirroring `SigningProvider` (`LocalEncryptionKeyProvider` default; KMS/Vault adapter documented as future work, no vendor SDK in core). The key ring is a `MultiFernet` over the active key + `MODEL_CREDENTIAL_ENCRYPTION_KEYS` (retained prior keys) — additive rotation, no big re-encrypt transaction. `app.main` lifespan calls `verify_key_material(db)` before serving: install mode is decided by **the presence of encrypted state in the database**, not the absence of a file, so a restored DB with rows and no key is EXISTING → fail loud (`KeyMaterialError`, deterministic, no secret in the message), never bootstrap. A `key_material_canary` verifier row + a trial-decrypt of real ciphertext catch a *wrong* key too. Signing: the configured default identity must still have a usable private key whose public half matches the database.

**§2/§3/§5 re-derived live**: migration **`0051_telemetry_privacy` → `0052_key_material_canary`** (one new table `key_material_canary`, `uq_key_material_canary_purpose_fp`; **136 tables** total, `sqlalchemy.inspect()`; 135 in `Base.metadata`); no existing table changed, no backfill, `alembic downgrade -1`/`upgrade head` re-verified clean; **changes no decrypt behaviour for any existing row**. Routes **unchanged at 590** (the operator surface is a CLI, `python -m app.security.keys`, not HTTP). New settings: `ENCRYPTION_KEY_PROVIDER`, `MODEL_CREDENTIAL_ENCRYPTION_KEYS`, `KEY_MATERIAL_FAIL_LOUD`, `ENCRYPTION_KEY_ALLOW_BOOTSTRAP`. No new permission, error code registered in the identity taxonomy, or audit event (`KeyMaterialError` is a startup abort, not an authorization decision). **23 new tests** in `backend/tests/runtime/test_key_material_integrity.py` (incl. the §16 end-to-end production-integrity proof); **one** existing test's fixture adapted, intent-preserving: `test_provider_credentials.py::_fresh_encryption_key` now writes its own throwaway key file (the deliberate-bootstrap contract) instead of relying on the removed auto-generation. Backend **2,303 passed, 0 failed, 1 deselected** (2,280 baseline + 23); frontend **359 passed**, unchanged (no frontend touched). See `docs/security/key-management.md`, the rewritten `RECOVERY.md`, and [ADR-0014](docs/architecture/adr/0014-key-material-recovery-and-fail-loud-integrity.md). **This is the production-integrity prerequisite for Milestone 5, now satisfied.**
