...
**99 tables**, extracted via live `sqlalchemy.inspect()` against the local Postgres database after running every migration through `0029_provider_credentials` (head). This reflects exactly what the migration chain produces. `alembic_version` (Alembic's own bookkeeping table, one column, no app data) is included below for completeness since it is a real table in the database.

**Phase 5.7a.5 update**: re-extracted this session (previously 98 tables at `0028_streaming_and_pricing`). 1 new table: `provider_credentials` (per-organization, per-provider encrypted model-provider credentials, `ACT-MDL-FR-080..083`). No existing table gained or lost a column — this phase is purely additive at the schema level.
...
| 29 | `0029_provider_credentials.py` | Phase 5.7a.5 - Per-Organization Provider Credentials: new `provider_credentials` table (encrypted-at-rest, one row per `(organization_id, provider)`), two new RBAC permissions (`runtime.provider.view`/`.manage`) backfilled onto the global catalog. **(head)** |

## 4. Implemented Modules

**240 non-`__init__.py` Python files** under `backend/app/` (239 at the last full count, post Phase 5.7a.2; Phase 5.7a.3/5.7a.4 added no new files, both landing entirely inside already-counted files. +1 this update, Phase 5.7a.5: `runtime/providers/credential_crypto.py` (the encryption utility) — `ProviderCredential`/`ProviderCredentialService`/the four new routes/schemas all landed inside already-counted files (`models/runtime.py`, `services.py`, `routes.py`, `schemas.py`), same pattern as every provider-layer sub-phase since 5.7a.3). AST-parsed for their module docstring (first line shown) and every top-level class/public function. Grouped by domain (backend directory structure). Frontend module structure is summarized separately at the end of this section (file-count only — the frontend was not AST-parsed since the exhaustive symbol-level inventory the user asked for was scoped to "modules" in the module/class/function sense, which is the backend's organizing unit; the frontend's `src/modules/*` React component tree does not have an equivalent exported-symbol convention).
...
- Classes: `AgentDefinition,AgentVersion,AgentReleaseChannel,AgentVersionSnapshot,AgentReleaseMetadata,AgentReleaseArtifact,AgentReleaseNote,AgentVersionStatusHistory,AgentDeployment,AgentExecution,ExecutionAttempt,ExecutionLock,ModelPricing,ProviderCredential,Capability,AgentCapability,Tool,AgentTool,ToolCall,RuntimeEvent,DeploymentHealth,IdempotencyRecord,RuntimeApproval` (`ModelPricing` — Phase 5.7a.3, previously missing from this list only because this pass is the first full re-check of it since; `ProviderCredential` — Phase 5.7a.5, `__repr__` overridden to redact `encrypted_secret`, see §10.31)
...
- Phase 5.7a.5 added the four `list_provider_credentials`/`upsert_provider_credential`/`delete_provider_credential`/`test_provider_credential` functions, gated by two new permission constants `_PROVIDER_VIEW`/`_PROVIDER_MANAGE`.
...
- Classes: `CostResult,PricingService,ProviderCredentialInfo,ResolvedCredential,CredentialTestResult,ProviderCredentialService,ModelGatewayError,ModelGatewayService,AgentRegistryService,AgentVersionService,DeploymentService,CapabilityService,ToolRegistryService,ToolGatewayService,PolicyResult,RuntimePolicyService,IdempotencyService,ExecutionRequestService,ExecutionWorkerService,RuntimeApprovalService,HealthMonitoringService,KillSwitchService,RuntimeDashboardService` (`CostResult`/`PricingService` added Phase 5.7a.3; `ProviderCredentialInfo`/`ResolvedCredential`/`CredentialTestResult`/`ProviderCredentialService` added Phase 5.7a.5, `ACT-MDL-FR-080..083` — per-organization, encrypted, direct-ORM, no-repository, matching the established runtime-domain service pattern)
...
Output of `git branch --sort=-committerdate`, with committer dates added via `--format`. Names and dates only — 35 local branches, re-verified this session (was 34 at the last full count); each mirrored by an `origin/*` remote-tracking branch at the same commit/date (shown once below; the `origin/<name>` counterpart is identical). 1 new branch since the last full count: `feat/5.7a.5-credentials-cost` (merged into `main` via a local `--no-ff` merge, per the git-log evidence below).

| Branch | Committer date |
|---|---|
| `main` | 2026-07-30 02:45:08 +0500 |
...
Current branch: `main`, at `b9461ab` ("Merge Phase 5.7a.5: Per-Organization Provider Credentials"). Working tree is clean except for this document's own in-progress update (verified via `git status --porcelain`).
