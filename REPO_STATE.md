...
**106 tables**, extracted via live `sqlalchemy.inspect()` against the local Postgres database after running every migration through `0034_connector_auth` (head). This reflects exactly what the migration chain produces. `alembic_version` (Alembic's own bookkeeping table, one column, no app data) is included below for completeness since it is a real table in the database.
...
- Classes: `ConnectorTypeNotFoundError,ConnectorNotFoundError,ConnectorConfigInvalidError,ConnectorInvalidTransitionError,ConnectorCredentialNotFoundError,ConnectorAuthSchemeUnsupportedError,ConnectorCredentialInvalidError,ConnectorOAuthRefreshFailedError` (last four added Phase 2.1.2)
...
- Classes: `Connector,ConnectorInstance,ConnectorLifecycleEvent`
...
| **2.1.3-2.3.1** | **NOT STARTED** | No code references them beyond forward-looking doc comments in 2.1.1/2.1.2's own files (e.g. `service.py`'s note that its private `_CONNECTOR_TYPES` dict is explicitly superseded once 2.1.3's real registry lands). |
...
Output of `git branch --sort=-committerdate`, with committer dates added via `--format`. Names and dates only — 40 local branches, re-verified this session (was 39 at the last full count); each mirrored by an `origin/*` remote-tracking branch at the same commit/date (shown once below; the `origin/<name>` counterpart is identical). 1 new branch since the last full count: `feat/2.1.2-connector-auth` (pushed to origin, merged into `main` via a `--no-ff` merge, per the git-log evidence below).

| Branch | Committer date |
|---|---|
| `main` | 2026-08-04 22:11:11 +0500 |
...
Current branch: `main`, at `8e26934` ("Merge Phase 2.1.2: Connector Authentication Framework"). Working tree is clean (verified via `git status --porcelain`).
...
17. ~~`ROADMAP.md` is stale and should not be trusted over this document~~ — **closed**: backfilled in the same pass that recorded this gap (both `CHANGELOG.md` and `ROADMAP.md` now cover every phase through 2.1.1, one entry each for 5.7a.2-5.7a.5, 5.6a.1-5.6a.3, and 2.1.1). Re-verified this pass: `ROADMAP.md` also already reflects Phase 2.1.2 having shipped (added in the same regeneration pass as this document, not left for a future one). **This document (§6) remains the authoritative, always-current source per the Update Protocol in §11** — `CHANGELOG.md`/`ROADMAP.md` are kept in sync opportunistically, not on every single phase, so a future gap here is possible again; re-check before trusting either blindly. **Milestone 2 now has two of nine sub-phases implemented** (2.1.1 Connector Abstraction & Lifecycle, 2.1.2 Connector Authentication Framework — see §6) — 7 remain NOT STARTED.
