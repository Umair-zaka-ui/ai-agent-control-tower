...
**104 tables**, extracted via live `sqlalchemy.inspect()` against the local Postgres database after running every migration through `0033_connector_core` (head). This reflects exactly what the migration chain produces. `alembic_version` (Alembic's own bookkeeping table, one column, no app data) is included below for completeness since it is a real table in the database.
...
- Classes: `ConnectorTypeNotFoundError,ConnectorNotFoundError,ConnectorConfigInvalidError,ConnectorInvalidTransitionError`
...
**`backend/app/integration/routes.py`** — Enterprise Integration Framework API (Phase 2.1.1 SRS §7) — /api/v1/integration.
- Functions: `list_connector_types,list_connectors,create_connector,get_connector,configure_connector,activate_connector,disable_connector,list_connector_events`

**`backend/app/integration/schemas.py`** — Phase 2.1.1 — Pydantic request/response schemas for `/api/v1/integration`.
- Classes: `ConnectorTypeRead,ConnectorInstanceRead,ConnectorLifecycleEventRead,ConnectorInstanceCreate,ConnectorInstanceConfigure,ConnectorDisableRequest`
...
| **2.1.2-2.3.1** | **NOT STARTED** | No code references them beyond forward-looking doc comments in 2.1.1's own files (e.g. `service.py`'s note that its private `_CONNECTOR_TYPES` dict is explicitly superseded once 2.1.3's real registry lands). |
...
Output of `git branch --sort=-committerdate`, with committer dates added via `--format`. Names and dates only — 39 local branches, re-verified this session (was 38 at the last full count); each mirrored by an `origin/*` remote-tracking branch at the same commit/date (shown once below; the `origin/<name>` counterpart is identical). 1 new branch since the last full count: `feat/2.1.1-connector-core` (pushed to origin, merged into `main` via a `--no-ff` merge, per the git-log evidence below).

| Branch | Committer date |
|---|---|
| `main` | 2026-08-01 05:56:32 +0500 |
...
Current branch: `main`, at `dcc314e` ("Merge Phase 2.1.1: Connector Abstraction & Lifecycle"). Working tree is clean (verified via `git status --porcelain`).
...
17. **`ROADMAP.md` is stale and should not be trusted over this document.** Re-checked this pass (`grep`-verified): its "Milestone 1" section only covers 5.7a.1 and stops — 5.7a.2 through 5.7a.5 and all of 5.6a.1-3 (i.e. the entirety of what actually closed Milestone 1) are simply absent from it, and it now also has no mention of Milestone 2 or Phase 2.1.1 having shipped. This document (§6) is what has actually been kept current every phase, per the Update Protocol in §11 — `ROADMAP.md` has not been part of that protocol and has silently fallen behind since 5.7a.1. Still not fixed this pass (out of scope; flagged so a future session doesn't cite it as authoritative). **Milestone 2 now has its first sub-phase implemented** (2.1.1, Connector Abstraction & Lifecycle — see §6) — 8 of 9 sub-phases remain NOT STARTED.
