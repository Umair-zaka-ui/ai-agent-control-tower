...
**Phase 4.3 (Runtime Governance Enforcement Engine) shipped 2026-08-26** — Milestone 4's third sub-phase, and **the first phase since 3.9 to modify the execution path itself.** That is the fact to carry forward: `ToolLoopOrchestrator.run` — the model→tool→model loop — was restructured, not merely extended.

**The central decision was to generalize rather than add beside.** Since Phase 5.6a.3 the loop enforced four termination caps with inline `if` statements: iteration count, wall-clock, token budget, and repeated-identical-call. 4.3 needed richer constraints (cost ceilings, restricted models and tools, data sensitivity, duration, approval obligations) at the same and additional points, and the cheap option was a second mechanism running beside the first. **It was rejected.** Two enforcers can disagree about whether to stop an execution, and the winner would be whichever `if` the loop reached first — an implementation detail nobody wrote down. So the four caps became *constraints inside* `RuntimeGovernanceEngine`, reached through the same `evaluate()` call as every richer rule. There is now exactly one place that decides "may this loop continue".

**Behaviour preservation was treated as the deliverable, not a constraint on it.** Every one of the four caps still writes its original `termination_reason` (`MAX_ITERATIONS`/`WALL_CLOCK`/`TOKEN_BUDGET`/`REPEATED_CALL`), still raises `TOOL_LOOP_LIMIT_EXCEEDED`, and still records the same `loop_iterations`. The full pre-existing tool-loop suite passes unmodified. The one subtlety worth recording: the wall-clock and token checks that used to sit at the *bottom* of the loop body now run at the top of the next iteration — the adjacent half of the same boundary, with no code between them — and the evaluation order that decides *which* cap is reported when two breach on the same turn was derived from the pre-4.3 statement order (wall-clock, tokens, iterations) and is asserted by a test rather than trusted.

**The one-path property is structural, not documentary.** `ToolLoopOrchestrator` no longer names `TOOL_LOOP_MAX_TOTAL_TOKENS` or `TOOL_LOOP_MAX_WALL_CLOCK_SECONDS` in any comparison and no longer raises `TOOL_LOOP_LIMIT_EXCEEDED` itself — asserted over the AST, so a check reintroduced in a helper, a comprehension or a nested function fails CI. A grep would only catch someone restoring the old lines verbatim; the failure mode being guarded against is someone adding "just one quick check" beside the engine.

**Governance fails CLOSED — the deliberate inverse of 4.1/4.2's telemetry plane, and the two now live inches apart in one loop.** A *mandatory* policy that cannot be evaluated STOPs the execution; a non-mandatory one is logged and skipped. Policy *resolution* failing fails closed regardless of any flag, because then the platform does not merely not know what the rules say — it does not know whether a mandatory one applies. Both plane directions are tested: a telemetry failure neither stops an execution nor changes a governance decision, and (the more dangerous direction) a broken telemetry plane cannot suppress a governance stop either. The separation is structural — the governance package does not import `app.observability`'s read models at all, and a `CheckpointContext` carries no telemetry handle for a constraint to read.

**Six checkpoints, and no lock held across model or tool I/O.** Every checkpoint does at most one plain non-locking `SELECT` (kill state, read fresh because an operator's kill arrives on a different connection and the identity map cannot see it) plus, for a *material* decision only, one `INSERT`. No statement in the package takes `FOR UPDATE`. The only lock a checkpoint can cause is the foreign-key `KEY SHARE` the transcript writer has already been taking every turn since 5.6a.3 — compatible with what a tool thread needs. Proven both structurally and behaviourally: a test evaluates a decision-writing checkpoint, leaves it uncommitted, and takes the tool-thread's lock from a second real connection with `NOWAIT`.

**The kill switch is triggered, never paralleled (§19).** A STOP whose policy declares a `stop_action` calls the new `KillSwitchService.activate_system`, which reuses the same `_cancel_executions`/`_suspend_deployments` and writes the same `RUNTIME_KILL_SWITCH_ACTIVATED` audit event as the operator path. Automation reaches **EXECUTION and AGENT scope only** — a rule misconfigured by one tenant must not be able to halt a project, an organization or the platform. The engine never *clears* a kill (asserted over the AST: no assignment to `lifecycle_status`/`cancel_requested`/`status` anywhere in the package), and `KILL_SWITCH_ACTIVE` is non-retryable, because an automatic retry past a kill would be automation overruling an operator.

**One capability gap, stated rather than papered over.** A CHALLENGE raised at the first checkpoint parks the execution in `PENDING_APPROVAL` and the existing approval funnel resumes it. A challenge raised *later* cannot be resumed — this platform has no way to re-enter a partially-run loop, and re-queuing would repeat tool calls that already had their side effects — so it raises its obligation and terminates in `BLOCKED`. Parking it where nothing could move it out again would have been worse. Two new execution-state edges exist as a result: `RUNNING → PENDING_APPROVAL` and `RUNNING → BLOCKED`.

**Cost: the ceiling and the headroom rule are different things, and both are tested including the unflattering case.** A bare `max_execution_cost` can only notice an overshoot — a model call's cost is unknowable until it returns, so the iteration that crossed the line has been paid for (measured: a 0.0025 ceiling with 0.001 turns stops on the third turn at 0.003). `min_remaining_cost` is what actually keeps spend *within* a bound (same ceiling, stopped after two turns at 0.002). The engine reads existing cost and reserves nothing — budgets are Phase 4.4's, and two concurrent executions under one ceiling can each pass a check the other is about to invalidate.

**Measured overhead (§25):** one full checkpoint evaluation — kill-state read, built-in caps, policy constraints — is **0.42ms p50 / 0.68ms p95**, roughly 1.7ms per loop iteration (four checkpoints; five when a tool is called). Recorded as an executable test, not a commit-message note.

**Two deviations from the build prompt, reported not silently taken.** (1) §6's decision-read route would have been `/api/v1/executions/{id}/governance-decisions`, creating a second, prefix-less execution namespace beside the established `/api/v1/runtime/executions/{id}/...` family; it is mounted on the existing prefix instead. (2) Only **one** new permission was registered, `runtime.governance.manage`. Decision reads reuse `runtime.execution.view` ("View executions, tool calls and telemetry"), because *why did this execution stop* is a fact about an execution rather than a separate capability — a second code would leave operators holding execution-view unable to answer the one question this phase exists to answer.

Five new routes under `/api/v1/runtime` (544 → **549**). Two new tables — `runtime_governance_policies` and the append-only `runtime_governance_decisions` (`REVOKE UPDATE, DELETE ... FROM PUBLIC`, mirroring `deployment_events`) — schema **124 → 126 tables**, and **no new column on `agent_executions`**: the engine reads the cost, token and timing columns that already existed. Four new error codes (`GOVERNANCE_POLICY_INVALID`, `GOVERNANCE_POLICY_NOT_FOUND`, `GOVERNANCE_CHECKPOINT_UNEVALUABLE` — deliberately *retryable*, `GOVERNANCE_EXECUTION_STOPPED` — not), two new audit events (`RUNTIME_POLICY_EVALUATED`, `RUNTIME_EXECUTION_STOPPED`). Migration head **`0047_runtime_governance`**, reversible (downgrade/upgrade verified live). **77 new tests** in `backend/tests/runtime/test_runtime_governance.py`. Backend **2,024 passed, 0 failed, 1 deselected** (1,947 baseline + 77), 776s; frontend **327 passed** across 49 files, unchanged (the governance operator view is 4.9's). See `docs/runtime/runtime-governance.md`, `docs/runtime/runtime-policy-checkpoints.md`, and ADR-0009 (runtime governance as a fail-closed plane).

**Phase 4.4 (Enterprise AI Cost Governance & FinOps) shipped 2026-08-28** — Milestone 4's fourth sub-phase. Two gates: **cost truth** (Gate D) and **budget enforcement that holds under concurrency** (Gate E, the §35 proof).

**The §35 race is the phase, and it is proven with real separate Postgres sessions.** The failure being prevented: twenty workers each read *"$9 remaining"* against a $10 budget, each conclude they may spend $9, and $180 is spent — every worker having read a true balance and acted on it correctly. The defect is the gap between the read and the act, and on this platform that gap contains a model call. The mechanism is `SELECT ... FROM budgets ... FOR UPDATE`, then sum the period's holds, then refuse-or-insert, then **commit** — releasing the lock before the caller does anything slow, which is commit-before-dispatch applied to money. The proof: twelve threads on twelve real connections racing a $1.00 budget at $0.25 a hold; **exactly four are granted**, and the sum of reserved never exceeds the limit. §11 forbids an in-process lock explicitly and a structural test asserts none exists — the Phase 3.9 fleet would make one worthless anyway.

**What is guaranteed and what is not, stated rather than glossed.** Guaranteed: total *reserved* never exceeds the limit. **Not** guaranteed: total *actual*. A model call's cost is unknowable until it returns, so an execution admitted with a $0.25 hold that costs $0.40 overshoots by $0.15; the total overshoot is bounded by the sum of (actual − reserved) over in-flight executions, and per-execution by Phase 4.3's `min_remaining_cost` headroom rule. Claiming otherwise would require knowing a price before paying it. **There is a test that asserts the overshoot is real** (four executions at 0.40 against a 1.00 ceiling ⇒ 1.60 spent, and the gate then closed), so nobody discovers it in production instead of in the suite. Recorded in ADR-0010.

**One enforcement path, preserved.** A refused reservation is not a refused execution: 4.4 reports `budget_remaining <= 0` and Phase 4.3's engine turns it into a `STOP`/`BUDGET_EXCEEDED` at `BEFORE_FIRST_MODEL_CALL`, before any model call. `app/finops` contains no code that writes an execution's status, raises the governance stop, or calls the kill switch — asserted over the AST, checking assigned *values* against the execution-state vocabulary so that `budget_reservations`' own `status` column does not force the test to be weakened. The budget constraint was added to the engine as a **separate tier** rather than as another built-in cap, specifically so 4.3's exact cap-ordering assertion did not have to be relaxed to make room.

**Cost truth (Gate D): the real figure, and the legacy one deprecated in place.** Aggregation runs over `agent_executions.cost_amount` by organization, agent, version, environment, provider, model, project, department and time. **Actual, estimated and unpriced are three separate numbers** and are never added: `cost_is_estimated` exists because the platform sometimes cannot meter a call, and a NULL `cost_amount` counts in neither sum — treating "we don't know" as zero is how a spend figure becomes a lie that gets repeated. The legacy `GET /analytics/cost` (note: **un-prefixed** — the Phase-3 analytics router mounts through `api_router` with `settings.API_PREFIX`, which is the empty string) multiplies `agent_actions` row counts by flat constants and is **not rewired**: the Phase-3 dashboard consuming it expects six synthetic categories (human review, policy evaluation, storage) that `agent_executions` knows nothing about, so pointing it at real data would have silently redefined every number on a dashboard this phase does not own. It keeps working and now carries a `deprecation` object in its response naming the replacement.

**Immutable provenance (§10) is a property of what already existed.** `PricingService.set_price` closes the prior price row and inserts a new one rather than mutating in place, and an execution records the `pricing_version` that produced its amount — so ten-x-ing a model's price leaves last month's charges and every aggregate over them untouched. A test 10×s a price and asserts both the single charge and the org-wide total are unchanged; it would fail loudly if either half ever became an in-place update.

**Measured, and no index added.** At **109,398 executions** (8,627 priced) across 10,934 tenants: tenant-scoped summary **0.80ms p50**, per-dimension breakdowns 1.99–5.69ms p50, timeseries 1.11ms — all reaching rows through Phase 4.2's `ix_agent_executions_org_created`, no sequential scan. The honest worst case (one tenant owning the table) is 20.46ms for a sum and 41.70ms grouped, and **no index fixes that**: summing a tenant's spend requires reading every row of it, O(rows the tenant owns) by definition — unlike 4.2's list view, whose bitmap-plus-sort was O(tenant size) only because it lacked an ordered path to a LIMIT. The only sublinear answer is a materialized rollup, which is the parallel cost store this phase is forbidden to build. The bound is the default 30-day window.

**Two accounting sources, deliberately.** Enforcing budgets (HARD_LIMIT/APPROVAL_REQUIRED) are accounted from the reservation ledger, because you cannot atomically claim a share of a number you compute by scanning. Signalling budgets (INFORMATIONAL/WARNING) take no reservations at all — holding budget for a mode that will never refuse anything would slow the execution path to enforce nothing — so they observe real spend from `agent_executions`. **The consequence is written down** because someone will meet it: a budget switched from WARNING to HARD_LIMIT starts its ledger at that moment and its reported utilization drops.

**Three defects the tests caught in this phase's own code**, each fixed rather than accommodated: (1) a budget `STOP` was **retryable**, so an exhausted budget would burn every remaining attempt taking and releasing holds against a budget with nothing left — `BUDGET_EXCEEDED` is now non-retryable alongside `GOVERNANCE_EXECUTION_STOPPED`; (2) signalling budgets could never signal, because their utilization was read from a ledger they never write — they now observe real spend; (3) a stale `QUEUED` row from an unrelated suite starves the eager inline worker (`claim_next` takes the oldest queued row across *every* organization), which surfaces as an execution stuck in QUEUED with no error at all — the suite now quiesces the queue per test, the same discipline Phase 3.9 established.

Ten new routes under `/api/v1/cost` and `/api/v1/budgets` (549 → **559**); no collision with the legacy `/analytics/cost`, which stays. Two new tables — `budgets` and `budget_reservations` — plus one nullable `budget_id` on `runtime_governance_decisions` (a budget-driven decision has no governance *policy* behind it, so lineage would otherwise lose which ceiling decided); schema **126 → 128 tables**, and **no new index on `agent_executions`**. Four new error codes (`BUDGET_NOT_FOUND`, `BUDGET_INVALID`, `BUDGET_EXCEEDED` — 402, not 403: the request was permitted, the money was not there — and `BUDGET_RESERVATION_CONFLICT`), two new audit events (`RUNTIME_BUDGET_THRESHOLD_REACHED`, `RUNTIME_BUDGET_BLOCKED`), two new permissions (`runtime.budget.view`/`.manage`; the cost read model reuses the pre-existing `runtime.cost.view`). Migration head **`0048_cost_governance`**, reversible (downgrade/upgrade verified live). **59 new tests** in `backend/tests/runtime/test_cost_governance.py`. Backend **2,083 passed, 0 failed, 1 deselected** (2,024 baseline + 59), 838s; frontend **327 passed** across 49 files, unchanged (the Cost Center UI is deferred to 4.9). See `docs/runtime/cost-governance.md`, `docs/runtime/budgets.md`, and ADR-0010 (budget reservation semantics).

**Phase 4.5 (Behavioral Signals & Runtime Anomaly Detection) shipped 2026-08-28** — Milestone 4's fifth sub-phase, and **a discipline-of-restraint phase: the hardest part was what not to build.**

**No machine learning, and the reason is stated rather than merely obeyed.** §4.5 forbids opaque anomaly scoring because *"this agent is 0.87 anomalous"* is unauditable, unappealable and ungovernable — a regulated tenant cannot act on it, dispute it, or show a regulator why it fired. *"Tool `send_email` failed 34% of 118 calls this week against a 3% baseline"* is all three. So every rule is arithmetic over a window, the thresholds are declared rather than learned, and the rules are **pure functions of `(candidate, baseline, thresholds)`** — no session, no clock, no globals — which is what makes "the same data always yields the same finding" testable rather than aspirational. A test runs a full evaluation three times and compares every field; another asserts over the AST that no numerical or ML library (including `random`) is importable anywhere in the package.

**The Phase 3.5 engine was reused, not forked** — veto → sufficiency → threshold → baseline, `INSUFFICIENT_DATA` first-class. Three of the five state values are **byte-identical** to 3.5's (`DEGRADED`/`INSUFFICIENT_DATA`/`UNKNOWN`) and a test asserts it against the live 3.5 module, so a rename on either side breaks the build. The terminal-status sets are asserted equal too, rather than copied and hoped for.

**A genuine prompt-vs-repo conflict, resolved and reported.** §4.1 FR-003 asked for `NORMAL/DEGRADED/ANOMALOUS/INSUFFICIENT_DATA/UNKNOWN` while saying, in the same sentence, *"align to the existing set — do not fork a parallel vocabulary"*. The existing set is `HEALTHY/DEGRADED/UNHEALTHY/INSUFFICIENT_DATA/UNKNOWN`, so the proposed vocabulary differs in two of five positions and taking the prompt literally would itself have been the fork it forbids. Resolved by keeping the shared middle exactly and letting the endpoints differ, because they are different claims: 3.5 asks *"is this version fit for more traffic"* (a **fitness** judgement) and 4.5 asks *"has this agent's behavior changed"* (a **deviation** judgement). They come apart in both directions — a p95 that drops 80% overnight is anomalous and in no sense unhealthy; an agent that has failed 30% of its calls every day for a month is unhealthy and not anomalous at all.

**Seven signals, each with a stated rule**: `error_rate_shift`, `policy_denial_surge`, `latency_drift`, `cost_drift`, `tool_failure_spike`, `tool_pattern_shift`, `loop_termination_anomaly`. Three deserve note. **`latency_drift` reports speedups**, because a metric that halves overnight usually means the agent stopped doing something it used to do — the clearest place a *behavioral* signal differs from a *health* signal. **`tool_failure_spike` is per tool and names the worst offender**, because one broken integration among five healthy ones barely moves an aggregate. **`loop_termination_anomaly` is the signal invisible in the error rate**: a model that has started looping is stopped by Phase 5.6a.3's caps working exactly as designed, so nothing errors from the caller's view and only the termination mix shows it — there is a test asserting the loop signal fires while `error_rate_shift` stays NORMAL.

**`tool_pattern_shift` uses total-variation distance**, chosen because it is a stated arithmetic operation on two distributions that an operator can recompute by hand from the breakdown in the finding — not because it is the most sophisticated available.

**Every finding explains itself from its own row.** `explanation` is structured, not prose: metric, both window bounds with their sample counts, observed value, threshold and/or baseline, the crossing in words, and the evidence. A test walks every persisted finding and asserts the record is self-contained.

**The veto has a sharper reason than 3.5's.** 3.5 vetoes because automation reads its verdict to move traffic; this vetoes because **a killed agent's runtime data describes the kill, not the agent** — cancellations from a kill switch would show as a huge spike, and reporting that as ANOMALOUS would raise an alarm about the intervention at the moment an operator is already using it. A test asserts aggregation never runs when the veto fires.

**Connector attribution is deferred and named in every finding.** The runtime has no record of which external system a version depends on (ACT-INT-FR-006 — the runtime-never-knows boundary Phases 3.2, 3.3 and 3.5 each reported in turn), so every finding carries `connector: null` **with a reason string**, rather than omitting the key and letting a reader assume it was never considered. A test asserts the package imports nothing from `app.integration`, so the boundary is intact rather than merely respected. Provider, model and tool attribution are present and tested.

**Findings are signals; 4.3 remains the only enforcer.** Nothing in `app/behavior` writes an execution's status, raises the governance stop or reaches the kill switch — asserted over the AST against the execution-state vocabulary, the same proof Phase 4.4 gave for budgets — and a behavioural test confirms evaluating a wildly anomalous agent leaves every execution row byte-identical. Emission is **non-gating**: a failed evaluation produces no finding and never a STOP, the telemetry-plane posture and the deliberate inverse of 4.3's.

**The 4.4/4.5 cost boundary is demonstrated rather than asserted**, in both directions: per-execution cost doubling while traffic halves leaves total spend flat, so 4.4 sees nothing and 4.5 fires; traffic multiplying at unchanged per-execution cost is the reverse. Neither package imports the other, tested both ways.

**Measured; no index added.** At 115,381 executions the agent-scoped window aggregate runs at **0.57ms p50 / 1.42ms p95**, and the plan is the interesting part: no composite covers `(organization_id, agent_id, created_at)` and Postgres does not need one — it combines Phase 4.2's `ix_agent_executions_org_created` with Milestone 1's `ix_agent_executions_agent` via a `BitmapAnd`. Two indexes built for other queries intersect to answer this one. One shape is **named rather than fixed on suspicion**: `tool_calls` indexes `agent_id` alone, so the window's time bound is a post-index Filter — the shape 4.2 found dangerous — but with 5,210 tool calls platform-wide and 10 for the busiest agent it cannot be demonstrated, and migration `0049` records the revisit trigger (50,000 tool calls for one agent, or a p50 past 10ms) instead of adding an index against a projection.

**Evaluation is the interim trigger, marked for Phase 3.8.** No scheduler was built — that would be the fork this milestone has refused three times. The operation is idempotent at two levels (3.1's `Idempotency-Key` on the request, a unique constraint on `(agent, signal, window)` beneath it) so adopting it into 3.8's scheduler is a registration rather than a rewrite.

Three new routes under `/api/v1/runtime/behavior` and `/api/v1/runtime/agents/{id}/behavior` (559 → **562**), mounted on the existing runtime prefix rather than the prompt's prefix-less `/api/v1/agents/{id}/behavior`, which would have created a second agents namespace beside the established one. **No new permission** — `runtime.telemetry.view` already covered "derived telemetry about an execution stream", reused rather than shadowed (the reasoning 4.2 and 4.4 both applied). One new table, `behavioral_findings`; schema **128 → 129 tables**; no new index on `agent_executions` or `tool_calls`; no new error code (validation reuses `VALIDATION_ERROR`); no new audit event — findings are telemetry-plane records, queryable in their own table, and inventing an audit event for routine emission would misrepresent which plane they belong to. Migration head **`0049_behavioral_signals`**, reversible (downgrade/upgrade verified live). **46 new tests** in `backend/tests/runtime/test_behavioral_signals.py`. Backend **2,129 passed, 0 failed, 1 deselected** (2,083 baseline + 46), 862s; frontend **327 passed** across 49 files, unchanged (the Behavior view is deferred to 4.9). See `docs/runtime/behavioral-signals.md`.

**Phase 4.6 (OpenTelemetry & Metrics Interoperability) shipped 2026-08-31** — Milestone 4's sixth sub-phase. The platform now speaks OpenTelemetry at its boundary, and a collector outage is an observability event, not an execution event.

**The OTel SDK lives behind one adapter package and nowhere else.** New sibling package `app/telemetry_export/` (10 modules: `config`, `buffer`, `health`, `mapping`, `sinks`, `dispatcher`, `metrics`, `service`, `routes`, `schemas`). `sinks.py` is the **only** module in the codebase that imports `opentelemetry`, and it imports it at function scope — `test_ac02_*` in `tests/runtime/test_otel_interop.py` walks the AST of every other package and fails on a stray vendor/OTel import. There is no vendor name anywhere in the code; switching collectors (Datadog → Grafana → Splunk) is editing an `endpoint` config value. Recorded in [ADR-0011](docs/architecture/adr/0011-opentelemetry-export-as-a-fail-open-plane.md), which also extends ADR-0008 (the exported spans are also derived — the domain row still wins any disagreement).

**Export is fail-open and not on the execution path.** `ExportDispatcher` reads executions that have *already reached a terminal state*, assembles their traces through 4.2's `TraceAssembler`, converts them to neutral `ExportSpan` records, buffers them, and flushes to a sink. The runtime is never instrumented — `app/runtime` imports neither `telemetry_export` nor `opentelemetry`, asserted over the AST. Every dispatcher method catches `Exception` and returns; a runtime export failure surfaces only as exporter-health state and a metric (`act_telemetry_export_degraded`), never an exception, never an error code. The §36 proof (`test_ac05_execution_completes_normally_with_the_collector_down`) runs a real execution, drives five dispatch cycles with the collector down, and asserts the execution row is byte-for-byte unchanged, no telemetry/audit row was written by export, the error is visible via `exporter_health`, and the buffer never grew past its cap — plus a companion test against a real `OTLPSpanExporter` on a dead port.

**Bounded buffering — never an unbounded queue.** `BoundedSpanBuffer` caps on *span count* (`TELEMETRY_EXPORT_BUFFER_MAX_SPANS`, default 10,000) with a declared full-policy (`drop_oldest` / `drop_newest` / `block_bounded`); `config.BUFFER_FULL_POLICIES` contains no "grow" option and a test asserts that. The buffer lock is never held across the network export.

**Metrics scrape, not push.** `GET /metrics` returns Prometheus text exposition, **authenticated** (`runtime.telemetry.view`) and **tenant-scoped** — deliberately not the open-unauthenticated-scrape model, which on a multi-tenant platform is a cross-tenant disclosure. Every label routed through `metric_label_set()`, which reuses 4.1's `HIGH_CARDINALITY_ATTRIBUTES`/`SENSITIVE_ATTRIBUTES` denylist plus `METRIC_DIMENSIONS` and a small declared 4.6 extension (`signal_type`, `state`); a structural test parses `metrics.py` and checks every `r.add()` call's keywords.

**No new table, no migration.** Per-environment export config lives in `Environment.policy["telemetry_export"]` (validated on write, `EXPORT_CONFIG_INVALID` on a malformed document); the platform default is `TELEMETRY_EXPORT_*` env vars. Exporter health is in-process and ephemeral — dropped on restart, never a phantom (see `RECOVERY.md`).

**New dependency:** `opentelemetry-sdk==1.44.0` + `opentelemetry-exporter-otlp-proto-http==1.44.0` (in `backend/requirements.txt` under a new "Observability / OpenTelemetry export" section; pulls `requests`, `protobuf`, `googleapis-common-protos`, `charset_normalizer` transitively) — imported only behind the adapter. **§2/§3 re-confirmed unchanged, not re-derived from scratch, since this phase added no migration and no new table** (verified live: `alembic current` still reports `0049_behavioral_signals`; `sqlalchemy.inspect()` still reports **129 tables**). **§5 re-derived live**: routes **562 → 566** (four new: `GET`/`PUT /api/v1/observability/export/config`, `GET /api/v1/observability/export/health`, `GET /metrics`). One new permission (`runtime.telemetry.export.manage`), one new error code (`EXPORT_CONFIG_INVALID`, 422), one new audit event (`RUNTIME_TELEMETRY_EXPORT_CONFIGURED`). **51 new tests** in `backend/tests/runtime/test_otel_interop.py`; **two** existing 4.2 tests updated, both intent-preserving, neither touching production code: `test_ac13_the_observability_prefix_does_not_collide` (bare route count → membership assertions for the two new routes under the shared prefix) and `test_ac06_the_explorer_query_uses_the_index_and_does_not_scan` (named the 0046 composite index specifically; loosened to "an organization-leading index, never a Seq Scan" — its documented invariant — after shared-DB growth made Postgres prefer the compact single-column `ix_agent_executions_org` for the bitmap path on a tiny tenant; **reproduces on unmodified `main`**, 4.6 touches neither the query nor its indexes, and the composite's real benefit stays guarded by the specific companion test `test_ac06_at_volume_the_index_elides_the_sort`). Backend **2,180 passed, 0 failed, 1 deselected** (2,129 baseline + 51); frontend **327 passed** across 49 files, unchanged (the observability center is 4.9). See `docs/observability/opentelemetry.md` and `docs/observability/metrics.md`.

**Phase 4.7 (SLOs, Alert Rules & Incident Signals) shipped 2026-08-31** — Milestone 4's seventh sub-phase, and a discipline-of-restraint phase: the line is "build the signal, not the notification platform".

**No notification delivery was built, and the restraint is enforceable.** New sibling package `app/slo/` (9 modules: `states`, `sli`, `definitions`, `evaluator`, `alerts`, `pipeline`, `schemas`, `routes`, `__init__`). Two AST tests walk the package and fail on any delivery client (`requests`/`httpx`/`smtplib`/`slack`/`pagerduty`/`boto3`/`app.email`/notification service) or any method named `send`/`notify`/`deliver`/`page`. Alert *creation* and external *notification* are separate concepts (§4.7); this phase owns creation. Recorded in [ADR-0012](docs/architecture/adr/0012-alerts-as-signal-creation-not-notification.md).

**SLOs** are an SLI, a target, a window (`1h`/`6h`/`24h`/`7d`/`30d`), and an error budget. Six SLIs (`success_rate`, `timeout_rate`, `provider_error_rate`, `tool_failure_rate`, `latency_p95`, `queue_delay`), each with an exact predicate over `agent_executions`/`tool_calls` — **not** the 4.6 `/metrics` surface (windowed gauges rounded to bounded-cardinality dims); the two share a source of truth, not a query. The `TERMINAL` status set is asserted byte-identical to 3.5's and 4.5's. The objective *direction* is a fixed property of the SLI (`SLI_SPECS`), never a stored column.

**Evaluation reuses the 3.5/4.5 shape**: veto → sufficiency → objective → budget, deterministic and explainable. `INSUFFICIENT_DATA` first-class (below 20 terminal samples, a platform constant like 3.5's/4.5's). Shared state values (`INSUFFICIENT_DATA`, `UNKNOWN`) spelled character-for-character as 3.5/4.5, asserted against the live modules. A full run three times is byte-identical; no `random`/`numpy`/ML importable. Error-budget consumption tracked; a burned budget reads `budget_remaining = 0`.

**One alert lifecycle over two evidence sources (the §18 model decision, mandatory report #1)**: `runtime_alerts` is a **distinct** table that references `slo_evaluations`/`behavioral_findings` by `(source, source_id)` — *not* a shared `runtime_findings` re-homing 4.5's table. 4.5's `behavioral_findings` is untouched and gained no alert columns (asserted). Escalation is **explicit**: SLO → alert only when `BREACHED`; finding → alert only when `ANOMALOUS` (a named constant) — a `DEGRADED` finding stays a finding (§18, AC-08). No "incident" concept.

**Lifecycle** OPEN → ACKNOWLEDGED → RESOLVED → SUPPRESSED, every transition audited; an illegal transition is `ALERT_TRANSITION_INVALID` (409), an identical one converges. **DB-enforced dedup**: `uq_runtime_alerts_active_dedup`, a partial unique index over `(organization_id, dedup_key) WHERE status IN ('OPEN','ACKNOWLEDGED')` — the 3.7 `uq_rollback_events_dedup` primitive. A `RESOLVED` alert re-opens on recurrence (same row); a `SUPPRESSED` one does not. Proven under a real 8-thread / 8-connection Postgres race: one breach → one evaluation row, one alert.

**Signal, not enforcement**: `app/slo` references no kill switch, no governance engine, no execution-state mutation (AST-asserted, the 4.4/4.5 proof); `app/runtime` and `app/workers` import nothing from `app.slo`. Non-gating.

**The interim idempotent evaluate op** (`POST /api/v1/runtime/slos/evaluate`, `Idempotency-Key`-aware) for Phase 3.8's scheduler to adopt (the 4.5/3.7/3.5 pattern); `uq_slo_evaluations_window` makes a re-run a no-op. **No scheduler built.**

**§2/§3/§5 re-derived live**: migration **`0049_behavioral_signals` → `0050_slos_and_alerts`** (three new tables — `slo_definitions`, append-only `slo_evaluations`, `runtime_alerts` — **132 tables** total, `sqlalchemy.inspect()` confirmed; 131 in `Base.metadata`); no existing table changed, no backfill, `alembic downgrade -1`/`upgrade head` re-verified clean. **No index added to `agent_executions`/`tool_calls`** — SLI aggregation reuses the tenant-leading shape 3.5/4.5 already run. Routes **566 → 578** (12 new: SLO CRUD + `/evaluate` + `/evaluations`, alert list/get + acknowledge/resolve/suppress, all under `/api/v1/runtime`). Two new permissions (`runtime.slo.manage`, `runtime.alert.manage`; reads reuse `runtime.telemetry.view`), four new error codes (`SLO_DEFINITION_INVALID` 422 / `SLO_NOT_FOUND` 404 / `ALERT_NOT_FOUND` 404 / `ALERT_TRANSITION_INVALID` 409), five new audit events (`RUNTIME_SLO_CONFIGURED` + four `RUNTIME_ALERT_*`). **41 new tests** in `backend/tests/runtime/test_slos_and_alerts.py`. Backend **2,221 passed, 0 failed, 1 deselected** (2,180 baseline + 41); frontend **327 passed** across 49 files, unchanged (SLO Dashboard + Alert Center are 4.9). See `docs/operations/slos.md` and `docs/operations/alerts.md`.

**Phase 4.8 (Telemetry Privacy, Retention & Access Governance) shipped 2026-09-01** — Milestone 4's eighth sub-phase, and the last policy-heavy one: it makes the telemetry plane a governed, security-sensitive data system rather than a debugging feature. Closes Gate F (privacy).

**Capture policy, four modes, conservative default.** New sibling package `app/telemetry_privacy/` (7 modules: `modes`, `policy`, `redaction`, `content`, `retention`, `schemas`, `routes`). A policy resolves per tenant / environment / agent / data-classification to `METADATA_ONLY` / `REDACTED_CONTENT` / `FULL_CONTENT` / `DISABLED`. Precedence is `classification > agent > environment > tenant > platform-default`, deterministic and returned fully explained by `GET /api/v1/runtime/telemetry/effective-mode`. A production or sensitively-classified scope with no explicit row resolves to `METADATA_ONLY` (`source: "conservative-default"`), never `FULL_CONTENT`; a malformed stored mode is coerced toward `METADATA_ONLY` — a misconfiguration fails toward *less* capture (AC-02).

**Content storage decision (mandatory report #3): a dedicated governed store, not govern-in-place.** New table `trace_content` holds the telemetry copy of an execution's content, materialised on the *first authorised content view and never before*. The domain rows it is derived from (`execution_messages`, `agent_executions.input_payload`/`output_payload`, `tool_calls.input_summary`/`output_summary`) are **domain truth** the tool loop and execution-detail depend on; redacting them in place would corrupt execution state and would break "retention deletes telemetry, never domain truth" (M4-4.8-FR-032). The dedicated store is exactly the "`trace_content` table with a redaction/classification column and its own retention" §5 permits.

**Scrub before persist, in every content mode; no chain-of-thought in any mode.** `redaction.redact_for_capture` runs, in fixed order, `strip_reasoning` (4.1, §7 floor — asserted first in the pipeline by an AST test) → `scrub` (4.1 secret scrubber, §14) → classification field-masking (only for `REDACTED_CONTENT`). `FULL_CONTENT` keeps business content but the secret scrubber still runs — a planted `sk-live-…` key and `Bearer` header never reach `trace_content` in any mode (AC-04). `DISABLED`/`METADATA_ONLY` materialise nothing — `trace_content` stays empty, asserted.

**Content view is a distinct, stronger, audited permission.** `GET /api/v1/observability/traces/{trace_id}/content` requires `runtime.trace.content.view` — registered here for the first time (4.1/4.2 named it in code without registering it), **not** in the read-only bundle, never implied by execute or by `runtime.telemetry.view`. The route depends on the metadata view, resolves the trace, then checks the content permission imperatively (`user_has_permission`) so the **404-vs-403 discipline** holds: a trace absent for the tenant is `TRACE_NOT_FOUND` (404, no cross-tenant existence leak); a trace present but caller lacking the content permission is `TRACE_CONTENT_ACCESS_DENIED` (403). Every successful view emits `RUNTIME_TRACE_CONTENT_VIEWED` with actor + resource ids and **never the payload** (AC-07).

**Retention per class + a safe, idempotent, bounded sweep.** Six classes: `trace_content` / `trace_metadata` / `metrics_aggregate` / `alert_history` (deletable → `trace_content` / `runtime_events` / `slo_evaluations` / terminal `runtime_alerts`) and `governance_decision` / `financial_record` (**retain-only** — configurable minimum, never deleted; evidence outlives payloads, §24). `RetentionSweeper.run` deletes in committed 1,000-row batches capped at 50 batches/class/run; a second run continues where the first stopped (`POST /api/v1/runtime/telemetry/retention/run`, `Idempotency-Key`-aware, the 4.5/4.7/3.5 interim pattern — **no scheduler built**). A purge removes the `trace_content` rows while the `agent_executions` and `execution_messages` rows survive (AC-08).

**Non-gating; planes separate.** `app/telemetry_privacy` references no kill switch, no governance engine, no execution-state mutation (AST-asserted); the retention sweep names only telemetry tables in a `delete()`. A capture/retention operation never stops or alters an execution.

**§2/§3/§5 re-derived live**: migration **`0050_slos_and_alerts` → `0051_telemetry_privacy`** (three new tables — `telemetry_capture_policies`, `telemetry_retention_policies`, `trace_content` — **135 tables** total, `sqlalchemy.inspect()`; 134 in `Base.metadata`); no existing table changed, no backfill, `alembic downgrade -1`/`upgrade head` re-verified clean. Routes **578 → 588** (10 new: capture-policy CRUD `GET`/`POST`/`GET`/`PATCH`/`DELETE`, `GET /effective-mode`, retention `GET`/`POST /retention-policies` + `POST /retention/run`, `GET /observability/traces/{id}/content`). Three new permissions (`runtime.trace.content.view`, `runtime.telemetry_policy.view`, `runtime.telemetry_policy.manage`); four new error codes (`TELEMETRY_POLICY_INVALID` 422 / `TELEMETRY_POLICY_NOT_FOUND` 404 / `RETENTION_POLICY_INVALID` 422 / `TRACE_CONTENT_ACCESS_DENIED` 403); four new audit events (`RUNTIME_TELEMETRY_POLICY_CHANGED`, `RUNTIME_TRACE_CONTENT_VIEWED`, `RUNTIME_TRACE_EXPORTED`, `RUNTIME_TELEMETRY_RETENTION_RUN`). **36 new tests** in `backend/tests/runtime/test_telemetry_privacy.py`; **two** existing tests updated, both intent-preserving: `test_ac04_the_content_permission_is_not_registered` → `_is_owned_by_phase_4_8` (4.8 is the owner that registers it) and `test_ac13_the_observability_prefix_does_not_collide` (the new content route added to the expected set, as 4.6 added export). Backend **2,257 passed, 0 failed, 1 deselected** (2,221 baseline + 36); frontend **327 passed**, unchanged (a Telemetry Policy admin view assembles in 4.9). See `docs/observability/privacy.md`, `docs/observability/retention.md`, and [ADR-0013](docs/architecture/adr/0013-trace-content-capture-and-access-policy.md).

**Phase 4.9 (Enterprise Runtime Governance & Observability Center) shipped 2026-09-01** — Milestone 4's ninth sub-phase. The operator control plane where all of Milestone 4 becomes visible; the M4 analogue of M3's Release Operations Center (3.10), built to the same read-and-trigger pattern. Closes Gate M.

**Nine per-persona operator views.** New frontend module `frontend/src/modules/observability/` (9 pages: `RuntimeOverviewPage`, `TraceExplorerPage`, `TraceDetailPage`, `CostCenterPage`, `GovernanceDecisionsPage`, `BehaviorAnomaliesPage`, `SloDashboardPage`, `AlertCenterPage`, `TelemetryPolicyPage`; plus `personas.ts`, `components/ObservabilityNav.tsx`, `components/badges.tsx`, `index.ts` — 13 `.ts`/`.tsx` files) and one new service (`frontend/src/services/observabilityService.ts`). Reuses the 3.10 pattern and components — `ConfirmActionDialog` / `useGuardedAction` imported verbatim from `@/modules/operations`; no new design system. A persona lens (7 personas, §29) narrows the visible views; it never grants access (each view is permission-gated, the server is the authority) and the choice is a `localStorage` convenience.

**Content governance inherited from 4.8, in full (mandatory report).** The Trace Detail content pane is the only place content is rendered. It shows content only to `runtime.trace.content.view` holders — a metadata-only operator sees the timeline plus a truthful gated message, never the content, never a misleading error, and the client does not issue the request in that case. It fetches content **exclusively** through 4.8's `GET /observability/traces/{trace_id}/content` (the endpoint with the `RUNTIME_TRACE_CONTENT_VIEWED` audit) — no bypass route, a test asserts exactly one `apiClient` `/content` call in the service, and content is fetched only on explicit click. Capture mode is truthful (`METADATA_ONLY`/`DISABLED` → "no content captured — the policy working, not an error"; `REDACTED_CONTENT`/`FULL_CONTENT` → items flagged `redacted`/`secret scrubbed`). 404-vs-403 honoured.

**Read + trigger; dangerous actions gated.** Every action is an existing 4.x operation (alert ack/resolve/suppress → 4.7; capture-policy edit + retention run → 4.8) — the server authorizes, isolates by tenant, honours idempotency, audits. Suppress-with-reason, raise-capture-to-a-content-mode (type-to-confirm), run-retention are confirmation-gated; setting `METADATA_ONLY`/`DISABLED` is plain. Truthful state throughout (burned budget "spent" in red, `INSUFFICIENT_DATA` its own warning state, degraded-exporter banner). A concurrency conflict surfaces and refreshes, never a silent retry.

**Two new read-only backend aggregation endpoints** — `app/runtime/observability_center.py` (`ObservabilityCenterReadModel`), read-only by construction with an AST test that fails on any `add`/`commit`/`delete`/`flush`, wired via two routes in `app/runtime/routes.py`: `GET /api/v1/runtime/overview` (`runtime.telemetry.view` — the fleet composite in one request; `success_rate` is `null` not 0 below the 20-sample floor) and `GET /api/v1/runtime/governance/decisions` (`runtime.execution.view` — the tenant-wide, filterable governance-decision list 4.3 exposed only per execution; `reason` is a platform-templated sentence, never a prompt or model output). Both tenant-scoped. **Exporter health is deliberately not in the overview composite** — it belongs to the 4.6 OpenTelemetry export plane that `app/runtime` never imports (`test_ac09_export_health_is_never_read_by_governance_or_the_runtime` greps the whole `app/runtime` tree for `exporter_health` / `telemetry_export`); the screen fetches it from 4.6's `GET /observability/export/health` directly.

**§2/§3/§5 re-derived live**: **no migration, no new table** — this phase reads existing data; `alembic current` still reports `0051_telemetry_privacy`, `sqlalchemy.inspect()` still 135 tables. Routes **588 → 590** (`+2`: `GET /api/v1/runtime/overview`, `GET /api/v1/runtime/governance/decisions`). **No new permission**, **no new error code, no new audit event** (actions reuse the endpoints' own). **8 new backend tests** (`backend/tests/runtime/test_observability_center.py`) + **32 new frontend tests** (`frontend/src/modules/observability/tests/observability.test.tsx`, incl. the content-governance set). Backend **2,265 passed, 0 failed, 1 deselected** (2,257 baseline + 8); frontend **359 passed** across 50 files (327 baseline + 32). No existing test changed. See `docs/operations/observability-center.md`.

**Phase 4.10 (Enterprise Hardening & the Milestone 4 Proof) shipped 2026-09-02** — Milestone 4's tenth and final sub-phase. **A proof phase, not a feature phase. MILESTONE 4 IS COMPLETE (10/10).** No product capability, no migration, no new table/route/permission/error-code/audit-event. One new file: `backend/tests/runtime/test_milestone_4_proof.py` (15 tests). No existing test changed.

**The §33 end-to-end proof** (`test_ac01`–`test_ac05`): one `pytest` fixture (`governed_run`) configures a real execution — a deployed & published agent version, a real priced model (unique name, `PricingService.set_price`), a function tool, a governance cost policy (`max_execution_cost`/`min_remaining_cost` headroom rule), a `HARD_LIMIT` budget, a `REDACTED_CONTENT` capture policy, and an input payload carrying a sensitive-named field and a planted `sk-live-…` secret — and runs it via `POST /executions`. **Every assertion is a real effect of the run** (`test_ac05` proves this structurally by AST-scanning the fixture: it only ever `add`s a `Budget` and a `TelemetryCapturePolicy` — never an outcome row). The execution routes to the deployed version, a worker claims it (`execution_attempts` row + `started_at`), the first model call records real tokens + a real priced cost, the model requests a tool, governance evaluates mid-loop and permits it (the `tool_calls` row is the evidence — a denied call never reaches it), a second model call begins, and **as spend approaches the bound the governance engine STOPs the loop** (`GOVERNANCE_EXECUTION_STOPPED`, one `STOP` decision, `MIN_REMAINING_COST`, the mock model called exactly twice, `loop_iterations == 2`); the **budget stays within bound** (reserved ≤ limit; actual spend 0.002 ≤ 0.0025); the **trace reconstructs** (`TraceAssembler` spans `execution`/`model_call`/`tool_call`, root status the real `FAILED`); **sensitive fields are redacted** (the secret is absent from `trace_content` *and* from the serialised OTLP wire bytes; the `prompt` field is masked); **metrics reflect it** (`act_runtime_executions{status="FAILED"}` on `GET /metrics`); the **decision is audited** (`RUNTIME_EXECUTION_STOPPED`); and the **telemetry is OTLP-valid** (round-tripped through `encode_spans` → `SerializeToString` → re-parse; every span a 16-byte trace id + 8-byte span id).

**The three enterprise proofs, at the milestone level** (elevating the phase-level tests, not duplicating): **§34** (`test_ac06`) — two orgs with sensitive traces; org B reaches none of org A's metadata, content, or existence, including via 4.9's `/runtime/overview`, `/governance/decisions` and `/cost/summary`; a known trace id → 404. **§35** (`test_ac07`) — twelve concurrent workers on **real separate `SessionLocal()` connections** race a $1.00 budget at $0.25/execution; exactly four fit; `sum(reserved) ≤ 1.00`, decided by `SELECT … FOR UPDATE`. **§36** — telemetry **fails open** (`test_ac08`: a real execution `SUCCEEDED` with a `Failing` sink; five cycles later the exporter is `degraded` with a visible `last_error`, the bounded buffer's `span_count ≤ capacity_spans`, and the execution row's `status`/`cost_amount` are unchanged) and governance **fails closed** (`test_ac09`: a mandatory constraint that raises at `BEFORE_FIRST_MODEL_CALL` STOPs the execution with `GOVERNANCE_CHECKPOINT_UNEVALUABLE`, a `STOP` decision with `CHECKPOINT_UNEVALUABLE`, and `loop_iterations == 0` — it never proceeds ungoverned).

**Hardening** (`test_ac10`–`test_ac11`): the trace explorer stays capped and `EXPLAIN`-verified free of a `Seq Scan on agent_executions` at 400 executions/tenant; `/metrics` never emits an unbounded label across 200 executions with 40 distinct error codes; a simulated restart (a fresh `Session`) leaves the governance policy, budget, capture policy and an `OPEN` alert all intact and effective.

**The §41 gate-closure audit** (`test_ac12`): all fifteen gates A–O map to a named passing proof. The A–O letter↔concern mapping is reconstructed from the per-phase `Gate X` references in the phase docs/tests plus the 4.10 build prompt — **the SRS §41 consolidated table is not carried in the repo** (added to §9 as a gap). See `docs/runtime/milestone-4-proof.md`.

**The one finding, and it is not a defect.** The first end-to-end run of the §36 governance proof left the execution `QUEUED` rather than terminally `FAILED`. Reading `ExecutionWorkerService._fail_or_retry` (`app/runtime/services.py`) confirmed this is **documented intentional behaviour** — its own comment reads *"GOVERNANCE_CHECKPOINT_UNEVALUABLE is deliberately not in [the non-retryable] set … a transient dependency failure is exactly what the retry policy exists for."* Fail-closed is about the *attempt* (the loop never ran past the checkpoint, `loop_iterations == 0`); the requeue is the retry policy. `test_ac09` asserts the honest property (`status ∈ {FAILED, QUEUED, BLOCKED, DEAD_LETTERED}`, never `SUCCEEDED`) rather than a weakened one. **No product code was changed by this phase.**

**§2/§3/§5 re-confirmed unchanged, not re-derived** — this phase added no migration, no table, no route: `alembic current` still `0051_telemetry_privacy`, `sqlalchemy.inspect()` still **135 tables**, live `APIRoute` count still **590**. Backend **2,280 passed, 0 failed, 1 deselected** (2,265 baseline + 15); frontend **359 passed**, unchanged (backend-only phase). **Gate O — the entire M1/M2/M3/4.1–4.9 suite passes unchanged — is itself the proof.**

**🏁 MILESTONE 4 — Enterprise Runtime Governance & Observability — is COMPLETE (10/10).** All fifteen completion gates A–O closed. The platform executes real governed AI (M1), integrates with the enterprise both ways (M2), deploys and operates versions safely at production scale (M3), and **understands, governs, controls, investigates and financially manages what its agents do while they execute (M4)** — demonstrated end to end.

**Phase M4.11 (Production Integrity Closure — key-material recovery & fail-loud integrity) shipped 2026-09-02** — a bounded integrity/recovery phase between Milestone 4 and Milestone 5, closing the one verified production hazard that blocked M5: ACT's encryption and signing key material was in no backup, and a missing key at startup was *silently regenerated*, rendering every pre-existing ciphertext undecryptable with nothing failing loudly.

**What changed.** New package `app/security/` (M4.11: `errors`, `encryption_provider`, `install_mode`, `canary`, `key_integrity`, `bootstrap`, `keys` (CLI), `backup`; M4.11a adds `installation`). One new model `app/models/security.py::KeyMaterialCanary` (M4.11a adds `InstallationBootstrap`). `credential_crypto.py` and `LocalKeyProvider` **no longer generate a key when one is absent** — generation moved behind an explicit `bootstrap()` / `python -m app.security.keys bootstrap` that refuses to run on an EXISTING install. The Fernet key is now obtained through an `EncryptionKeyProvider` seam mirroring `SigningProvider` (`LocalEncryptionKeyProvider` default; KMS/Vault adapter documented as future work, no vendor SDK in core). The key ring is a `MultiFernet` over the active key + `MODEL_CREDENTIAL_ENCRYPTION_KEYS` (retained prior keys) — additive rotation, no big re-encrypt transaction. `app.main` lifespan calls `verify_key_material(db)` before serving: a restored DB with rows and no key is EXISTING → fail loud (`KeyMaterialError`, deterministic, no secret in the message), never bootstrap. A `key_material_canary` verifier row + a trial-decrypt of real ciphertext catch a *wrong* key too. Signing: the configured default identity must still have a usable private key whose public half matches the database. *(M4.11's NEW-vs-EXISTING rule — "presence of encrypted state in the database" — was corrected by M4.11a; see below.)*

**§2/§3/§5 re-derived live**: migration **`0051_telemetry_privacy` → `0052_key_material_canary`** (one new table `key_material_canary`, `uq_key_material_canary_purpose_fp`); no existing table changed, no backfill, `alembic downgrade -1`/`upgrade head` re-verified clean; **changes no decrypt behaviour for any existing row**. Routes **unchanged at 590** (the operator surface is a CLI, `python -m app.security.keys`, not HTTP). New settings: `ENCRYPTION_KEY_PROVIDER`, `MODEL_CREDENTIAL_ENCRYPTION_KEYS`, `KEY_MATERIAL_FAIL_LOUD`, `ENCRYPTION_KEY_ALLOW_BOOTSTRAP`. **23 new tests** in `backend/tests/runtime/test_key_material_integrity.py` (incl. the §16 end-to-end production-integrity proof); **one** existing test's fixture adapted, intent-preserving. Backend **2,303 passed, 0 failed, 1 deselected** (2,280 baseline + 23); frontend **359 passed**, unchanged. See `docs/security/key-management.md`, `RECOVERY.md`, and [ADR-0014](docs/architecture/adr/0014-key-material-recovery-and-fail-loud-integrity.md).

**Phase M4.11a (Install-Mode Classification Hardening — the durable bootstrap marker) shipped 2026-09-03** — a corrective phase for M4.11. Its NEW-vs-EXISTING classification inferred "NEW" from the *absence of encrypted state*, so an established install holding zero encrypted credential rows could be misread as new and — under `ENCRYPTION_KEY_ALLOW_BOOTSTRAP` — silently handed a fresh cryptographic identity after key loss.

**What changed.** New module `app/security/installation.py` (the durable `installation_bootstrap` marker: `marker_present`, `record_bootstrap`). New model `app/models/security.py::InstallationBootstrap` (singleton, `ck_installation_bootstrap_singleton_true`; non-secret provenance only). `detect_install_mode` corrected: **EXISTING iff the marker is present OR any encrypted/signed state exists; NEW iff the marker is absent AND no such state** — a missing key file never by itself implies NEW. `key_integrity.py` gained a **five-state `KeyState` taxonomy** (`KEY_ABSENT` / `KEY_MALFORMED` / `KEY_PRESENT_BUT_WRONG` / `KEY_PROVIDER_UNAVAILABLE` / `INSTALLATION_NEVER_BOOTSTRAPPED`) each with a distinct operator-safe error code and no material leak, plus two half-init guards (`INSTALLATION_BOOTSTRAP_INCOMPLETE`, `ENCRYPTION_KEY_UNVERIFIED_ESTABLISHED_INSTALL`). New `EncryptionKeyProvider.check_available()` seam + `ProviderUnavailableError` — a provider outage fails loud (`ENCRYPTION_KEY_PROVIDER_UNAVAILABLE`) and is *not* an install-mode signal. `bootstrap_key_material` now records the marker atomically with the canary + signing identity and *adopts* an operator-supplied key. Startup **backfills** the marker (`recorded_via="backfill"`) once an existing key is verified — no window in which an M4.11-bootstrapped install reads as NEW.

**§2/§3/§5 re-derived live**: migration **`0052_key_material_canary` → `0053_installation_bootstrap`** (one new table `installation_bootstrap`, `uq_installation_bootstrap_singleton` + `ck_installation_bootstrap_singleton_true`; **137 tables** total, `sqlalchemy.inspect()`; 136 in `Base.metadata`); no existing table changed, no data backfill in the migration, `alembic downgrade -1`/`upgrade head` re-verified clean; **changes no decrypt behaviour for any existing row**; classification only *tightens* (nothing EXISTING becomes NEW). Routes **unchanged at 590**. No new setting, permission, error code in the identity taxonomy, or audit event. **14 new tests** appended to `test_key_material_integrity.py` (`test_m411a_*`); **two** existing M4.11 tests adjusted intent-preserving (`test_ac06` error-code assertion renamed; `test_ac18` extended with the zero-ciphertext leg — never weakened). Backend **2,317 passed, 0 failed, 1 deselected** (2,303 baseline + 14); frontend **359 passed**, unchanged. See `docs/security/key-management.md`, the amended `RECOVERY.md`, and the **M4.11a amendment** to [ADR-0014](docs/architecture/adr/0014-key-material-recovery-and-fail-loud-integrity.md). **With M4.11a the production-integrity prerequisite for Milestone 5 is satisfied without the residual edge case.**

---

**Phase 5.1 / M5.1 (Universal Agent Asset Model + Ownership) shipped 2026-09-04** — **Milestone 5 (Universal Agent Control & Security Fabric) begins here.** Its foundation: the one canonical `agents` registry is extended in place so it can truthfully describe native, external, discovered, claimed, registered, governed and unknown agents — **no second registry**. It builds *no* discovery, graph, MCP, posture, threat, containment, external-gateway or command-center machinery (those are 5.2–5.9); it only makes the model able to *say* the truth.

**What changed.** New module `app/runtime/registry/control.py` — `AgentProvenanceService.record_external_agent` (the 5.2 discovery seam; **no HTTP route** in M5.1) and `AgentControlStateService` (the server-authoritative claim + control-state machine). `agents` gains **seven additive columns** (migration `0054_agent_asset_model`): `control_state` (`DISCOVERED`/`CLAIMED`/`REGISTERED`/`GOVERNED`, `ck_agents_control_state`, `ix_agents_control_state`, default `GOVERNED`), `origin_category` (`NATIVE`/`EXTERNAL`/`UNKNOWN`, `ck_agents_origin_category`, default `NATIVE`), `origin_provider` (soft vendor string — **not** a DB enum — default `ACT_NATIVE`), and discovery metadata `first_observed_at` / `last_observed_at` / `discovery_source_ref` / `discovery_confidence` (**columns only**; 5.2 populates them; native rows leave them `NULL`). **`control_state` is a DISTINCT dimension from `lifecycle_status`** — the existing 13-state lifecycle machine (`app/runtime/registry/services.py::_TRANSITIONS`) is byte-for-byte unchanged and every one of its ~27 consumers reads it identically; a native agent is `GOVERNED` and may be in any lifecycle state. `control_state` and `origin_*` are **absent from every write schema** (`AgentRegistrationCreate`, `AgentRegistryUpdate`) — a client cannot mass-assign them; they move only through `POST /agents/{id}/claim` and `POST /agents/{id}/control-state`, both authorizing via the existing `AuthorizationGateway`, locking the row `FOR UPDATE`, writing the existing `AgentOwnershipHistory` ledger and audit trail, tenant-scoped by `get_or_404` (cross-tenant → 404, no existence leak; in-tenant-unpermitted → 403). Claiming ⇒ `CLAIMED` (responsibility), never `GOVERNED` (enforcement — that's 5.7). Ownership reuses the three existing owner columns + `AgentOwnershipHistory` + org hierarchy — **no new ownership table** (the gap analysis found no genuine gap). Provenance is *not* `registration_source` (which means *how the row was created* — `MANUAL`/`IMPORT` — orthogonal).

**§2/§3/§5 re-derived live**: migration **`0053_installation_bootstrap` → `0054_agent_asset_model`** — **seven additive columns on `agents`, no new table** (`sqlalchemy.inspect()` still **137 tables**, 136 in `Base.metadata`); idempotent backfill sets every pre-existing agent row (58,044 live) to `control_state=GOVERNED, origin_category=NATIVE, origin_provider=ACT_NATIVE` — every existing agent *is* native and governed, so this states a known truth, not an inference; `alembic downgrade -1`/`upgrade head` re-verified clean; **no existing-row behaviour change**, every `lifecycle_status` consumer reads identically. Routes **590 → 593** (+3: `GET`/`POST /agents/{id}/control-state`, `POST /agents/{id}/claim`). Two new permissions (`runtime.agent.claim`, `runtime.agent.control.manage`), three new error codes (`AGENT_CLAIM_CONFLICT`, `CONTROL_STATE_TRANSITION_INVALID`, `CONTROL_STATE_TRANSITION_FORBIDDEN`), two new audit events (`RUNTIME_AGENT_CLAIMED`, `RUNTIME_AGENT_CONTROL_STATE_CHANGED`). **33 new tests** in `backend/tests/runtime/test_agent_asset_model.py` (AC-01..AC-17 + the §22 end-to-end proof + real-Postgres concurrency + the mass-assignment negative + the external-agent forward-compat invariant); **no existing test weakened**. Backend **2,350 passed**, 0 failed, 1 deselected (2,317 baseline + 33); frontend **359 passed**, unchanged (no frontend touched — all M5 UI deferred to 5.8). See `docs/runtime/registry/asset-model.md`, `docs/runtime/registry/domain-model.md`, `docs/runtime/registry/api.md`, and [ADR-0015](docs/architecture/adr/0015-universal-agent-asset-model-and-control-state.md).

---

**Phase 5.2 / M5.2 (Agent Discovery Framework) shipped 2026-09-05** — **the first Milestone 5 phase that reaches outside ACT.** 5.1 laid the canonical asset model on the local DB; 5.2 builds the vendor-neutral framework that populates it: adapters that observe agents in external systems, append-only **observations** (evidence, not truth), and **reconciliation** that *derives* canonical `agents` state from that evidence — plus one real, non-mocked reference adapter proving the whole path.

**What changed.** New sibling package `app/discovery/` (mirrors `app/integration/` and `app/runtime/`): `adapters/base.py` (the `DiscoveryAdapter` ABC — `describe`/`validate_configuration`/`build_client`/`fetch`/`normalize`; **`fetch()`'s signature accepts no `Session`/`db` parameter at all** — the structural half of the no-lock-across-external-I/O proof), `adapters/registry.py` (a fixed, decorator-populated dict, mirroring the scheduler's `handler_key` security property), `adapters/http_agent_registry.py` (**THE reference adapter**, `HTTP_AGENT_REGISTRY` — a generic HTTP paginated-agent-list client, proven against a real local `http.server`, never a vendor catalog), `service.py` (`DiscoverySourceService` + `DiscoveryRunService` — three short transactions per sweep: start → COMMIT, fetch via `GovernedHttpClient` with **no database session in scope at all**, persist observations (each its own SAVEPOINT) + reconcile, each its own short commit), `reconciliation.py` (`ReconciliationService` — deterministic exact-`external_reference` matching, a fixed `0.75` confidence threshold, three outcomes only: CREATE a `DISCOVERED` agent through the 5.1 `AgentProvenanceService` seam, LINK (discovery metadata only — ownership/control_state/lifecycle_status never touched), or FLAG a `discovery_findings` row — no silent merge/split, ever; plus staleness: an agent missing from enough sweeps gets an `OPEN` `STALE_AGENT` finding, **never a deletion**, auto-resolved on re-observation). Four new tables (migration `0055_agent_discovery`): `discovery_sources`, `discovery_runs`, `discovery_observations` (**append-only** — `UPDATE`/`DELETE` revoked from `PUBLIC`, mirroring migration `0047`'s `runtime_governance_decisions` precedent exactly), `discovery_findings`. New scheduler handler `discovery.sweep` (Phase 3.8, no new scheduler). New API surface under `/api/v1/discovery` (sources/runs/findings CRUD + a synchronous, `Idempotency-Key`-aware manual-trigger endpoint) — **no route writes `agents` directly**. Two permissions (`discovery.source.view`/`.manage`, mirroring the connector precedent), seven error codes, eight audit events.

**The no-DB-lock-across-external-I/O proof is both structural and behavioral** — the load-bearing rule for this phase, extending the M1 deadlock lesson to external discovery I/O for the first time. Structural: every registered adapter's `fetch()` signature is asserted, via `inspect.signature`, to carry no `Session`-typed or `db`-named parameter. Behavioral: a real local HTTP server's response is held open mid-fetch while a *separate* Postgres session issues a real `SELECT ... FOR UPDATE` against the very `discovery_sources` row the sweep is using — it completes immediately, proving no lock is held across the call (the exact M1 deadlock shape, proven absent).

**§2/§3/§5 re-derived live**: migration **`0054_agent_asset_model` → `0055_agent_discovery`** — **four new tables, no existing table changed** (`sqlalchemy.inspect()` now **141 tables**, 140 in `Base.metadata`); `alembic downgrade -1`/`upgrade head` re-verified clean; no data backfill (a genuinely new domain, nothing to backfill). Routes **593 → 603** (+10, all under `/api/v1/discovery`). **35 new tests** in `backend/tests/discovery/test_discovery_framework.py` (AC-01..AC-21 + the §15 end-to-end proof, incl. real-Postgres concurrency, the no-lock behavioral proof, and a genuine non-mocked reference-adapter run against a real local `http.server`); **no existing test weakened**. Backend **2,385 passed**, 0 failed, 1 deselected (2,350 baseline + 35); frontend **359 passed**, unchanged (no frontend touched — all M5 UI deferred to 5.8). See `docs/discovery/framework.md`, `docs/discovery/reconciliation.md`, and [ADR-0016](docs/architecture/adr/0016-discovery-evidence-vs-canonical-truth.md).

---

**Phase 5.3 / M5.3 (Identity, Delegation & Trust Graph) shipped 2026-09-08** — **the pivotal Milestone 5 phase.** 5.1 built the canonical asset model; 5.2 built discovery; 5.3 builds the **relational control-graph substrate** — typed edges connecting existing node rows (humans, agents, identities, tools, credentials, resources, orgs) — plus **authority-chain reconstruction** (who authorized this action, under whose delegated authority, through which identity, down to which resource) generalizing the Phase 4.2 human→agent→model→tool trace into a first-class recursive query.

**The load-bearing architectural decision, tested for the first time: NO GRAPH DATABASE.** The graph is **relational** — one typed edge table (`control_graph_edges`) + recursive CTEs (`WITH RECURSIVE` over that table and over `agent_executions.parent_execution_id`) + **no materialized projection** (a recorded measurement in `test_ac03` shows assembly is well within budget, the ADR-0008 discipline). No Neo4j, no second datastore. See [ADR-0017](docs/architecture/adr/0017-relational-control-graph-no-graph-database.md).

**The sharpest new isolation property: per-hop tenant-bounded traversal.** Every recursion step re-applies `organization_id = :tenant`; because every edge is validated same-tenant on *both* endpoints at creation, that predicate is a complete bound for service-created edges, and for a hostile direct-inserted row the traversal additionally truncates at the first node that does not resolve in-tenant — it stops at the tenant edge. Proven adversarially (`test_ac06_*`): a planted cross-tenant edge does not extend a chain or a reachability result across the boundary.

**The graph represents; it never creates.** A **trust edge** is created explicitly but only between two nodes that already exist in the caller's tenant, and grants nothing (`AuthorizationGateway` stays authoritative). A **delegation edge** mirrors a row `DelegationService` already owns (a `delegations` row), with `evidence` pointing back at it — 5.3 writes `delegations` never, and invents no agent→agent delegation: `AGENT_DELEGATES_TO` is a declared edge type with **no producer** this phase, because the runtime has no agent→agent invocation (`request_execution_as_agent` is self-only). This runtime-never-knows boundary is recorded in the ADR.

**What changed.** New sibling package `app/graph/` (`nodes.py` — the `(type,id)`↔table map + tenant-scoped node resolution; `traversal.py` — `traverse()` (bounded/cycle-safe/per-hop-tenant reachability) + `reconstruct_authority_chain()`; `service.py` — `ControlGraphService` (edge management through `AuthorizationGateway`) + `AuthorityChainService`; `schemas.py`/`routes.py`). One new table (migration `0056_control_graph`): `control_graph_edges` (typed, directed, soft-revoked, optionally time-bounded; `uq_control_graph_edges_active` partial unique index makes create idempotent and a concurrent double-create resolve to one live edge; bidirectional `(org, source)` / `(org, target)` indexes). New API surface under `/api/v1/graph` (edge list/read, trust-edge + delegation-edge create, edge revoke, authority-chain reconstruction, reachability, node-type vocabulary) — **no route grants authority, no route writes `agents` or `delegations`**. Two permissions (`graph.view`/`.manage`), seven error codes, five audit events (edge create/revoke for trust + delegation, and authority-chain reconstruction; reading a chain is not a change and is not audited).

**§2/§3/§5 re-derived live**: migration **`0055_agent_discovery` → `0056_control_graph`** — **one new table, no existing table changed** (`sqlalchemy.inspect()` now **142 tables**, 141 in `Base.metadata`); `alembic downgrade -1`/`upgrade head` re-verified clean; no data backfill (a genuinely new domain). Routes **603 → 611** (+8, all under `/api/v1/graph`). **34 new tests** in `backend/tests/graph/test_control_graph.py` (AC-01..AC-18 + the §14 end-to-end proof, incl. real-Postgres concurrency for edge create/revoke races, the adversarial per-hop tenant-bound proof, and cycle-safety). **Two pre-existing tests updated, intent-preserving** (M5.2 `test_ac01`'s moving-target migration-head guard → a subset check; M5.1 `test_ac12_no_discovery_or_graph_machinery_shipped` now expects `app/graph/` + `control_graph_edges` as legitimate, while still asserting no runtime-embedded graph engine and no parallel `agents` registry); **no existing test weakened**. Backend **2,419 passed**, 0 failed, 1 deselected (2,385 baseline + 34); frontend **359 passed**, unchanged (all M5 UI deferred to 5.8). See `docs/graph/control-graph.md`, `docs/graph/authority-chain.md`, and [ADR-0017](docs/architecture/adr/0017-relational-control-graph-no-graph-database.md).

---

**Phase 5.4 / M5.4 (MCP / Tool / Credential / Resource Dependency Graph) shipped 2026-09-09** — the phase where the control graph becomes a **security** capability. 5.3 built the relational edge substrate + authority-chain reconstruction; 5.4 extends it with **dependency edges** (agent→tool / →credential / →MCP-server / →connector / →resource, MCP→tool, tool→credential, tool→resource, credential→resource) and delivers the **blast-radius queries** the milestone points at: *which agents can reach payroll · what breaks if this credential is revoked · which agents depend on this MCP server · which agents can reach a resource of kind K.*

**MCP is a first-class, security-relevant dependency — represented via the existing `Tool` domain, NOT a second tool registry** ([ADR-0018](docs/architecture/adr/0018-mcp-representation-via-tool-domain.md)). New `mcp_servers` table (identity, provenance, trust/approval state, version, endpoint ref) holds **no tools**; the tools an MCP server exposes are ordinary `tools` rows linked by **one additive nullable column, `tools.mcp_server_id`**. One tool registry, one M1 schema-validation path, one M1 gateway. The agent↔MCP and MCP↔tool relationships are dependency *edges*.

**Observed vs declared dependency is distinguished** — each edge carries `evidence.mode` ∈ {OBSERVED (from `tool_calls`), DECLARED (from `agent_tools` / `tool_credentials` / `tools.mcp_server_id` / the version's model provider)}. There is deliberately **no `agent→model` edge**: a model is a `{provider, model}` string, not a governed row (the `AGENT_DELEGATES_TO`-style restraint); the model-provider dependency surfaces as `DEPENDS_ON_CREDENTIAL` against the `provider_credentials` row.

**Blast-radius reuses the 5.3 recursive-CTE machinery** — `traverse_with_edges` is `traverse` plus an accumulated edge-id array hydrated into an explainable path (each hop names its edge + evidence). Same per-hop `organization_id = :org` bound (re-proven adversarially for dependency edges), same path-array cycle guard, same `MAX_TRAVERSAL_DEPTH = 32`, same out-of-tenant truncation. No new traversal engine. Every answer names its path, carries an explicit `incomplete` flag (never falsely "empty"), and is a read that grants nothing.

**MCP trust as evidence, not enforcement** — a server's `trust_status` other than `APPROVED` (`PENDING`/`UNKNOWN`/`REJECTED`) makes an agent's dependency on it a surfaced *finding-evidence* (`GET /graph/blast-radius/unapproved-mcp`). Unknown ≠ safe. The posture finding is 5.5's job; 5.4 surfaces the evidence.

**The §V graph-at-scale benchmark — NO materialised projection.** `test_ac08_v_blast_radius_benchmark_at_scale` builds a single-busy-tenant fixture (~12k agents, ~128k dependency edges), `ANALYZE`s it, and times the four blast-radius queries: **40–274 ms**. ADR-0017's "no projection" call holds, now backed by a real measurement at scale (exactly what ADR-0017 asked 5.4 to produce). A cold bulk load (stale planner stats) plans at ~20 s — a statistics artefact, not algorithmic; does not occur under autovacuum-maintained stats. Numbers recorded in [ADR-0018](docs/architecture/adr/0018-mcp-representation-via-tool-domain.md) and `docs/graph/blast-radius.md`.

**What changed.** `app/graph/` gains `mcp.py` (`McpServerService`), `dependencies.py` (`DependencyGraphService` — derive edges from evidence + declared-edge create + revoke), `blast_radius.py` (`BlastRadiusService` — the four §8 queries + `unapproved_mcp_dependencies`); `traversal.py` gains `traverse_with_edges` (the explainable blast-radius walk). New model `McpServer` (`app/models/graph.py`); one additive column `Tool.mcp_server_id`. Migration `0057_dependency_graph`: `mcp_servers` (new), `tools.mcp_server_id` (additive nullable FK), and the `control_graph_edges` `source_type`/`target_type`/`edge_type` CHECK constraints **widened in place** (drop + recreate, no column altered) to admit the new node types (`MCP_SERVER`, `CONNECTOR`) and the 9 dependency `edge_type` values. New API under `/api/v1/graph` (MCP server register/list/get/trust/link-tool, agent dependency list/rebuild, dependency-edge create/revoke, four blast-radius endpoints + unapproved-mcp) — **no route grants authority; MCP-trust management reuses `graph.manage` (no new permission)**. Three new error codes, six new audit events. `app/graph/` still imports no tool-execution / schema-validation / egress code — it represents, it does not enforce.

**§2/§3/§5 re-derived live**: migration **`0056_control_graph` → `0057_dependency_graph`** — **one new table + one additive column on `tools`, no existing column altered/dropped** (`sqlalchemy.inspect()` now **143 tables**, 142 in `Base.metadata`); `alembic downgrade -1`/`upgrade head` re-verified clean; no data backfill. Routes **611 → 630** (+19, all under `/api/v1/graph`). **34 new tests** in `backend/tests/graph/test_dependency_graph.py` (AC-01..AC-18 + the §15 end-to-end proof + the §V scale benchmark, incl. real-Postgres concurrency for dependency-edge + MCP-trust races, the adversarial per-hop tenant-bound proof for dependency traversal, and the MCP-via-Tool / no-second-registry structural assertions). **One pre-existing test updated, intent-preserving** (`tests/graph/test_control_graph.py::test_ac01`'s migration-head guard → a subset check); **no existing test weakened**. Backend **2,453 passed**, 0 failed, 1 deselected (2,419 baseline + 34); frontend **359 passed**, unchanged (all M5 UI deferred to 5.8). See `docs/graph/dependency-graph.md`, `docs/graph/mcp-via-tool.md`, `docs/graph/blast-radius.md`, and [ADR-0018](docs/architecture/adr/0018-mcp-representation-via-tool-domain.md).

---

**Phase 5.5 / M5.5 (Security Posture & Shadow Findings) shipped 2026-09-10** — where the graph evidence becomes **visible risk.** 5.1-5.4 built the canonical asset model, discovery, the authority graph and the dependency/blast-radius graph; 5.5 turns that evidence (plus credentials, governance policies, SLOs) into **deterministic, explainable posture findings** — no accountable owner, stale/expired credential, dormant agent with active credential, unapproved/unknown-provenance MCP dependency, dangerous dependency (blast-radius to a sensitive resource), prohibited model, unapproved tool, excessive tool scope, unknown provenance, missing governance policy / SLO, discovered-outside-lifecycle, governance/control mismatch. ~16 rules, code-defined + versioned.

**Three disciplines.** (1) **No opaque score** — every finding is a structured record (rule/control, evidence rows in 5.1-5.4, affected asset, severity, reason, first/last-seen, remediation, status, governing policy); `GET /posture/summary` is a **deterministic weighted sum** of the open findings (`score = Σ severity_weight × count`), returning the formula, weights, `ruleset_version` and per-rule contributions so a CISO can recompute it and see why it moved. No ML (AST-asserted, the 4.5 proof). (2) **Shadow is a derived finding-state, not a boolean** — no `shadow` column anywhere (AST-asserted); "shadow agents" = agents with an open finding whose `rule_id` is a shadow-class rule (`discovered_outside_lifecycle` / `unmanaged_external_agent` / `unowned_with_production_access` / `production_activity_without_governance`), each condition disputable and self-clearing. (3) **Findings are signals — 5.5 does not enforce** — `app/posture` names no `KillSwitchService`, no `RuntimeGovernanceEngine`, no execution-status mutation (AST); the 4.3 engine + kill switch stay the sole enforcers (5.6 wires threat findings to them).

**Reuses the finding engine proved three times** (3.5 health → 4.5 behavioral → 4.7 SLO/alert): the 4.5 deterministic evaluation shape (`INSUFFICIENT_DATA` first-class — a rule that can't evaluate, e.g. `dangerous_dependency` with no dependency edges, says so; never a false "healthy") and the **4.7 finding lifecycle** (`app.slo.states` imported, not re-spelled — OPEN/ACKNOWLEDGED/RESOLVED/SUPPRESSED, partial-unique dedup on open findings, reopen-on-recurrence, suppression ≠ resolution). A **dedicated `posture_findings` table** rather than a `source='POSTURE'` discriminator on `runtime_alerts` — the same "new table, not a discriminator" call 4.5/4.7 made (ADR-0019).

**Idempotent + 3.8-schedulable** — `PostureEvaluator.evaluate_tenant` is the `posture.evaluate` registered scheduler handler; no new scheduler. Per rule, after it runs, any open finding it did not re-assert is auto-resolved. **Fails open** — a rule that raises is caught (`rule_errors` reported), fabricates nothing, blocks nothing.

**Evidence-gap honesty:** "excessive privilege" (RBAC) and "excessive delegated authority" are **not** delivered as rules — an agent holds no RBAC role assignments and 5.3's delegation edges are human↔human; the privilege-scope concern is covered by `excessive_tool_scope` + `unapproved_tool`, and the gap is recorded (docs/posture/rules.md, ADR-0019) rather than fabricated.

**What changed.** New sibling package `app/posture/` (`rules.py` — the deterministic rule catalog + `PostureContext` evidence bundle; `evaluator.py` — `PostureEvaluator`, idempotent, per-rule reconciliation, fails open; `lifecycle.py` — `PostureFindingService`, the 4.7 create-or-reopen primitive + operator transitions; `settings.py` — `PostureRuleSettingService`, per-tenant enable/disable/param override, versioned + audited; `summary.py` — the deterministic reconstructable score; `shadow.py` — the "shadow agents" query; `schemas.py`/`routes.py`). New models `PostureFinding`, `PostureRuleSetting` (`app/models/posture.py`). Migration `0058_security_posture`: two new tables (`posture_findings`, `posture_rule_settings`), no existing table changed. New API under `/api/v1/posture` (findings list/get, ack/resolve/suppress, per-agent + tenant evaluate, shadow-agents, agent shadow, summary, rules list/update). Two permissions (`posture.view`/`.manage`, group `runtime`). Four error codes, seven audit events. `app/posture/` imports no enforcement code — it represents risk, it does not act on it.

**§2/§3/§5 re-derived live**: migration **`0057_dependency_graph` → `0058_security_posture`** — **two new tables, no existing table changed, no data backfill** (`sqlalchemy.inspect()` now **145 tables**, 144 in `Base.metadata`); `alembic downgrade -1`/`upgrade head` re-verified clean. Routes **630 → 643** (+13, all under `/api/v1/posture`). **31 new tests** in `backend/tests/posture/test_security_posture.py` (AC-01..AC-18 + the §14 end-to-end proof, incl. real-Postgres concurrency for concurrent evaluations + ack races, the per-hop tenant-bound proof for the graph-traversing rule, the AST no-ML / no-enforcement / no-shadow-column proofs, and fail-open). **Two pre-existing tests updated, intent-preserving** — Phase 5.4's own `tests/graph/test_dependency_graph.py::test_ac01` migration-head guard → a subset check (the same moving-target fix 5.3→5.4 applied), and M5.1's `tests/runtime/test_agent_asset_model.py::test_ac12_no_discovery_or_graph_machinery_shipped` (a time-capsule "posture does not exist yet" assertion) → it now expects `app/posture/` + `posture_findings` as a legitimate sibling package while still enforcing the real invariant: no engine inside `app/runtime/`, no parallel `agents` registry, **no `shadow`/`is_shadow` column on any table** (5.5's own "shadow is derived, not a flag" rule). **No existing test weakened.** Backend **2,484 passed**, 0 failed, 1 deselected (2,453 baseline + 31); frontend **359 passed**, unchanged (all M5 UI deferred to 5.8). See `docs/posture/overview.md`, `docs/posture/rules.md`, and [ADR-0019](docs/architecture/adr/0019-posture-findings-and-derived-shadow.md).

---

**Phase 5.10 / M5.10 (Milestone Hardening + Enterprise End-to-End Proof) shipped 2026-09-16 — MILESTONE 5 COMPLETE.** A proof phase: no new capability, no migration, one reported bug fix. The M5 thesis — govern the AI you didn't build — demonstrated across a **real process boundary**, and all eighteen §44 gates (A–R) closed by a named proof.

**The E2E crosses the OS process boundary twice.** Inbound: ACT discovers an agent it did not build through a real `http.server` registry on a real socket, no DB lock held across the fetch. Outbound: that agent calls ACT's gateway from a **separate OS process** over a **real uvicorn socket**, its AST asserted to import nothing from `app`. Fourteen real-cause/real-effect steps through real authorities; **the kill is asserted REFUSED** (ACT does not run the agent and 5.6 refuses to fake it), and the containment ACT genuinely holds — grant revocation — is proven to take effect by re-running the external process. A draft asserting a successful suspend would have passed only if the platform lied.

**Scale, measured:** `agents-reaching` **98.9 ms / 119.9 ms** over 4,400 edges, one busy tenant, 4,000 real agent rows — relational CTE, no projection, no graph DB (ADR-0017), asserted against `requirements.txt`. **One real defect fixed** (`app/discovery/service.py`): the loser of two concurrent sweeps hit `StaleDataError` — the optimistic lock working, hence no duplicate — but `run_source` did not catch it as the model contract (`app/models/agent.py`) requires, so it escaped as a 500 with the run dangling at STARTED. Now finished as a truthful FAILED run naming `AGENT_CONCURRENT_MODIFICATION`, per this service's own fail-open convention; proven deterministically (`test_ac13`), full 5.2 suite unchanged.

**What changed.** New `backend/tests/milestone/` (17 proofs, `test_milestone_5_proof.py` + conftest); `app/discovery/service.py` (the fix); `docs/milestone-5/{proof,summary}.md`. **§2/§3/§5 re-derived live:** head **unchanged at `0061_assurance_evidence`** (**152 tables**, 151 in `Base.metadata`); routes **unchanged at 676**; permissions unchanged at 162. Backend **2,639 passed**, 1 failed, 1 deselected — **2,640 collected** (2,623 + 17). The one failure is the known pre-existing posture-seed flake (`test_ac09_existing_agents_are_backfilled_native_and_governed`), which failed this phase's clean-tree baseline, the final run, **and a standalone re-run afterwards** — so it is reported as a live flake, not as "passes standalone". Query-verified root cause: 16 `agents` rows (of ~100k) are `origin_category='NATIVE'` yet `control_state='DISCOVERED'`, every one named `agent-<hex8>` by `tests/posture/test_security_posture.py::_insert_agent`, whose line-282 call passes `control_state="DISCOVERED"` on the helper's default `origin_category="NATIVE"` — an internally inconsistent fixture (a NATIVE agent cannot be DISCOVERED). Two of the 16 were created at 16:48 and 17:31, exactly this phase's two full runs; **none by 5.10** (its inserts take invariant-satisfying defaults, and its hostile-source proof asserts the created row is *not* NATIVE). 5.1's `test_ac09` samples `limit(500)` with no `ORDER BY`, so each full run raises the hit probability (~8% per attempt now). Left untouched per verify-and-leave; the recommended fix belongs to 5.5's fixture (pass `origin_category="EXTERNAL"` at line 282), not to 5.1's guard, run **isolated** — no mid-run tree/REPO_STATE edits (the 5.9 lesson). Frontend **384 passed**, `npm run build` green. **Next: the post-M5 Enterprise Validation Lab & Red-Team Gate; no new milestone begun.** See `docs/milestone-5/proof.md`, `docs/milestone-5/summary.md`.

**Phase 5.9 / M5.9 (Assurance, Evidence & Compliance) shipped 2026-09-16** — the evidence M1–5.8 already produce, turned into control evaluations an auditor can use. It produces evidence and control mappings, and is built so that it cannot produce a compliance verdict.

**Phase 0 first — the frontend build was restored.** `tsc -b` / `npm run build` had been failing on `main` since Phase 4.9: `TraceContentResponse` is exported from `services/observabilityService.ts` but was never re-exported by the `@/services` barrel that `TraceDetailPage` imports it from. `vitest` transpiles without typechecking, so a green 383-test suite masked a broken production build across four phases. Fixed with one barrel re-export (the two `TS7006` errors were downstream of the same unresolved type). `frontend/src/test/build-integrity.test.ts` now runs the real `tsc -b` inside the suite that was masking it, and was verified to **fail** when the fix is reverted. The repo has no CI workflows, so a pipeline-only check would not have existed; adding `npm run build` to CI when it exists is the documented follow-up.

**Three results, and the line between the last two is the phase.** PASS / FAIL / INSUFFICIENT_EVIDENCE. FAIL = evidence exists **and shows the control unmet**; INSUFFICIENT_EVIDENCE = ACT cannot tell either way. Both directions of confusion are failures — calling absence a pass is the false-green; calling a conclusive negative "insufficient" hides a real finding. So an agent with no owner **FAILs** (the row is the evidence, conclusive), while an agent with no executions is **INSUFFICIENT_EVIDENCE** for traceability (ACT cannot see what it does not run; FAIL would invent a violation).

**"Has anyone actually looked?" is asked first.** Posture-backed controls check the `POSTURE_EVALUATED` **audit event** before reading `posture_findings` — zero findings is indistinguishable from "posture never ran". Verified in both directions on live data: a fresh tenant reports 9 of 12 controls insufficient, and a real posture evaluation flips them to genuine PASS/FAIL each carrying an `as_of`.

**A stale pass is unrepresentable** — `ck_assurance_eval_stale_never_passes`, enforced by the database rather than only by the evaluator. **No verdict, enforced by absence** — no `compliant`/`status`/`score`/`coverage`/`verdict` column, schema field or response key anywhere; a percentage would need a denominator ACT does not know and would compress INSUFFICIENT_EVIDENCE into the same number as PASS. **Mappings are relevance claims in versioned code** (12 across NIST AI RMF 1.0 / ISO 42001 / SOC 2), partial on purpose — uncovered controls absent rather than stubbed. **Exceptions document but never flip a result**, and survive re-evaluation. **Bundles reuse M4.11's DSSE signing** (verified live, `keyid default:3`) and export unsigned-and-labelled rather than not at all — the deliberate asymmetry with M4.11's fail-closed rule for agent versions, which are executed rather than merely read.

**What changed.** New sibling package `app/assurance/` (`controls.py` — 12 deterministic controls + `AssuranceContext`; `frameworks.py` — the versioned mappings; `service.py` — evaluation, exceptions, export + DSSE signing; `schemas.py`/`routes.py`). New models `AssuranceEvaluation`, `AssuranceEvidenceBundle` (`app/models/assurance.py`). New frontend `services/assuranceService.ts`, and 5.8's Assurance placeholder replaced by the real view. `assurance.evaluate` registered scheduler handler — no new scheduler.

**§2/§3/§5 re-derived live**: migration **`0060_external_gov_bridge` → `0061_assurance_evidence`** — two new tables, no existing table changed, no data backfill (`sqlalchemy.inspect()` now **152 tables**, 151 in `Base.metadata`); `alembic downgrade -1`/`upgrade head` re-verified clean. Routes **668 → 676** (+8 under `/api/v1/assurance`). Permission catalog **159 → 162** (`assurance.view`/`.manage`/`.export`, group `audit`; export never implied by view or manage). **36 new backend tests** (`backend/tests/assurance/test_assurance.py` — AC-00..AC-17 + the §14 end-to-end proof) + **1 new frontend test** (the build guard); one 5.8 frontend test updated intent-preservingly where 5.9 replaced the placeholder it asserted. Backend **2,623 passed**, 0 failed, 1 deselected (2,587 + 36); frontend **384 passed** with `npm run build` green.

**One process note.** This phase's baseline run failed `test_ac01_repo_state_is_regenerated_at_the_true_post_3_10_head` — caused by copying the 5.9 migration into the tree *while the suite was running*, exactly the trap that guard exists to catch. Verified by stashing and re-running on a clean tree (passed); the true baseline was 2,587/0/1. See `docs/assurance/overview.md` and [ADR-0022](docs/architecture/adr/0022-assurance-evidence-not-verdict.md).

**Phase 5.8 / M5.8 (Enterprise Agent Command Center) shipped 2026-09-16** — the first M5 phase to touch the frontend, carrying all the UI deferred since 5.1: the operator surface over everything 5.1–5.7 built. It reads and triggers; it decides nothing and enforces nothing.

**Truthful affordances, made structural.** The UI derives nothing about ACT's reach. `frontend/src/modules/command/affordances.ts` contains **no enforcement-mode name, no control-state name and no mode→label map** (asserted over its own source) and reads three fields the *server* computed with 5.7's `app.bridge.modes`: `reaches_agent_execution`, `reaches_boundary_calls`, and the `display`/`limits` sentences rendered **verbatim**. A second structural test scans every file in the module and fails on any `control_state ===` / `enforcement_mode ===` comparison. Proven per state: OBSERVED/DISCOVERED shows **no** containment affordance (and says why, in the server's own sentence); GATEWAY_ENFORCED is labelled **"authorize boundary calls"**, never "governed"; NATIVE shows full containment, `ConfirmActionDialog`-gated, dispatching to 5.6's real endpoint.

**"Hidden" is never "zero".** Each estate section is probed against its own domain permission through the real `AuthorizationGateway`; a section the caller cannot read returns `visible: false` and renders as "hidden from you — not a count of zero". Shadow shows **why** (5.5's conditions, never a bare badge); unmeasurable cost is counted and named, never summed (5.7's `NOT_MEASURABLE`); `INSUFFICIENT_DATA` and `REFUSED` show as themselves; a failed read shows the failure rather than an empty state.

**Reuses 3.10/4.9 verbatim** — `ConfirmActionDialog` + `useGuardedAction` imported from `@/modules/operations`, the persona vocabulary from `@/modules/observability` (no second persona list, so "Security" means one thing platform-wide). Structurally asserted: no second dialog or hook in the module.

**What changed.** New frontend module `frontend/src/modules/command/` (13 views + the agent drilldown, `affordances.ts`, `personas.ts`, `components.tsx`, `index.ts`) and `frontend/src/services/commandCenterService.ts`. New backend sibling package `app/command_center/` (`service.py` — `CommandCenterService`, counts + inventory; `routes.py` — two `GET`s). Routes constants, `AppRoutes`, sidebar navigation and `services/index.ts` extended. **No migration, no new table, no new column, no new permission, no new authz, no new engine, no new design system.**

**§2/§3/§5 re-derived live**: migration head **unchanged at `0060_external_gov_bridge`** (**150 tables**, 149 in `Base.metadata`). Routes **666 → 668** (+2, both `GET` under `/api/v1/command-center`). The two endpoints exist only because an estate count over tens of thousands of agents, and a per-row enforcement mode, cannot be assembled from a 500-row paginated list without counting in the browser — the same narrow addition 4.9 made. Both are GET-only (asserted over the AST *and* the route table), tenant-scoped, and compute no domain state: posture score from 5.5's `PostureSummaryService`, shadow from 5.5's `ShadowAgentService`, mode and reach from 5.7's `app.bridge.modes` **including its `NATIVE_CONTROL_STATE` constant rather than a re-typed `"GOVERNED"` literal** — a drift a test caught mid-build and which was fixed by importing the constant. The agent drilldown needed **no** endpoint (react-query composition is orchestration, not logic). **13 new backend tests** (`backend/tests/command_center/test_command_center.py`) + **24 new frontend tests** (`frontend/src/modules/command/tests/command.test.tsx`) — the first frontend tests any M5 phase has added. Backend **2,587 passed**, 0 failed, 1 deselected (2,574 + 13); frontend **383 passed** (359 + 24).

**One 5.7 defect fixed, not left.** `tests/bridge/test_external_governance_bridge.py::test_ac14_concurrent_calls_under_a_revocation_race` failed this phase's baseline on a clean tree — cause was mine from 5.7: `with TestClient(...)` in three threads runs the app lifespan per thread, and the concurrent M4.11 key-material-canary inserts race its unique constraint (the platform-singleton-row trap). The context manager was never needed; the suite's own `client` fixture is bare. Harness corrected, assertions untouched, stable over three runs. **Pre-existing and left:** `tsc -b` fails on 4.9's `TraceDetailPage.tsx` (3 errors — `TraceContentResponse` not exported), confirmed on clean `main` before any 5.8 code; `vitest` does not typecheck, so the 359-suite was always green. Reported, not fixed — it belongs to 4.9. See `docs/command-center/overview.md`.

**Phase 5.7 / M5.7 (External Agent Governance Bridge) shipped 2026-09-15** — the highest-risk phase in Milestone 5: it is where ACT governs agents it does **not** run. 5.6 could *observe* that an external agent's containment was truthfully absent; 5.7 makes the four enforcement modes real, and bounds each one to exactly what it can deliver.

**Four modes, each carrying its own limit.** `OBSERVED` sees (no enforcement). `ADVISORY` evaluates the real 4.3 policies and recommends (no enforcement). `GATEWAY_ENFORCED` authorizes/denies the capability calls an external agent routes **through ACT** — and reaches nothing else it does. `NATIVE_ENFORCED` is the M1–M4 platform (4.3 engine + kill switch). Every mode ships a `display` **and** a `limits` string and both travel in every API response, so a client cannot render ACT's claim without its bound. The permitted sentence for GATEWAY is "ACT authorizes this agent's capability calls that route through ACT's gateway", never "ACT governs this agent" — asserted per mode.

**`NATIVE_ENFORCED` is not storable.** The new nullable `agents.external_enforcement_mode` CHECK admits only OBSERVED/ADVISORY/GATEWAY_ENFORCED; the strongest mode is **derived** from `control_state == 'GOVERNED'` — the same single signal Phase 5.6's containment gate reads. Full enforcement has to be *true*, not declared, the two phases cannot drift apart, and **no backfill is needed** (native rows are already GOVERNED → NATIVE_ENFORCED; everything else → OBSERVED, the weakest truthful claim).

**The design decision that mattered most: a GATEWAY_ENFORCED agent is NOT `GOVERNED`.** M5.1's own `control_state` comment anticipated external agents reaching GOVERNED "at NATIVE/GATEWAY enforcement, which is Phase 5.7". Building 5.7 showed that would have been a **false claim** — it would have made 5.6's `SUSPEND_AGENT` report success for an agent ACT cannot suspend. So the agent stays `REGISTERED`, **5.6 is untouched and still truthfully refuses it**, **no eighth containment action was added**, and the superseded anticipations in ADR-0015 / ADR-0020 / `docs/threat/truthful-containment.md` were corrected rather than left to mislead (ADR-0021).

**External identity is never internal.** Federation was **rejected**: `FederationService._resolve_or_provision_user` → `_provision_user` → `_issue_session` mints a `User` with roles and a session, exactly the escalation this phase forbids. A bearer `AgentApiKey` was rejected too — a bearer token cannot be replay-protected. Identity is a **signed request**: HMAC-SHA256 over method/path/timestamp/nonce/**digest of the exact body bytes**, secret stored only as Fernet ciphertext (M4.11 `credential_crypto`) and shown once; replay defeated by the `(grant_id, nonce)` unique constraint, and the nonce is consumed **after** the HMAC verifies so a garbage signature cannot burn a caller's nonces.

**The boundary is a boundary, not a second authz** — it calls the real `AuthorizationGateway.authorize_agent`, resolves the real 4.3 `runtime_governance_policies` and prices against the real 4.4 budgets, holding no permission/policy/budget logic of its own (AST-asserted). Proven behaviourally, not just structurally: a real published ABAC **DENY** stops a call; a real agent-scoped **`requires_approval`** policy denies with `APPROVAL_REQUIRED`; a real **`HARD_LIMIT`** budget with no headroom denies with `EXCEEDED`. No reservation is taken (`ReservationService.reserve` needs an `AgentExecution` a boundary call has none of, and fabricating one would corrupt the 4.4 ledger) — ACT enforces the ceiling it can read and records `NOT_MEASURABLE` otherwise, rather than inventing a number.

**Commit-before-dispatch, proven for real** — unlike 5.6 (whose authorities did no I/O), the reference capability makes a genuine outbound call through M1's egress guard. No statement in `app/bridge` takes `FOR UPDATE` and `dispatch.py` imports no session, both asserted **over the AST** (deliberately: these modules describe the rule in their docstrings, and a substring scan would have matched that prose and passed for the wrong reason); and while a call is in flight a **second real Postgres connection reads *and writes*** the already-committed gateway row.

**Do-not-proxy-everything** — one governed capability ships (`http_tool.invoke`), no catch-all route, no pass-through, no HTTP client importable from the decision path. A broad catalog and an external-agent SDK are deferred (§25A). **Fail semantics by §9 plane**: a governance decision fails **closed** (`UNEVALUABLE` → DENY, recorded); telemetry ingest fails **open**; a downstream outage is recorded truthfully as *allowed + dispatch failed*, never faked and never rewritten into a denial; a DB CHECK makes "denied but dispatched" unrepresentable.

**What changed.** New sibling package `app/bridge/` (`modes.py` — the four modes, their reach table and the one `effective_mode` derivation; `identity.py` — signed requests, scoped grants, nonce consumption, `ExternalGrantService`; `capabilities.py` — the declared capability registry + per-capability plane/fail-mode; `gateway.py` — `CapabilityBoundary`, the decide/record/commit path; `dispatch.py` — the post-commit downstream call, session-free by construction; `observed.py` — OBSERVED ingest + ADVISORY recommendation, importing no enforcement authority at all; `service.py` — `EnforcementModeService`; `schemas.py`/`routes.py`). New models `ExternalCapabilityGrant`, `ExternalRequestNonce`, `ExternalGatewayCall` (`app/models/bridge.py`). Migration `0060_external_gov_bridge`: three new tables + one nullable `agents` column, no existing table changed. New API under `/api/v1/bridge` (mode read/set, mode + capability catalogs, grant issue/list/revoke, advisory, gateway-call list, and the two boundary endpoints `/capability` and `/events`). Three permissions (`external_governance.view`/`.manage`, `external_grant.issue`, group `runtime`). 17 error codes, 6 audit events. No new authz, no new scheduler, no second agent registry.

**§2/§3/§5 re-derived live**: migration **`0059_threat_containment` → `0060_external_gov_bridge`** — **three new tables + one additive nullable `agents` column, no existing table changed, no data backfill** (`sqlalchemy.inspect()` now **150 tables**, 149 in `Base.metadata`); `alembic downgrade -1`/`upgrade head` re-verified clean. Routes **655 → 666** (+11, all under `/api/v1/bridge`). Permission catalog **156 → 159**. **53 new tests** in `backend/tests/bridge/test_external_governance_bridge.py` (AC-01..AC-20 + the §15 end-to-end proof, which runs ACT under a **real uvicorn server on a real socket** driven by a **real external agent in a separate OS process** whose own AST is asserted to import nothing from ACT; plus real-Postgres concurrency for grant/revocation races and the duplicate-nonce constraint, the per-mode truthful-reach assertions, and the AST no-second-authz / no-lock / no-internal-identity / no-proxy-everything proofs). **No pre-existing test changed.** One pre-existing test (`tests/runtime/test_agent_asset_model.py::test_ac09_existing_agents_are_backfilled_native_and_governed`) failed in the full-suite run and passed standalone/unmodified — **confirmed failing on a clean tree before any 5.7 code existed**. Root cause diagnosed and left as-is per the verify-and-leave discipline: `tests/posture/test_security_posture.py`'s `_insert_agent(..., control_state="DISCOVERED")` relies on that helper's default `origin_category="NATIVE"`, creating NATIVE+DISCOVERED rows that violate 5.1's invariant, and the assertion samples `limit(500)` of ~76k rows with **no `ORDER BY`**, so it fails only when one of the 7 such rows lands in the sample (one more accumulates per full-suite run). A 5.5/5.1 test interaction on the shared dev DB, unrelated to this phase; in the final run it passed, consistent with its sampling-dependent nature. Backend **2,574 passed**, 0 failed, 1 deselected (2,521 baseline + 53); frontend **359 passed**, unchanged (all M5 UI deferred to 5.8). See `docs/bridge/overview.md`, `docs/bridge/enforcement-modes.md`, `docs/bridge/gateway.md`, `docs/bridge/external-identity.md`, and [ADR-0021](docs/architecture/adr/0021-truthful-external-enforcement-modes.md).

**Phase 5.6 / M5.6 (Runtime Threat Detection & Containment) shipped 2026-09-14** — the highest-risk phase in Milestone 5: it touches the enforcement path. 5.5 made risk *visible* (posture findings, signals only); 5.6 gives the milestone its **teeth** — deterministic runtime **threat detection** over M4 signals (4.5 behavioral findings, 4.3 governance decisions, `tool_calls`) + 5.4/5.5 evidence, and **containment** that routes **only** to enforcement authorities the platform already has.

**The defining honesty requirement: TRUTHFUL containment.** `ContainmentOrchestrator.truthful_capability` is the one gate — capability derives from exactly one signal, `agents.control_state`. `GOVERNED` (ACT actually executes/enforces this agent) → every action routes to its real authority. Anything else (`DISCOVERED`/`CLAIMED`/`REGISTERED`) → every enforcement-requiring action is **truthfully refused** (`status='REFUSED'`, a real reason, `authority_ref`/`result` both empty) — never a fake success. Proven with real rows: a `GOVERNED` agent is genuinely suspended (`lifecycle_status` actually flips, executions actually cancel); a `DISCOVERED` agent gets a threat finding but a truthfully-refused containment.

**Containment routes to exactly seven actions, each naming its one existing authority — no eighth action, no second enforcer.** `TERMINATE_EXECUTION`/`SUSPEND_AGENT` → `KillSwitchService`; `DENY_TOOL` → `ToolRegistryService.revoke`; `REVOKE_CAPABILITY` → `CapabilityService.revoke`; `ISOLATE_CREDENTIAL` → `api_key_service.revoke_key`; `DISABLE_INTEGRATION` → `ConnectorService.disable`; `REQUIRE_APPROVAL` → `GovernancePolicyService.create` (a real, mandatory, agent-scoped policy the 4.3 engine's own checkpoints read). `app/threat` implements no enforcement of its own (AST-asserted: no parallel-enforcer class, no direct `UPDATE` of an enforcement column). **One genuine bug caught during build**: the planned `DISABLE_INTEGRATION` revert called `ConnectorService.activate()` directly, but the real lifecycle has no `disabled → active` edge (only `disabled → configured → active`) — fixed to call `update_configuration` then `activate`, matching the actual state machine (ADR-0020).

**Kill-switch dominance is structural.** `SUSPEND_AGENT`/`TERMINATE_EXECUTION` are `reversible=False` by construction; nothing in `app/threat` sets `lifecycle_status` back to `'ACTIVE'` or `cancel_requested` back to `False` (AST-asserted, the same proof `app.runtime.governance` gives for itself). Automated detection (`ThreatEvaluator`) never invokes a containment authority — a new HIGH/CRITICAL finding only ever creates a `RECOMMENDED` action row (bounded automation, proven by a test that evaluates a GOVERNED agent with an anomaly and asserts it stays ACTIVE). Proven under a real concurrent race: a human-confirmed suspend followed by a subsequent automated re-evaluation leaves the agent still `SUSPENDED`.

**A dedicated `threat_findings` table** — the same "new table, not a discriminator" call 4.5/4.7/5.5 each made (ADR-0020, extending ADR-0019): a threat is a *runtime event*, posture is a *standing state*. The canonical example, delivered as `unapproved_mcp_tool_invoked`: an agent depending on an unapproved MCP server is posture (5.5); the agent actually *invoking* a tool that server exposes is a threat. The lifecycle *shape* is reused (`app.slo.states` imported, not re-spelled). **`containment_actions` records; it does not enforce** — every `EXECUTED` row's `authority_ref` names the real row a real authority produced.

**Commit-before-dispatch holds by construction** — none of the six authorities perform network/model I/O (every one is a plain ORM write + commit), so there is no lock to hold across a dispatch; the AST proof (no `with_for_update`/`FOR UPDATE`) plus a real-second-connection `FOR UPDATE NOWAIT` behavioral proof both pass. **`containment.execute` is a distinct, stronger permission**, never implied by `threat.view`/`.manage`.

**Evidence-gap honesty:** "prompt/indirect-injection" and "cross-agent/delegation abuse" threat rules are **not** delivered — no reason-code taxonomy exists for content-based denials, and 5.3's delegation edges carry no timestamped "used after revocation" evidence. Recorded (ADR-0020, docs/threat/rules.md) rather than fabricated.

**What changed.** New sibling package `app/threat/` (`rules.py` — 6 deterministic threat rules + `ThreatContext`; `lifecycle.py` — `ThreatFindingService`, the 4.7 lifecycle; `evaluator.py` — `ThreatEvaluator`, idempotent, fails open, recommend-only; `containment.py` — `ContainmentOrchestrator`, the action→authority router + truthful capability + revert; `schemas.py`/`routes.py`). New models `ThreatFinding`, `ContainmentAction` (`app/models/threat.py`). Migration `0059_threat_containment`: two new tables, no existing table changed. New API under `/api/v1/threat` (findings list/get/ack/resolve/suppress, per-agent + tenant evaluate, containment list/get/execute/revert). Three permissions (`threat.view`/`.manage`, `containment.execute`, group `runtime`). Six error codes, twelve audit events. `threat.evaluate` registered scheduler handler (mirrors `posture.evaluate`/`discovery.sweep`) — no new scheduler.

**§2/§3/§5 re-derived live**: migration **`0058_security_posture` → `0059_threat_containment`** — **two new tables, no existing table changed, no data backfill** (`sqlalchemy.inspect()` now **147 tables**, 146 in `Base.metadata`); `alembic downgrade -1`/`upgrade head` re-verified clean. Routes **643 → 655** (+12, all under `/api/v1/threat`). **37 new tests** in `backend/tests/threat/test_runtime_threat_containment.py` (AC-01..AC-18 + the §14 end-to-end proof, incl. real-Postgres concurrency for containment races + threat dedup, the truthful-containment proof across all three non-GOVERNED control states, the kill-switch-dominance-survives-automation proof, and the AST one-enforcement-path / no-row-lock proofs). **No pre-existing test changed.** One pre-existing test (`tests/runtime/test_execution_tracing.py::test_ac05_each_filter_dimension_narrows[agent_id]`) failed in the full-suite run and passed standalone/unmodified — a pre-existing order-dependent flake, unrelated to this phase (touches no file this phase changed), left as-is and reported per the verify-and-leave discipline. Backend **2,521 passed**, 0 failed, 1 deselected (2,484 baseline + 37); frontend **359 passed**, unchanged (all M5 UI deferred to 5.8). See `docs/threat/overview.md`, `docs/threat/rules.md`, `docs/threat/truthful-containment.md`, and [ADR-0020](docs/architecture/adr/0020-truthful-containment-via-existing-authorities.md).


**Post-M5 Validation Gate V0 (Baseline Integrity, Fixture Cleanup & Reproducibility) completed 2026-09-17 on branch `validation/v0-baseline`.** A verification phase: no product code, no migration, no schema change, no new capability. Evidence bundle: `docs/validation/v0/`. **§2/§3/§5 re-derived live and unchanged:** head `0061_assurance_evidence` (152 tables live, 151 in `Base.metadata`), routes 676, collection 2,641 (1 deselected). **The M5 close record above is preserved as history and was not altered.**

**Pre-fix baseline (frozen tree, frozen DB, `main` at `9667707`):** **2,640 passed, 0 failed, 1 deselected, 26:52.** The known posture-seed failure did not occur in that run; it was then characterized by repetition on the same frozen state — the 5.1 guard `test_ac09_existing_agents_are_backfilled_native_and_governed` alone, 50 consecutive invocations: **3 failures / 50 (6%)**, one exact traceback preserved (`assert 'DISCOVERED' == 'GOVERNED'`, line 429). The baseline itself created the 17th invalid row (`agent-452ea2da`, 00:31:20, inside the run window) — the defect reproducing under observation.

**Diagnosis independently verified, then corrected.** All 17 `NATIVE`+`DISCOVERED` rows were mechanically classified `TEST RESIDUE (fixture-attributable)`: every one in a single-agent "Posture Org", named `agent-<first 8 hex of its own id>`, `api_key_hash='x'`, unowned, no source ref, created minutes before a phase commit from 5.5 onward, with zero audit or discovery linkage; zero rows failed the fixture signature. **§9B: no production path can create the combination** — the only writer of `DISCOVERED` (`AgentProvenanceService.record_external_agent`) rejects `NATIVE`, no transition targets `DISCOVERED`, the fields are absent from every write schema, and no raw SQL writes `agents`. Fix: `tests/posture/test_security_posture.py:282` now passes `origin_category="UNKNOWN", origin_provider="UNKNOWN"` — the *truthful* value (the test establishes no provenance; `EXTERNAL` would assert evidence it does not have), mirroring the same file's line 397. Residue: 17 rows deleted under a full-signature guard; 0 remain. Production invariants, the 5.1 guard, enums and transitions untouched.

**Post-fix runs (frozen tree):** targeted (posture, asset model, discovery, control graph) **133 passed**; full backend **2,639 passed, 1 failed, 1 deselected, 24:42** — the failure is *not* the posture flake but a second, previously unrecorded probabilistic test-infrastructure defect: `tests/authorization/test_runtime.py::test_idempotency_is_scoped_per_agent_not_shared` names its two same-org agents `Agent <6 hex>` with identical description/purpose, and 5.1's duplicate detector (`difflib` weighted score ≥ 0.85 → `AGENT_DUPLICATE_REVIEW_REQUIRED`) blocks the second registration whenever the names score ≥ 0.70 — **5.4% per full run** by Monte-Carlo with the product's own formula; the failing pair scored exactly 0.875. The only exposed test in the suite. **Not re-rolled, not fixed** (V0 authorizes one fixture correction; a proposal is recorded). Re-run 10× standalone on both the modified and the stashed tree: 10/10 passed. **Repeatability (§16): posture suite + 5.1 guard, 50 consecutive runs, 0 failures** (`docs/validation/v0/repeat50.log`). Frontend **384 passed** (52 files), `tsc -b` clean, build green; the 5.9 build guard proven to bite (reverting the fix fails it with TS2305). **Verdict: V0 CONDITIONAL — REVIEW REQUIRED** (W-7 flake proposal, W-2 native-agent demotion finding, §J guard proposal, §T CI proposal). **V0.1 closure (2026-09-17, same branch):** W-7 fixed as test data only (the two same-org agents now carry fixed, unrelated name/description/purpose scoring a constant 0.2548 against 5.1's live duplicate model; 30/30 consecutive runs; a negative control proves the test still fails when per-agent scoping is broken). The 5.1 guard `test_ac09` now asserts a full-population violating-rows count of zero (one sequential scan, ~71 ms at 116k rows; proven to bite on a planted row). **W-2 NOT implemented — §2A stop:** the derived origin × control-state matrix is complete for NATIVE (GOVERNED only) but the EXTERNAL/UNKNOWN × GOVERNED cell cannot be derived from live semantics (GOVERNED means "ACT runs it", yet `test_m51_end_to_end_proof` grants it to a CrewAI agent by request); three options recorded in `docs/validation/v0/closure-v0.1.md`. Targeted regression 268 + 112 passed; **V0.1 final backend run: 2,640 passed / 0 failed / 1 deselected** (2,641 collected, 0:24:38); frontend 384, `tsc -b` clean, build green. **Verdict: V0 CONDITIONAL — REVIEW REQUIRED (W-2 decision).** **V0.2 closure (2026-09-17, same branch) — W-2 resolved under the reviewer's Option 1 ruling (ADR-0023):** `GOVERNED` ⇔ `NATIVE`, enforced in `AgentControlStateService` before the matrix at every writer (`LEGAL_CONTROL_STATES_BY_ORIGIN`), stable error `CONTROL_STATE_ORIGIN_INCOMPATIBLE` (409), refusals audited as `RUNTIME_AGENT_CONTROL_STATE_REJECTED` (committed before raising), both `GOVERNED` edges removed from `CONTROL_TRANSITIONS`; mass assignment proven closed in both directions; the M5.1 guard now asserts the full matrix over the whole table. `test_m51_end_to_end_proof` step 6 corrected (asserts REGISTERED **and** the refusal). 103 non-native GOVERNED rows (33 from that proof, 70 from posture:766) classified fixture residue and deleted; both fixtures corrected. No migration (DB CHECK recommended, §J). Targeted **348 passed, 2000 warnings in 446.20s (0:07:26)** + tenant **113 passed, 2536 deselected, 1271 warnings in 316.38s (0:05:16)**; **V0 final clean baseline: 2,648 passed / 0 failed / 1 deselected** (2,649 collected, 0:26:41); frontend 384 passed, `tsc -b` clean, build green. **Verdict: V0 PASSED — READY FOR V1 AUTHORIZATION.** See `docs/validation/v0/closure-v0.2.md`. **Next: V1 only after review; no red-team, no external agents, no cloud, no M6.**
...
> **Stale, and flagged rather than fully regenerated (Phase 4.6/4.7/4.8):** this tree predates Milestone 4's later sub-phases and does not show the sibling packages `app/behavior/` (4.5), `app/finops/` (4.4), `app/observability/` (4.1), `app/telemetry_export/` (4.6), `app/slo/` (4.7) or `app/telemetry_privacy/` (4.8). Each is a top-level package under `app/`, described in §4 and in the per-phase narrative above; the authoritative module inventory is §4, not this tree. Regenerating the tree in full is deferred to the next full REPO_STATE pass.
...
**152 tables** (live `sqlalchemy.inspect()`; 151 in `Base.metadata` — Alembic manages `alembic_version` itself and it is not a model), extracted against the local PostgreSQL **17.10** database after running every migration through **`0061_assurance_evidence`** (head). This reflects exactly what the migration chain produces. *(Re-derived live 2026-09-16 for Phase 5.9 / M5.9: +2 tables — `assurance_evaluations` (one control's result against real evidence: PASS / FAIL / INSUFFICIENT_EVIDENCE, the evidence it read or the absence it found, the as-of that evidence carries, whether that made it stale, and any documented exception; note `ck_assurance_eval_stale_never_passes` — the database refuses to store a stale PASS, because a false-green must not be representable) and `assurance_evidence_bundles` (a record that evidence left the system: scope, catalog/mapping versions, a content digest and the DSSE signature — the payload is deliberately NOT stored, since it is reconstructable from the evaluations it names and a second copy could drift from them). **No control-definition or framework-mapping table**: both live in versioned code, the 5.5 rule-catalog precedent. No existing table changed, no data backfill. Phase 5.8 / M5.8 added no table at all. Re-derived live 2026-09-15 for Phase 5.7 / M5.7: +3 tables — `external_capability_grants` (a scoped, expiring, revocable, rate-limited signing credential for an agent outside ACT; the secret is stored ONLY as Fernet ciphertext via M4.11 `credential_crypto` and returned once at issue — it is NOT an internal principal: no `users` row, no role, no session), `external_request_nonces` (the replay ledger; the `(grant_id, nonce)` unique constraint IS the anti-replay primitive) and `external_gateway_calls` (one row per boundary decision — capability, the real AuthorizationGateway verdict, the 4.3 policy outcome, the 4.4 cost outcome, allow/deny, fail-mode, dispatch status; `ck_ext_calls_denied_never_dispatched` makes "denied but dispatched" unrepresentable). Plus **one nullable column on `agents`** (`external_enforcement_mode`) whose CHECK admits only OBSERVED/ADVISORY/GATEWAY_ENFORCED — **`NATIVE_ENFORCED` is deliberately not storable**, it is derived from `control_state == 'GOVERNED'`, so no row can claim enforcement ACT lacks and **no backfill is needed**. No existing table changed. Re-derived live 2026-09-14 for Phase 5.6 / M5.6: +2 tables — `threat_findings` (a runtime-event finding, the 4.7 lifecycle shape, distinct from 5.5's standing-state `posture_findings`) and `containment_actions` (a record of one containment action -- authority invoked or truthfully refused, trigger, control_state at decision time, result, reversibility; references the real authority's own row via `authority_ref`, never reimplements it); no existing table changed, no data backfill. Re-derived live 2026-09-10 for Phase 5.5 / M5.5: +2 tables — `posture_findings` (a standing, explainable posture finding with the 4.7 lifecycle: rule/control id, evidence refs to 5.1-5.4 rows, severity, reason, remediation, `outcome` FINDING/INSUFFICIENT_DATA, `uq_posture_findings_active` partial unique index over open findings) and `posture_rule_settings` (per-tenant enable/disable + param override, monotonic `revision`); no existing table changed, no data backfill. Re-derived live 2026-09-09 for Phase 5.4 / M5.4: +1 table — `mcp_servers` (an MCP server's identity, provenance, trust/approval state, version, endpoint ref — holds NO tools; the tools it exposes are ordinary `tools` rows linked by the one additive nullable column `tools.mcp_server_id`); the `control_graph_edges` `source_type`/`target_type`/`edge_type` CHECK constraints widened in place (drop + recreate) for the `MCP_SERVER`/`CONNECTOR` node types and the 9 dependency `edge_type` values; no existing column altered/dropped, no data backfill. Re-derived live 2026-09-08 for Phase 5.3 / M5.3: +1 table — `control_graph_edges` (typed, directed, tenant-scoped control-graph edges; soft-revoked, optionally time-bounded; `uq_control_graph_edges_active` partial unique index + bidirectional `(org,source)`/`(org,target)` indexes); no existing table changed, no data backfill. Re-derived live 2026-09-05 for Phase 5.2 / M5.2: +4 tables — `discovery_sources`, `discovery_runs`, `discovery_observations` (append-only — `UPDATE`/`DELETE` revoked from `PUBLIC`), `discovery_findings`; no existing table changed, no data backfill. Re-derived live 2026-09-04 for Phase 5.1 / M5.1: **no table change** — `0054_agent_asset_model` adds **seven columns to `agents`** (`control_state`, `origin_category`, `origin_provider`, `first_observed_at`, `last_observed_at`, `discovery_source_ref`, `discovery_confidence`) + `ck_agents_control_state` / `ck_agents_origin_category` + `ix_agents_control_state`, with an idempotent backfill setting every pre-existing row to `GOVERNED` / `NATIVE` / `ACT_NATIVE`; `control_state` is a distinct dimension from `lifecycle_status`, which is unchanged. Re-derived live 2026-09-03 for Phase M4.11a: +1 table — `installation_bootstrap` (the durable bootstrap marker; singleton). Re-derived live 2026-09-02 for Phase M4.11: +1 table — `key_material_canary` (the key-validation canary verifier store). Re-derived live 2026-09-01 for Phase 4.8: +3 tables — `telemetry_capture_policies`, `telemetry_retention_policies`, `trace_content`. Re-derived live 2026-08-31 for Phase 4.7: +3 tables — `slo_definitions`, `slo_evaluations`, `runtime_alerts`. First re-derived 2026-08-25 during Phase 4.1's pre-step A, when this paragraph said "119 tables ... through `0041_canary_rollout`" — stale since Phase 3.5. The headline number is taken from the live database each phase, never carried forward.)* `alembic_version` (Alembic's own bookkeeping table, one column, no app data) is included below for completeness since it is a real table in the database.

**Phase 4.5 update**: 1 new table — `behavioral_findings` (one deterministic behavioral signal about one agent over one window, carrying a structured `explanation` and an `attribution` whose `connector` key is permanently null with a stated reason). **129 tables total.** A distinct table rather than a `signal_type` discriminator on `deployment_health_evaluations`: a health evaluation is *release*-scoped and consumed by a rollout's stage gate, a behavioral finding is *agent-runtime*-scoped and consumed by an operator; sharing would have meant a nullable `rollout_plan_id` always null on one half of the rows and a `signal_type` always null on the other. The **engine shape** is reused regardless. The unique constraint on `(agent_id, signal_type, window_start, window_end)` is the idempotency primitive, not a performance index. **No index added to `agent_executions` or `tool_calls`** — measurement showed Postgres combines two existing indexes via `BitmapAnd`. Migration `0049_behavioral_signals` is purely additive with no data backfill; `alembic downgrade -1`/`upgrade head` re-verified clean this session.

**Phase 4.4 update**: 2 new tables — `budgets` (a limit and a mode, scoped organization/project/agent/environment/model; **holds no cost** — real spend stays on `agent_executions.cost_amount` where `PricingService` wrote it) and `budget_reservations` (the reserve-then-reconcile ledger: `reserved_amount`/`actual_amount`/`status`/`period_key`, with a **partial unique index on `(budget_id, execution_id) WHERE status <> 'RELEASED'`** that is the idempotency primitive rather than a performance index) — plus one nullable `budget_id` on `runtime_governance_decisions`, so a budget-driven governance decision can name the ceiling that decided it. **128 tables total.** No index was added to `agent_executions`: measurement showed Phase 4.2's `ix_agent_executions_org_created` already carries the aggregation, and the worst case that remains is O(rows the tenant owns) which no index can fix. Migration `0048_cost_governance` is purely additive with no data backfill; `alembic downgrade -1`/`upgrade head` re-verified clean this session.

**Phase 4.3 update**: 2 new tables — `runtime_governance_policies` (the configurable in-loop governance rules, scoped organization → environment → agent with most-specific-first resolution, `constraints` JSONB, and the `mandatory` flag that drives fail-closed) and `runtime_governance_decisions` (append-only lineage of every material checkpoint decision: `checkpoint`/`decision`/`reason_code`/`obligation`/`policy_id`/`trace_id`, with CHECK constraints on the two enumerated columns and `REVOKE UPDATE, DELETE ... FROM PUBLIC`) — **126 tables total**. **No column was added to `agent_executions`**: the engine reads the cost, token and timing columns that already existed, and writes its own record to the new table rather than widening the hottest row in the system. `organization_id` on the policies table is nullable and means *platform default*; no tenant-facing API path can write one. Migration `0047_runtime_governance` is purely additive with no data backfill; `alembic downgrade -1`/`upgrade head` re-verified clean this session.

**Phase 4.2 update**: no new table and no new column — one index, `ix_agent_executions_org_created` on `agent_executions (organization_id, created_at DESC)`, added by `0046_trace_explorer_index` after measurement showed the column had no index at all. 124 tables, unchanged.

**Phase 4.1 update**: no new table — two nullable columns, `agent_executions.request_id` (partial-indexed `WHERE request_id IS NOT NULL`) and `runtime_events.span_id`, added by `0045_runtime_telemetry_context`. No backfill. 124 tables, unchanged.
...
All **61** Alembic revisions in `backend/migrations/versions/`, in chain order (oldest → newest), verified via `alembic history` and a directory listing this session. **Current head: `0061_assurance_evidence`** *(re-derived live 2026-09-16 for Phase 5.9 / M5.9; re-derived live 2026-09-15 for Phase 5.7 / M5.7; re-derived live 2026-09-14 for Phase 5.6 / M5.6; re-derived live 2026-09-10 for Phase 5.5 / M5.5; re-derived live 2026-09-09 for Phase 5.4 / M5.4; re-derived live 2026-09-08 for Phase 5.3 / M5.3; re-derived live 2026-09-05 for Phase 5.2 / M5.2; re-derived live 2026-09-04 for Phase 5.1 / M5.1; re-derived live 2026-09-03 for Phase M4.11a; re-derived live 2026-09-02 for Phase M4.11; re-derived live 2026-09-01 for Phase 4.8; re-derived live 2026-08-31 for Phase 4.7; first re-derived 2026-08-25, Phase 4.1 pre-step A, when this line said "42 revisions" and "`0041_canary_rollout`", stale by four phases)* (verified via `alembic current` against the live database; `alembic upgrade head` / `downgrade -1` / `upgrade head` re-verified clean this session for the newest revision).
...
| 41 | `0041_canary_rollout.py` | Phase 3.5 - Canary Deployment Engine: three new tables — `rollout_plans` (the rollout state machine + `version_id_col` concurrency guard), `rollout_stages` (ordered stages and their three gates), `deployment_health_evaluations` (ruling #3's AI-aware release-health verdicts) — plus two indexes on the pre-existing `agent_executions`: `(agent_version_id, created_at)` and `(deployment_id, created_at)`, the latter the first index that column has ever had. The pre-existing `deployment_health` table is untouched (ruling #3). No existing column altered; no data backfill. |
| 42 | `0042_automated_rollback.py` | Phase 3.7 - Automated Rollback & Recovery: two new tables — `rollback_trigger_policies` (per-tenant, most-specific-wins rules deciding *when* an automatic rollback fires, with `AUTO_EXECUTE`/`NOTIFY_ONLY` modes), `rollback_events` (append-only, one row per rollback that actually ran). |
| 43 | `0043_distributed_scheduler.py` | Phase 3.8 - Distributed Scheduler: job definitions and `job_runs` occurrence history, with the lease columns and the partial unique index that make dispatch exactly-once. |
| 44 | `0044_worker_fleet_rolling.py` | Phase 3.9 - Distributed Execution Worker Fleet & Rolling Strategy: the worker registry/heartbeat tables and the rolling-cohort columns. |
| 45 | `0045_runtime_telemetry_context.py` | Phase 4.1 - Runtime Telemetry & Trace Context Foundation: two nullable columns only — `agent_executions.request_id` (partial-indexed) and `runtime_events.span_id`. **No table, no backfill**: spans are derived, and `trace_id_for()` gives historical rows a trace identity by pure function. |
| 46 | `0046_trace_explorer_index.py` | Phase 4.2 - Unified AI Execution Trace: **one index and nothing else** — `(organization_id, created_at DESC)` on `agent_executions`, which had no `created_at` index at all. Measured before and after; turns an O(rows-the-tenant-owns) bitmap-plus-sort into an O(limit) ordered scan. No table, no column, no backfill. |
| 47 | `0047_runtime_governance.py` | Phase 4.3 - Runtime Governance Enforcement Engine: two new tables — `runtime_governance_policies` (scoped organization → environment → agent, `constraints` JSONB, `mandatory` drives fail-closed) and the append-only `runtime_governance_decisions` (`REVOKE UPDATE, DELETE ... FROM PUBLIC`, plus CHECK constraints on `decision` and `checkpoint`). **No column on `agent_executions`** — the engine reads existing cost/token/timing data. No data backfill; reversible, verified live. |
| 48 | `0048_cost_governance.py` | Phase 4.4 - Enterprise AI Cost Governance & FinOps: two new tables — `budgets` (limit + mode, five scopes; holds no cost) and `budget_reservations` (the reserve-then-reconcile ledger, with a partial unique index on `(budget_id, execution_id) WHERE status <> 'RELEASED'` as the idempotency primitive) — plus a nullable `budget_id` on `runtime_governance_decisions`. **No index on `agent_executions`**, and no cost copied anywhere: both are measured conclusions recorded in the migration's own docstring. Additive, no backfill, reversible. |
| 49 | `0049_behavioral_signals.py` | Phase 4.5 - Behavioral Signals & Runtime Anomaly Detection: one new table, `behavioral_findings` (one deterministic signal about one agent over one window, `explanation`/`attribution` JSONB), with `uq_behavioral_findings_window` on `(agent_id, signal_type, window_start, window_end)` as the idempotency primitive. **No index on `agent_executions`/`tool_calls`** (Postgres combines two existing indexes via `BitmapAnd`; measured). Additive, no backfill, reversible. |
| 50 | `0050_slos_and_alerts.py` | Phase 4.7 - SLOs, Alert Rules & Incident Signals: three new tables — `slo_definitions` (SLI/target/window/error-budget, `uq_slo_definitions_org_name`), append-only `slo_evaluations` (`uq_slo_evaluations_window` on `(slo_id, window_start, window_end)` as the idempotency primitive), and `runtime_alerts` (the lifecycle layer over `slo_evaluations`/`behavioral_findings`, with `uq_runtime_alerts_active_dedup` — a **partial** unique index over `(organization_id, dedup_key) WHERE status IN ('OPEN','ACKNOWLEDGED')` — as the "one condition = one alert" primitive, the 3.7 pattern). No existing table changed. **No index on `agent_executions`/`tool_calls`.** Additive, no backfill, reversible (downgrade/upgrade re-verified). |
| 51 | `0051_telemetry_privacy.py` | Phase 4.8 - Telemetry Privacy, Retention & Access Governance: three new tables — `telemetry_capture_policies` (per tenant/env/agent/classification → `METADATA_ONLY`/`REDACTED_CONTENT`/`FULL_CONTENT`/`DISABLED`, `uq_telemetry_capture_policies_scope` a COALESCE-based unique index over the scope tuple so two "platform default" rows cannot coexist), `telemetry_retention_policies` (per-class retention days, `uq_telemetry_retention_policies_class`), and `trace_content` (the governed telemetry copy, materialised on first authorised view; `uq_trace_content_source` on `(execution_id, source_table, source_id, sequence)` as the materialisation-idempotency primitive; `ix_trace_content_created` for the expiration scan). No existing table changed. Additive, no backfill, reversible (downgrade/upgrade re-verified). |
| 52 | `0052_key_material_canary.py` | Phase M4.11 - Production Integrity Closure: one new table `key_material_canary` (`purpose`, `key_fingerprint`, `verifier` = `Fernet(key).encrypt(<public constant>)`, `key_provider`; `uq_key_material_canary_purpose_fp` one row per (purpose, fingerprint)). The startup fail-loud check decrypts the row for the active key's fingerprint to detect a *wrong* key, not only an absent one. Holds no key and no secret. No existing table changed, no backfill, changes no decrypt behaviour for any existing row; reversible (downgrade/upgrade re-verified). |
| 53 | `0053_installation_bootstrap.py` | Phase M4.11a - Install-Mode Classification Hardening: one new table `installation_bootstrap`, a **singleton** (`singleton` pinned true by `ck_installation_bootstrap_singleton_true` + `uq_installation_bootstrap_singleton`). Non-secret provenance only — `bootstrapped_at`, `active_key_fingerprint`, `key_provider`, `keyring_schema_version`, `recorded_via` (`bootstrap`|`backfill`). Its presence is the authoritative EXISTING signal for `detect_install_mode`, replacing M4.11's "absence of ciphertext ⇒ NEW" inference. No existing table changed, no data backfill in the migration (the marker is written by a deliberate bootstrap or backfilled at startup after key verification), changes no decrypt behaviour for any existing row; reversible (downgrade/upgrade re-verified). |
| 54 | `0054_agent_asset_model.py` | Phase 5.1 / M5.1 - Universal Agent Asset Model + Ownership: **no new table** — seven additive columns on `agents` (`control_state` DISCOVERED/CLAIMED/REGISTERED/GOVERNED with `ck_agents_control_state` + `ix_agents_control_state`; `origin_category` NATIVE/EXTERNAL/UNKNOWN with `ck_agents_origin_category`; `origin_provider` soft vendor string, no enum; discovery metadata `first_observed_at`/`last_observed_at`/`discovery_source_ref`/`discovery_confidence`, columns only, 5.2 populates). `control_state` is a distinct dimension from `lifecycle_status` (unchanged). Idempotent backfill: every pre-existing agent row → `GOVERNED`/`NATIVE`/`ACT_NATIVE` (a known truth — every existing agent is native and governed). Additive, reversible (downgrade/upgrade re-verified), no existing-row behaviour change. |
| 55 | `0055_agent_discovery.py` | Phase 5.2 / M5.2 - Agent Discovery Framework: four new tables — `discovery_sources` (configured, tenant-scoped, authenticated, governed external systems), `discovery_runs` (one sweep, checkpointed, resumable), `discovery_observations` (**append-only** evidence — `UPDATE`/`DELETE` revoked from `PUBLIC`, mirroring migration 0047's `runtime_governance_decisions`), `discovery_findings` (no-silent-merge + staleness escape hatch, with a partial unique index limiting one `OPEN` `STALE_AGENT` finding per agent). No existing table changed, no data backfill (a genuinely new domain). Additive, reversible (downgrade/upgrade re-verified). |
| 56 | `0056_control_graph.py` | Phase 5.3 / M5.3 - Identity, Delegation & Trust Graph: one new table — `control_graph_edges` (typed, directed, tenant-scoped edges between existing node rows; `edge_type` DELEGATES_TO / AGENT_DELEGATES_TO / TRUSTS / ACTS_AS; soft-revoked (`revoked_at`), optionally time-bounded (`valid_until`); `uq_control_graph_edges_active` partial unique index — one live edge per (org, source, edge_type, target), idempotent create; bidirectional `ix_control_graph_edges_source` `(org, source_type, source_id)` / `ix_control_graph_edges_target` `(org, target_type, target_id)` for recursive traversal; CHECK constraints on the type/provenance vocabularies and a no-self-loop CHECK). **No graph database** — the authority chain and reachability are recursive CTEs over this table. No existing table changed, no data backfill. Additive, reversible (downgrade/upgrade re-verified). |
| 57 | `0057_dependency_graph.py` | Phase 5.4 / M5.4 - MCP / Tool / Credential / Resource Dependency Graph: one new table — `mcp_servers` (an MCP server's identity, provenance, `trust_status` APPROVED/PENDING/REJECTED/UNKNOWN, version, endpoint ref, `uq_mcp_servers_org_name`; holds NO tools) — plus one additive nullable column `tools.mcp_server_id` (FK → `mcp_servers`, ON DELETE SET NULL: an MCP-exposed tool is an ordinary `tools` row pointing at its server — MCP via the Tool domain, no second registry, ADR-0018), and the `control_graph_edges` `source_type`/`target_type`/`edge_type` CHECK constraints **widened in place** (drop + recreate, no column altered) for the `MCP_SERVER`/`CONNECTOR` node types and the 9 dependency `edge_type` values (`DEPENDS_ON_TOOL`/`_MCP_SERVER`/`_CREDENTIAL`/`_CONNECTOR`/`_RESOURCE`, `MCP_EXPOSES_TOOL`, `TOOL_USES_CREDENTIAL`, `TOOL_ACCESSES_RESOURCE`, `CREDENTIAL_ACCESSES_RESOURCE`). **No graph database, no materialised projection** — the §V blast-radius benchmark (~128k edges, single tenant) measured assembly at 40–274 ms. No existing column altered/dropped, no data backfill. Additive, reversible (downgrade/upgrade re-verified). |
| 58 | `0058_security_posture.py` | Phase 5.5 / M5.5 - Security Posture & Shadow Findings: two new tables — `posture_findings` (a standing, explainable posture finding about an asset, with the Phase 4.7 finding lifecycle: `status` OPEN/ACKNOWLEDGED/RESOLVED/SUPPRESSED, `uq_posture_findings_active` partial unique index over `(organization_id, dedup_key) WHERE status IN ('OPEN','ACKNOWLEDGED')` — one open finding per condition, reopen-on-recurrence; `rule_id`/`control_id`, `evidence` JSONB refs to the 5.1-5.4 rows, `severity`, `reason`, `remediation`, `outcome` FINDING/INSUFFICIENT_DATA, `governing_policy` with the ruleset/rule version + params for score reconstructability) and `posture_rule_settings` (per-organization enable/disable + parameter override for a code-defined rule, monotonic `revision`, `uq_posture_rule_settings_org_rule`). **A dedicated table, not a `source='POSTURE'` discriminator on `runtime_alerts`** — the same "new table not a discriminator" call Phases 4.5/4.7 made (ADR-0019); the lifecycle *shape* is reused (`app.slo.states` imported). **No `shadow` column** (shadow is a query over `rule_id`), **no opaque score** (the summary is a deterministic weighted sum), **no enforcement** (findings are signals). No existing table changed, no data backfill. Additive, reversible (downgrade/upgrade re-verified). |
| 59 | `0059_threat_containment.py` | Phase 5.6 / M5.6 - Runtime Threat Detection & Containment: two new tables — `threat_findings` (a runtime-event finding with the Phase 4.7 lifecycle: `status` OPEN/ACKNOWLEDGED/RESOLVED/SUPPRESSED, `uq_threat_findings_active` partial unique index — one open finding per condition, reopen-on-recurrence; `rule_id`, `agent_id`, `attribution` for 5.3/5.4 blast-radius, `evidence` refs to the triggering M4 rows) and `containment_actions` (`action_type` one of 7 CONTAINMENT_ACTION_TYPES, `authority` one of 6 CONTAINMENT_AUTHORITIES, `trigger` THREAT/OPERATOR, `control_state_at_time` — the truthful-containment signal captured at decision time, `status` RECOMMENDED/PENDING_CONFIRMATION/EXECUTED/REFUSED/FAILED/REVERTED, `authority_ref`/`result` JSONB naming the real authority row, `reversible`). **Containment records; it does not enforce** — every EXECUTED row references a row a real existing authority (KillSwitchService/ToolRegistryService/CapabilityService/ConnectorService/GovernancePolicyService/api_key_service) produced (ADR-0020). **No `shadow`-style boolean for containment either** — a REFUSED row is a complete, honest record, never a fake EXECUTED. No existing table changed, no data backfill. Additive, reversible (downgrade/upgrade re-verified). |
| 60 | `0060_external_gov_bridge.py` | Phase 5.7 / M5.7 - External Agent Governance Bridge: three new tables — `external_capability_grants` (scoped/expiring/revocable/rate-limited signing credential for an agent outside ACT; `key_id` public, `secret_ciphertext` Fernet via M4.11 `credential_crypto`, `scope` JSONB of `{capability, target_ref}` entries — it BOUNDS reach and grants nothing; authority is decided per call by the existing `AuthorizationGateway`), `external_request_nonces` (replay ledger; `uq_ext_nonce_grant_nonce` IS the anti-replay primitive — the database, not a cache, so two concurrent replays cannot both commit) and `external_gateway_calls` (one boundary decision: `capability_key`, `enforcement_mode_at_time` stored not re-derived, the real gateway's `authz_decision`, `policy_outcome` NOT_APPLICABLE/SATISFIED/APPROVAL_REQUIRED/UNEVALUABLE, `cost_outcome` NOT_MEASURABLE/WITHIN_BUDGET/EXCEEDED/UNEVALUABLE, `outcome`, `fail_mode`, `dispatch_status`; `ck_ext_calls_denied_never_dispatched` makes a denied-but-dispatched row unrepresentable). Plus **one nullable column on `agents`** — `external_enforcement_mode`, CHECK-restricted to OBSERVED/ADVISORY/GATEWAY_ENFORCED. **`NATIVE_ENFORCED` is not storable anywhere**: it is derived from `control_state == 'GOVERNED'` (the same signal 5.6's containment gate reads), so full enforcement has to be true rather than declared, and **no backfill is required** — every pre-existing row gets a truthful mode the instant the column exists (ADR-0021). No existing table changed. Additive, reversible (downgrade/upgrade re-verified). |
| 61 | `0061_assurance_evidence.py` | Phase 5.9 / M5.9 - Assurance, Evidence & Compliance: two new tables — `assurance_evaluations` (`result` PASS/FAIL/INSUFFICIENT_EVIDENCE, `evidence` JSONB naming what the control read or the absence it found, `evidence_as_of`, `stale`, `exception_reason`/`_by`/`_at`, `catalog_version`; `ck_assurance_eval_stale_never_passes` makes a stale PASS unrepresentable — evidence older than the freshness policy is reported INSUFFICIENT_EVIDENCE, never a pass) and `assurance_evidence_bundles` (scope, `catalog_version`/`mapping_version`, `content_digest`, DSSE `signature`, `exported_by` — the record that evidence left the system; the payload is NOT stored). **No status, verdict or score column anywhere** — a badge cannot be rendered from a field that does not exist (ADR-0022). Control definitions and framework mappings live in versioned code, not a table. No existing table changed, no data backfill. Additive, reversible (downgrade/upgrade re-verified). **(head)** |

*(Rows 42–47 were added during Phase 4.3. This table had said "**(head)**" against `0041_canary_rollout` since Phase 3.5 while §3's headline above was kept current — the same stale-body/current-headline split Phase 4.1's pre-step A found in §2, §3 and §5, arriving here in the opposite direction. Descriptions for 42–44 are summarized from their own migration docstrings.)*

## 4. Implemented Modules

**352 non-`__init__.py` Python files** under `backend/app/` *(re-derived live during Phase 4.3; this headline had said **268**, a figure last correct around Phase 3.3 — the per-phase notes below were extended faithfully while the number above them was not, the same drift pattern Phase 4.1's pre-step A corrected in §2/§3/§5. Phase 4.9 adds **1** more: `app/runtime/observability_center.py` (the read-only aggregation read model for the Observability Center — no new package, lands directly in `app/runtime/` beside `operations.py` which it mirrors). Phase 4.8 before it added **7**: the new `app/telemetry_privacy/` package — `modes.py`, `policy.py`, `redaction.py`, `content.py`, `retention.py`, `schemas.py`, `routes.py` (`__init__.py` excluded as everywhere else in this section), a sibling of `app/runtime`, `app/observability`, `app/slo`. Phase 4.7 before it added **8**: `app/slo/`. Phase 4.6 before that added **10**: `app/telemetry_export/`. Phase 4.5 adds **5** more: the new `app/behavior/` package — `states.py`, `signals.py`, `engine.py`, `schemas.py`, `routes.py`, a sibling of `app/runtime`, `app/observability` and `app/finops`. Phase 4.4 before it added **6**: the `app/finops/` package — `aggregation.py`, `budgets.py`, `reservations.py`, `guard.py`, `schemas.py`, `routes.py`, a sibling of `app/runtime` and `app/observability` so the dependency direction is visible in the import graph. Phase 4.3 before it added **6**: the `app/runtime/governance/` package — `contract.py`, `constraints.py`, `engine.py`, `policies.py`, `schemas.py`, `routes.py`, with `__init__.py` excluded as everywhere else in this section.)* The per-phase history below is retained as written. 266 at the last full count, post Phase 3.2. **+2 this update, Phase 3.3**: the new `app/runtime/release_gate/` package — `checks.py`, `service.py` (`__init__.py` excluded from this count as it is everywhere else in this section). 263 at the count before that, post Phase 2.1.2. **+3 this update, Phase 2.1.3**: `app/integration/health.py`, `registry.py`, `scheduler.py` — no new sub-package, all three land directly in the existing `app/integration/` directory. 252 at the count before that, post Phase 2.1.1. **+11 that update, Phase 2.1.2**: the new `app/integration/auth/` sub-package — `base.py`, `registry.py`, `service.py`, `token_manager.py` (4 files) plus `schemes/api_key.py`, `basic.py`, `bearer.py`, `mtls.py`, `oauth2_authorization_code.py`, `oauth2_client_credentials.py` (6 files) — plus `app/integration/mock_authenticated.py` (1 file); `__init__.py` files excluded from this count as they are everywhere else in this section. 243 at the count before that, post Phase 5.6a.3; **+9 that update, Phase 2.1.1**: the entire new `app/integration/` package — `base.py`, `errors.py`, `lifecycle.py`, `mock.py`, `routes.py`, `schemas.py`, `service.py`, `types.py` (8 files) — plus `app/models/integration.py` (1 file). A genuinely new sibling domain, not an extension of an existing one, matching how `app/runtime/providers/` first appeared as a new package in Phase 5.7a.1's count. AST-parsed for their module docstring (first line shown) and every top-level class/public function. Grouped by domain (backend directory structure). Frontend module structure is summarized separately at the end of this section (file-count only — the frontend was not AST-parsed since the exhaustive symbol-level inventory the user asked for was scoped to "modules" in the module/class/function sense, which is the backend's organizing unit; the frontend's `src/modules/*` React component tree does not have an equivalent exported-symbol convention).
...
*(`operations` (Phase 3.10) and `observability` (Phase 4.9) were added after this table's initial generation; both are the M3 / M4 operator-center modules and follow the read-and-trigger pattern.)*

## 5. API Surface

Extracted by importing the live `app.main:app` FastAPI object and iterating `app.routes` — every method, path, and (where the endpoint depends on `require_permission(code)`) the exact permission code, read out of that dependency's closure. This is the actual routing table the server would serve. `/docs`, `/openapi.json`, `/redoc` **and `/docs/oauth2-redirect`** are excluded from the count and table below. *(The fourth exclusion was omitted from this sentence until Phase 4.3, when merging to `main` and re-measuring showed the documented three-path method yields 550 while the figure this document has carried forward since Phase 3.6 is built on four. Verified against a temporary `git worktree` at the Phase 4.2 merge (`8664306`): 545 with three exclusions, **544 with four** — so the recorded history is the four-path measurement and the number was right while the description of how to reproduce it was not. Corrected here rather than left as a figure nobody could re-derive; see §9 item 20.)* A permission of `—` means the route has no `require_permission` dependency (either public, or gated by a different mechanism — e.g. API-key auth via `get_current_agent`, or session/JWT auth alone via `get_current_user` with no RBAC check).

Grouped by top-level path prefix for readability. Re-extracted this session (previously 452 after Phase 5.7a.2/5.7a.3/5.7a.4, none of which added a route; 4 new routes at Phase 5.7a.5 — provider-credential CRUD/test endpoints, bringing the total to 456). Re-confirmed unchanged at 456 after both Phase 5.6a.1 and Phase 5.6a.2 — neither added an HTTP route; both extend the existing tool-call execution path. **Phase 5.6a.3** added 1 new route (`GET /executions/{id}/messages`, the conversation transcript) — this document's prose said "bringing the total to 457" at the time, but a direct re-measurement this pass, against a temporary `git worktree` checked out at that exact commit (`a98a9c0`), shows the live count was **456**, not 457 — a small, previously-uncaught inaccuracy in prior prose, corrected here rather than carried forward (see §9's new gap item). **Phase 2.1.1** adds 8 new routes under `/api/v1/integration` (connector types, tenant instances, lifecycle actions, lifecycle events) — freshly verified both sides of this addition: 456 before, 464 after, an exact +8 with nothing else added or removed. **Phase 2.1.2** adds 7 new routes under the same prefix (`GET /auth-schemes`, `GET`/`PUT`/`DELETE .../credentials`, `POST .../credentials/validate`, `GET`/`POST .../oauth/callback`) — 464 before, 471 after, an exact +7. **Phase 2.1.3** adds 3 new routes under the same prefix (`GET .../health`, `POST .../health/check`, `GET .../health/history`) — 471 before, 474 after, an exact +3. **Phase 2.2.1/2.2.2/2.2.3/2.2.4** add none (each generic connector is configured through the pre-existing `POST`/`PATCH /connectors` endpoints; the invocation bridge is a direct Python entry point, not yet an HTTP route) — unchanged at 474 through all four. **Phase 2.3.1** adds 10 new routes: 4 public under `/api/v1/auth/federation` (login/callback/SAML ACS/metadata) and 6 admin CRUD under `/api/v1/identity/federation/configs` — 474 before, 484 after, an exact +10. **Phase 3.1** adds 5 new routes under `/api/v1/runtime/deployments/{id}/lifecycle/...` (`transition`/`pause`/`resume`/`retire`/`events`) — 484 before, 489 after, an exact +5; the pre-existing `POST /deployments` is extended additively (honors `Idempotency-Key`) without becoming a new route. **Phase 3.2** adds 10 new routes: 1 under `/api/v1/runtime/deployments/{id}/promote` (no path collision found — used directly, unlike 3.1's own `/lifecycle/...` nesting) and 9 under `/api/v1/runtime/environments`/`/promotion-paths` (environment CRUD + policy, promotion-path CRUD) — 489 before, 499 after, an exact +10. **Phase 3.3** adds 3 new routes under `/api/v1/runtime/deployments/{id}/preflight` (run, get latest, get history) — no path collision found, used directly — 499 before, 502 after, an exact +3. **Phase 3.4** adds 3 new routes under `/api/v1/runtime/agents/{agent_id}/environments/{environment_id}/traffic` (get current, set weights, get history) — no path collision found — 502 before, 505 after, an exact +3. Mounted on agent+environment rather than the build prompt's alternative `/deployments/{id}/traffic`, because an allocation spans several deployments (reason recorded in §4's routes entry). **Phase 3.5** adds 10 new routes: 1 under `/api/v1/runtime/agents/{agent_id}/environments/{environment_id}/rollouts` (create+start) and 9 under `/api/v1/runtime/rollouts/{id}/...` (get, health, advance, evaluate, pause, resume, abort, promote, request-rollback) — no path collision found — 505 before, 515 after, an exact +10. **Phase 3.6** adds 3 new routes under `/api/v1/runtime/deployments/{id}/strategy/...` (execute, blue-green/switch, blue-green/rollback) — no path collision found — 515 before, 518 after, an exact +3. **Phase 4.3** adds 5 new routes: 4 under `/api/v1/runtime/governance/policies` (list/create/get/patch) and 1 under `/api/v1/runtime/executions/{id}/governance-decisions` — no path collision found — **544 before, 549 after, an exact +5**. Mounted on the existing `/api/v1/runtime` prefix rather than the build prompt's `/api/v1/executions/...`, which would have created a second, prefix-less execution namespace beside the established one. **No route can invoke a checkpoint** — enforcement is in the loop, which is what keeps "one enforcement path" true of the HTTP surface too. **Phase 4.4** adds 10 new routes: 4 under `/api/v1/cost` (summary, timeseries, anomalies, per-execution provenance) and 6 under `/api/v1/budgets` (list/create/get/patch/delete/utilization) — no path collision found, and deliberately clear of the legacy `/analytics/cost`, which is deprecated in place rather than moved — **549 before, 559 after, an exact +10**. **Phase 4.5** adds 3 new routes: 2 under `/api/v1/runtime/behavior` (findings, evaluate) and 1 under `/api/v1/runtime/agents/{id}/behavior` — no path collision found — **559 before, 562 after, an exact +3**. Mounted on the existing runtime prefix rather than the build prompt's `/api/v1/agents/{id}/behavior`, which would have created a second agents namespace beside the established one. **No new permission**: `runtime.telemetry.view` already covered derived telemetry about an execution stream. **Phase 4.6** adds 4 new routes: `GET`/`PUT /api/v1/observability/export/config` and `GET /api/v1/observability/export/health` under the existing 4.2 observability prefix, plus a prefix-less `GET /metrics` (Prometheus exposition, authenticated + tenant-scoped) — **562 before, 566 after, an exact +4**. No path collision found; one existing 4.2 test that asserted a bare count of routes under `/api/v1/observability` was updated to intent-preserving membership assertions. **One new permission** (`runtime.telemetry.export.manage`) — reading exporter health and the metrics surface reuses `runtime.telemetry.view`, but pointing telemetry at a third-party collector sends operational metadata off-platform and is audited, so configuring the target has its own manage code. **Phase 4.7** adds 12 new routes under `/api/v1/runtime`: SLO CRUD (`GET`/`POST /slos`, `GET`/`PATCH`/`DELETE /slos/{id}`), `POST /slos/evaluate` (the interim idempotent op), `GET /slos/{id}/evaluations`, and the alert surface (`GET /alerts`, `GET /alerts/{id}`, `POST /alerts/{id}/acknowledge`|`/resolve`|`/suppress`) — **566 before, 578 after, an exact +12**. No path collision found (`/slos/evaluate` is declared before `/slos/{id}` so the static path wins). Two new permissions (`runtime.slo.manage`, `runtime.alert.manage`); reads reuse `runtime.telemetry.view`, the 4.2/4.4/4.5 precedent. **Phase 4.8** adds 10 new routes: capture-policy CRUD (`GET`/`POST /api/v1/runtime/telemetry/capture-policies`, `GET`/`PATCH`/`DELETE .../{id}`), `GET .../telemetry/effective-mode`, retention (`GET`/`POST .../telemetry/retention-policies`, `POST .../telemetry/retention/run`), and `GET /api/v1/observability/traces/{trace_id}/content` under the 4.2 observability prefix — **578 before, 588 after, an exact +10**. No path collision found. Three new permissions (`runtime.trace.content.view` — registered for the first time, strictly stronger than `runtime.telemetry.view`, not in the read-only bundle; `runtime.telemetry_policy.view`/`.manage`). **Phase 4.9** adds 2 new routes under `/api/v1/runtime`: `GET /overview` (the fleet composite for the Observability Center) and `GET /governance/decisions` (the tenant-wide governance-decision list 4.3 exposed only per execution) — **588 before, 590 after, an exact +2**. Both read-only aggregation, no new permission (`runtime.telemetry.view` / `runtime.execution.view`), no new state-changing endpoint (the center's actions reuse 4.7/4.8's). *(This narrative is not extended for every phase — 3.7 through 4.2 are absent above. The authoritative figure is the "Total application routes" line below, which is re-derived live each phase; see §9 item 19 on why the headline, not the prose, is the thing that must be re-measured.)*

Total application routes (excluding `/docs`, `/openapi.json`, `/redoc`, `/docs/oauth2-redirect`): **603** *(re-derived live 2026-09-05, Phase 5.2 / M5.2: +10 — the full `/api/v1/discovery` surface (adapters, sources CRUD, manual run trigger, run history, findings list/resolve). Phase 5.1 / M5.1 before it: +3 — `GET`/`POST /api/v1/runtime/agents/{id}/control-state` and `POST /api/v1/runtime/agents/{id}/claim` (the server-authoritative control-state + claim surface; `control_state` never mass-assignable via the general agent path). Phase 4.9 before it: +2 — `GET /api/v1/runtime/overview` and `GET /api/v1/runtime/governance/decisions`, both read-only aggregation. Phase 4.8 before it: +10 capture/retention/content routes. Phase 4.7 before that: +12 SLO/alert routes.)*
...
| Method | Path | Permission | Handler |
|---|---|---|---|
| GET | `/api/v1/budgets` | `runtime.budget.view` | `app.finops.routes::list_budgets` |
| POST | `/api/v1/budgets` | `runtime.budget.manage` | `app.finops.routes::create_budget` |
| DELETE | `/api/v1/budgets/{budget_id}` | `runtime.budget.manage` | `app.finops.routes::delete_budget` |
| GET | `/api/v1/budgets/{budget_id}` | `runtime.budget.view` | `app.finops.routes::get_budget` |
| PATCH | `/api/v1/budgets/{budget_id}` | `runtime.budget.manage` | `app.finops.routes::update_budget` |
| GET | `/api/v1/budgets/{budget_id}/utilization` | `runtime.budget.view` | `app.finops.routes::budget_utilization` |
| GET | `/api/v1/cost/anomalies` | `runtime.cost.view` | `app.finops.routes::cost_anomalies` |
| GET | `/api/v1/cost/executions/{execution_id}/provenance` | `runtime.cost.view` | `app.finops.routes::cost_provenance` |
| GET | `/api/v1/cost/summary` | `runtime.cost.view` | `app.finops.routes::cost_summary` |
| GET | `/api/v1/cost/timeseries` | `runtime.cost.view` | `app.finops.routes::cost_timeseries` |
| GET | `/api/v1/runtime/agents/{agent_id}/behavior` | `runtime.telemetry.view` | `app.behavior.routes::agent_behavior` |
| GET | `/api/v1/runtime/behavior/findings` | `runtime.telemetry.view` | `app.behavior.routes::list_findings` |
| POST | `/api/v1/runtime/behavior/evaluate` | `runtime.telemetry.view` | `app.behavior.routes::evaluate_behavior` |
...
| GET | `/api/v1/runtime/executions/{execution_id}/governance-decisions` | `runtime.execution.view` | `app.runtime.governance.routes::list_governance_decisions` |
...
| GET | `/api/v1/runtime/governance/policies` | `runtime.governance.manage` | `app.runtime.governance.routes::list_governance_policies` |
| POST | `/api/v1/runtime/governance/policies` | `runtime.governance.manage` | `app.runtime.governance.routes::create_governance_policy` |
| GET | `/api/v1/runtime/governance/policies/{policy_id}` | `runtime.governance.manage` | `app.runtime.governance.routes::get_governance_policy` |
| PATCH | `/api/v1/runtime/governance/policies/{policy_id}` | `runtime.governance.manage` | `app.runtime.governance.routes::update_governance_policy` |
...
20. **A correct number under an incorrect description of how it was measured.** §5 stated its method as "excluding `/docs`, `/openapi.json`, `/redoc`" and reported **544** after Phase 4.2. Re-measured during Phase 4.3's merge, that three-path method yields **545**; the four-path method that also drops `/docs/oauth2-redirect` yields **544**. Verified against a temporary `git worktree` at the Phase 4.2 merge commit (`8664306`) rather than inferred. So every route figure this document has carried since Phase 3.6 is *correct*, and the sentence telling a reader how to reproduce it was *wrong* — which is a subtler version of item 19's failure: not a stale number under a maintained narrative, but a live number under a method nobody could execute to get it. The description is corrected in §5. **The lesson**: a stated method is a claim, and it should be run rather than read when a phase re-derives the figure it produces.
...
22. **The SRS §41 completion-gate table (gates A–O) is not carried in the repo.** Individual phases reference their own gate letter in a test or doc docstring (`test_cost_governance` names D/E, `test_telemetry_privacy` names F, `test_otel_interop` names H/I/N, `test_slos_and_alerts` names J/K, `test_behavioral_signals` names L, `observability-center.md` names M), but there is no single place mapping all fifteen letters to their concerns. Phase 4.10's gate-closure audit (`test_milestone_4_proof.py::_GATE_CLOSURE` + `docs/runtime/milestone-4-proof.md`) reconstructs the full A–O mapping from those per-phase references plus the 4.10 build prompt — **a reconstruction, not a transcription.** If a future milestone defines its own completion gates, it should either carry the SRS's gate table into `docs/` or continue this reconstruct-and-document pattern. Reported per the 4.10 build prompt's "report SRS/repo conflicts, don't silently redesign".

23. **`GOVERNANCE_CHECKPOINT_UNEVALUABLE` is retryable, and Phase 4.10's §36 proof confirmed it against `_fail_or_retry`'s own comment rather than fixing it.** A mandatory governance checkpoint that cannot be evaluated STOPs the *attempt* (fail-closed: the loop never runs past it, `loop_iterations == 0`) but the execution is then requeued for retry — because a transient dependency failure is exactly what the retry policy exists for, and after `max_attempts` it goes `DEAD_LETTERED`. This is not a defect; the code comment in `ExecutionWorkerService._fail_or_retry` states it explicitly. Named here because an end-to-end test that asserts `status == "FAILED"` immediately after the POST will see `QUEUED` instead and mistake documented behaviour for a bug — `test_ac09` asserts the honest property (`status ∈ {FAILED, QUEUED, BLOCKED, DEAD_LETTERED}`, never `SUCCEEDED`).
