...
| **3.9 – 3.10** | **NOT STARTED** | No code references them beyond forward-looking doc comments in the shipped sub-phases' own files (`lifecycle.py`'s module docstring names exactly which phase owns each declared-but-undriven transition edge; `strategies.py`'s `RollingStrategy` is the seam 3.9 fills). 3.9 will reuse 3.8's lease discipline and the same commit-before-dispatch lesson to run agent executions at scale, and is where the PG16/17 Compose mismatch becomes materially testable. |
