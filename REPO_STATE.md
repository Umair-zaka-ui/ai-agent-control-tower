...
**113 tables**, extracted via live `sqlalchemy.inspect()` against the local Postgres database after running every migration through `0038_environments_promotion` (head). This reflects exactly what the migration chain produces. `alembic_version` (Alembic's own bookkeeping table, one column, no app data) is included below for completeness since it is a real table in the database.
...
- Classes: `AgentDefinition,AgentVersion,AgentReleaseChannel,AgentVersionSnapshot,AgentReleaseMetadata,AgentReleaseArtifact,AgentReleaseNote,AgentVersionStatusHistory,AgentDeployment,Environment,PromotionPath,AgentExecution,ExecutionAttempt,ExecutionLock,ModelPricing,ProviderCredential,Capability,AgentCapability,Tool,AgentTool,ToolCall,ToolCredential,RuntimeEvent,DeploymentHealth,DeploymentEvent,IdempotencyRecord,IdempotencyKey,RuntimeApproval` (`ModelPricing` — Phase 5.7a.3, previously missing from this list only because this pass is the first full re-check of it since; `ProviderCredential` — Phase 5.7a.5; `ToolCredential` — Phase 5.6a.1, same `__repr__`-redaction discipline as `ProviderCredential`, see §10.31/§10.41; `DeploymentEvent`/`IdempotencyKey` — Phase 3.1; `Environment`/`PromotionPath` — Phase 3.2)
...
| **3.3 – 3.10** | **NOT STARTED** | No code references them beyond forward-looking doc comments in 3.1's/3.2's own files (`lifecycle.py`'s module docstring names exactly which phase owns each declared-but-undriven transition edge). |
...
Output of `git branch --sort=-committerdate`, with committer dates added via `--format`. Names and dates only — 49 local branches, re-verified this session (was 48 at the last full count); each mirrored by an `origin/*` remote-tracking branch at the same commit/date (shown once below; the `origin/<name>` counterpart is identical). 1 new branch since the last full count: `feat/3.2-environments-promotion` (merged into `main` via a `--no-ff` merge, per the git-log evidence below).

| Branch | Committer date |
|---|---|
| `main` | 2026-08-10 22:02:28 +0500 |
...
Current branch: `main`, at `a500991` ("Merge Phase 3.2: Environment & Promotion Model"). Working tree clean once this document's own update (and the accompanying `CHANGELOG.md`/`ROADMAP.md` updates) are committed.
