...
- **V0.1 closure (2026-09-17):** W-7 fixed as test data only — fixed, unrelated fields for the two same-org agents (constant score 0.2548 under the unchanged production model), 30/30 consecutive runs, and a negative control proving the test still fails when per-agent scoping is broken. The 5.1 guard `test_ac09` replaced its unordered `limit(500)` sample with a full-population violating-rows query (proven to bite). **W-2 not implemented (§2A stop):** the EXTERNAL/UNKNOWN × GOVERNED cell of the origin × control-state matrix cannot be derived from live semantics; options recorded in `docs/validation/v0/closure-v0.1.md`. Final backend run **2,640 passed / 0 failed / 1 deselected** (2,641 collected); frontend 384, `tsc -b` clean, build green. Verdict remains **CONDITIONAL — REVIEW REQUIRED** pending the W-2 decision.
- **V0.2 closure (2026-09-17) — W-2 resolved (ADR-0023):** `control_state = GOVERNED` requires `origin_category = NATIVE`, enforced in the one control-state service before the matrix at every writer, with the stable error `CONTROL_STATE_ORIGIN_INCOMPATIBLE` (409) and audited refusals (`RUNTIME_AGENT_CONTROL_STATE_REJECTED`); both `GOVERNED` edges removed from the transition matrix; mass assignment proven closed; the M5.1 full-population guard extended to the whole matrix. `test_m51_end_to_end_proof` step 6 now asserts REGISTERED as the truthful terminal state and that GOVERNED is refused. 103 non-native GOVERNED test rows classified and removed (0 unexplained); posture:766 fixture corrected. No migration (a DB CHECK is recommended for separate authorization). Targeted 348 passed, 2000 warnings in 446.20s (0:07:26); **V0 final clean baseline 2,648 passed / 0 failed / 1 deselected** (2,649 collected); frontend 384 passed, `tsc -b` clean, build green. **Verdict: V0 PASSED — READY FOR V1 AUTHORIZATION.**
...
- **The end-to-end proof crosses the OS process boundary twice, in both directions.** A demonstration made only of in-process service calls would prove that ACT's own functions call each other, which is not the claim. So: **inbound**, ACT discovers an agent it did not build through a real `http.server` registry on a real socket, holding no DB lock across the fetch; **outbound**, that agent calls ACT's gateway from a **separate OS process** over a **real uvicorn socket**, signing its own requests — its source parsed and asserted to import nothing from `app`, so it could not reach ACT's objects if it tried. Fourteen steps, each a real cause producing a real effect through a real authority, nothing pre-inserted: discovery → reconciliation (second sweep creates 0, links 1) → truthful `DISCOVERED`, unowned → shadow **with its conditions** → blast-radius to a payroll-kind resource via an **unapproved MCP** → claim (guarded, audited) → governance enrolment at `GATEWAY_ENFORCED` with `reaches_agent_execution: false` and no "govern" in the label → allowed call dispatched to a real capability, forbidden call denied → threat evaluation → containment → authority chain → assurance (owner now `PASS`, traceability `INSUFFICIENT_EVIDENCE`) → command center showing the true estate → nine audit event types reconstructing the flow, and tenant 2 receiving 404.
- **The single most important assertion in the milestone: the kill is REFUSED.** ACT does not run the external agent, and Phase 5.6 refuses to fake a suspend it cannot perform. The proof asserts `SUSPEND_AGENT → REFUSED` with a real reason and an empty `authority_ref`, and then asserts that the containment ACT *genuinely* holds at this reach — revoking the boundary grant — takes effect: the external process is re-run and its call is refused with nothing reaching the capability. An earlier draft asserted a *successful* suspend; it would have passed only if the platform lied.
- **§44 gate audit: A–R, all eighteen, each to a named passing proof.** 5.10 directly owns **N** (adversarial tenant isolation on every M5 surface, and **per-hop** — a foreign resource exposes no agents and a kind-scoped sweep never names another tenant's row), **O** (measured scale), **Q** (the full M1–5.9 regression unchanged) and **R** (the cross-process E2E). The other fourteen compose their owning phase's tests rather than duplicating them. The SRS §44 consolidated table is not carried in the repository; the letter↔concern mapping is reconstructed from the per-phase gate references and stated as such — the same gap Phase 4.10 reported for §41.
- **Scale, measured and recorded: `agents-reaching` in 98.9 ms and 119.9 ms** over 4,400 edges in a single busy tenant (4,000 **real** agent rows, 400 tools, 100 payroll-kind resources, `ANALYZE`d). The relational recursive CTE meets the target, so **no projection and no graph database** were introduced (ADR-0017); the proof asserts no graph-DB dependency has appeared in `requirements.txt`. The ceiling is deliberately generous (5 s) — it detects a cliff, not a micro-benchmark, because a tight bound on shared hardware is a flake generator rather than a guarantee.
- **One real defect surfaced, fixed as a reported bug fix — no proof weakened.** The race proof drove two concurrent sweeps of one source. Both loaded the same agent row at `row_version = N`; the first committed `N → N+1`; the loser's `UPDATE … WHERE row_version = N` matched zero rows and SQLAlchemy raised `StaleDataError` — the optimistic lock doing exactly its job, which is why **no duplicate was ever created** and the invariant held throughout. The gap: `app/models/agent.py` states this exception is "caught at the service layer and translated to `AGENT_CONCURRENT_MODIFICATION`"; the registry service honours that, `DiscoveryRunService.run_source` did not, so the losing sweep escaped as an unhandled 500 with its run row left dangling at `STARTED`. **Fix** (`app/discovery/service.py`): the reconcile-through-commit block catches `StaleDataError`, rolls back, and finishes the run as a truthful `FAILED` record naming `AGENT_CONCURRENT_MODIFICATION` and stating that observations were persisted, nothing was duplicated, and the other sweep's result stands. It follows this service's own convention for every other failure rather than raising, because discovery is the **fail-open plane**: a sweep must never surface a 5xx to the scheduler that drove it, and the run row is the durable, audited record. One subtlety handled: `observations_count` is assigned after the observations' own commit, so it is re-set after the rollback or the FAILED run would under-report what it genuinely persisted. **Proven deterministically** (`test_ac13`) by raising the real exception type from the reconciliation collaborator — the overlap itself cannot be forced without instrumenting product code — and the race proof additionally rejects any raised outcome so it catches a regression whenever the race genuinely overlaps. The full 5.2 suite passes unchanged with the fix in place.
- **A fixture lesson recorded rather than hidden.** A first draft of the scale proof inserted edges from phantom agent UUIDs and the traversal returned nothing. That was the graph **working**: every hop resolves its node against the tenant's real rows, which is precisely what stops a foreign or fabricated id extending a chain (Gate N). The fixture was corrected to insert real rows; the guard was not touched.
- **Failure semantics proven together, both planes.** Discovery fails **open** — a source outage degrades to staleness and the previously discovered agent is still there; an outage must never delete estate. Assurance fails to **`INSUFFICIENT_EVIDENCE`**, never a fabricated pass. Containment fails **closed and loud** — a mandatory containment that cannot complete is `REFUSED` with a reason, never a silent pass. M4.11 key continuity is composed rather than re-proven: its 14 existing proofs stand, and 5.10 adds that a grant secret is stored only as ciphertext, decrypts under the live key, and that durable M5 state is rows rather than cache.
- **No migration; head unchanged at `0061_assurance_evidence`; 152 tables; routes unchanged at 676.** No new permission, authz, scheduler, registry, or infrastructure. **17 new proofs** in `backend/tests/milestone/test_milestone_5_proof.py`; exactly one product file changed (`app/discovery/service.py`, the reported fix). Backend **2,639 passed**, 1 failed, 1 deselected — **2,640 collected** (2,623 + 17). The one failure is the known pre-existing posture-seed flake (`test_ac09_existing_agents_are_backfilled_native_and_governed`), which failed this phase's clean-tree baseline, the final run, **and a standalone re-run afterwards** — so it is reported as a live flake, not as "passes standalone". Query-verified root cause: 16 `agents` rows (of ~100k) are `origin_category='NATIVE'` yet `control_state='DISCOVERED'`, every one named `agent-<hex8>` by `tests/posture/test_security_posture.py::_insert_agent`, whose line-282 call passes `control_state="DISCOVERED"` on the helper's default `origin_category="NATIVE"` — an internally inconsistent fixture (a NATIVE agent cannot be DISCOVERED). Two of the 16 were created at 16:48 and 17:31, exactly this phase's two full runs; **none by 5.10** (its inserts take invariant-satisfying defaults, and its hostile-source proof asserts the created row is *not* NATIVE). 5.1's `test_ac09` samples `limit(500)` with no `ORDER BY`, so each full run raises the hit probability (~8% per attempt now). Left untouched per verify-and-leave; the recommended fix belongs to 5.5's fixture (pass `origin_category="EXTERNAL"` at line 282), not to 5.1's guard, **run isolated** with no mid-run tree or REPO_STATE edits (the 5.9 lesson, applied). Frontend **384 passed**, `npm run build` green.
- **Milestone 5 is COMPLETE.** ACT now discovers, inventories, maps, governs, secures and contains the heterogeneous AI-agent estate — including agents it did not create — truthfully, never claiming control it does not have. The approved next step is the post-M5 **Enterprise Validation Lab & Red-Team Gate**, a release-candidate validation against real infrastructure that this milestone was designed to make possible without architectural rework. No new milestone was begun.

See [docs/milestone-5/proof.md](docs/milestone-5/proof.md) and [docs/milestone-5/summary.md](docs/milestone-5/summary.md).

## [Phase 5.9 / M5.9] — Assurance, Evidence & Compliance

**Phase 0 — the frontend build was restored before any assurance work began.** `tsc -b` and `npm run build` had been failing on `main` since Phase 4.9: `TraceContentResponse` is exported from `observabilityService.ts` but was never re-exported by the `@/services` barrel that `TraceDetailPage` imports it from. `vitest` transpiles without typechecking, so a green 383-test suite masked a broken production build across four subsequent phases. Fixed with a single barrel re-export — the two accompanying `TS7006` implicit-any errors were downstream of the same unresolved type and cleared with it. A new `build-integrity.test.ts` shells out to the real `tsc -b` **inside the suite that was doing the masking**, and was verified to *fail* (naming the exact error) when the fix is reverted, so it is a genuine guard rather than a test that merely passes. The repository has no CI workflows at all, so a pipeline-only check would not have existed; when CI is introduced, `npm run build` belongs in it directly — documented as the recommended follow-up.

Then the phase itself: the evidence Phases M1–5.8 already produce, turned into control evaluations an auditor can use. It produces evidence and control mappings, and is built so that it cannot produce a compliance verdict.

- **Three results, and the boundary between the last two is the entire phase.** `PASS` / `FAIL` / `INSUFFICIENT_EVIDENCE`. FAIL means the evidence exists **and shows the control unmet**; INSUFFICIENT_EVIDENCE means ACT genuinely cannot determine either way. Both directions of confusion are real failures, and each control states which case it is: calling **absence a pass** is the false-green a compliance surface must never produce, while calling a **conclusive negative "insufficient"** hides a real finding behind a shrug. Concretely — an agent with no owner **FAILs** (the row *is* the evidence and it is conclusive), while an agent with no executions is **INSUFFICIENT_EVIDENCE** for traceability, because ACT cannot see what it does not run and calling that FAIL would invent a violation.
- **"Has anyone actually looked?" is asked before anything else.** Controls backed by Phase 5.5 check the `POSTURE_EVALUATED` **audit event** before reading `posture_findings` at all — zero open findings is indistinguishable from "posture was never evaluated", and treating the second as the first is exactly the failure this phase exists to prevent. The immutable audit trail is the evidence that an evaluation happened, and its timestamp is the as-of. Proven in both directions against real data: a fresh tenant reports 9 of 12 controls INSUFFICIENT_EVIDENCE, and running a real posture evaluation flips them to genuine PASS/FAIL results each carrying an `as_of`.
- **A stale pass is unrepresentable, not merely unwritten.** Evidence older than the freshness policy downgrades a would-be PASS to INSUFFICIENT_EVIDENCE with `stale=true`, and `CHECK (NOT (stale AND result = 'PASS'))` means the database itself refuses to hold one — belt and braces, because a false-green is the one artifact that must not exist even by accident. The operational consequence is deliberate: without the scheduled sweep, assurance decays toward "we cannot tell" rather than toward a stale green.
- **No verdict, enforced by absence rather than by intention.** There is no `compliant`, `status`, `score`, `coverage`, `verdict`, `grade` or `rating` anywhere — not a column, not a schema field, not a response key. A badge cannot be rendered from data that does not exist. A coverage percentage would additionally require a denominator ACT does not know (the full control set in the customer's chosen audit scope, most of which — personnel security, physical controls, vendor management, training — ACT cannot observe at all) and would compress INSUFFICIENT_EVIDENCE into the same number as PASS. Three counts, reported separately, instead.
- **Mappings are relevance claims, and they live in versioned code.** A mapping says "the evidence ACT's control X produces is *relevant to* NIST AI RMF GOVERN-1.1"; it never says "satisfying X means you comply with it". Twelve mappings ship across **NIST AI RMF 1.0**, **ISO/IEC 42001:2023** and **SOC 2 (2017, rev. 2022)**, each with a written rationale, and each framework carries a `scope_note` that travels **with the data** rather than living as a UI label someone can drop. They are **partial on purpose**: controls ACT holds no evidence for are absent rather than stubbed, because an absent mapping honestly means "no evidence here" while a stub evaluating to PASS would be a fabrication. Definitions live in `app/assurance/` rather than a table, the 5.5 rule-catalog precedent — a claim about what ACT's evidence means for a published control is logic to review in a diff, not data an operator can quietly edit.
- **Exceptions document a decision; they never flip a result.** A FAIL with an accepted-risk exception still reads FAIL, records who accepted it and why, is audited, and **survives re-evaluation** — a routine sweep must not silently discard a human decision. An exception that turned a result green would be indistinguishable from the control actually being met.
- **Evidence bundles reuse M4.11's signing, and say out loud when they are unsigned.** DSSE-signed over the canonicalized document using M4.11's own `pae` encoding, signing provider and key service rather than a reimplementation (verified signing end to end: `payloadType application/vnd.in-toto+json`, `keyid default:3`). The asymmetry with M4.11 is deliberate — M4.11 fails **closed** when a published agent version cannot be signed, because an unsigned version is an integrity hole in something ACT *executes*; a bundle is a read-only report, so it exports **unsigned and explicitly labelled unsigned** rather than not at all. What must never happen is an unsigned bundle presented as tamper-evident, which is why `signed` is returned explicitly and spelled out in words rather than inferred from a null signature. The row stores the digest and signature, never the payload — a second stored copy could drift from the rows it claims to summarize. `assurance.export` is a distinct, stronger permission never implied by view or manage: once a bundle leaves ACT, ACT's tenant isolation and audit no longer protect it.
- **The 5.8 Assurance placeholder became the real view**, under the same discipline: `INSUFFICIENT_EVIDENCE` gets its own tile and its own colour rather than being folded into a pass (a dashboard showing only passes and failures would quietly convert every unevaluated control into a green one), and the server's disclaimer renders verbatim. 5.8's "not built yet" time-capsule test was updated **intent-preservingly** to assert the invariant that placeholder was protecting, not deleted.
- **Head `0060_external_gov_bridge` → `0061_assurance_evidence`; two new tables** (`assurance_evaluations`, `assurance_evidence_bundles`), **152 tables**; routes **668 → 676** (+8, all under `/api/v1/assurance`). Three permissions (`assurance.view`/`.manage`/`.export`, group `audit`); 3 error codes, 3 audit events; `assurance.evaluate` registered as a scheduler handler (no new scheduler). **36 new backend tests** + **1 new frontend test**; one pre-existing frontend test updated intent-preservingly. Backend **2,623 passed**, 0 failed, 1 deselected (2,587 + 36); frontend **384 passed**, with `npm run build` green for the first time since 4.9.
- **One process note, reported rather than hidden:** this phase's own baseline run failed `test_ac01_repo_state_is_regenerated_at_the_true_post_3_10_head`, caused by copying the 5.9 migration into the tree *while the suite was running* — precisely the trap that guard exists to catch. Verified by stashing and re-running on a clean tree; the true baseline was 2,587 passed / 0 failed / 1 deselected.

See [docs/assurance/overview.md](docs/assurance/overview.md) and [ADR-0022](docs/architecture/adr/0022-assurance-evidence-not-verdict.md).

## [Phase 5.8 / M5.8] — Enterprise Agent Command Center

**The first Milestone 5 phase to touch the frontend**, carrying all the UI deferred since 5.1: the operator surface over everything 5.1–5.7 built. It reads and triggers; it decides nothing and enforces nothing.

- **Truthful affordances, made structural rather than promised.** The UI derives nothing about ACT's reach. `affordances.ts` contains **no enforcement-mode name, no control-state name, and no mode→label map** — asserted over the file's own source — and reads three fields the server computed with 5.7's `app.bridge.modes`: `reaches_agent_execution` (whether containment is offered at all), `reaches_boundary_calls`, and the `display`/`limits` sentences, rendered **verbatim**. A second structural test scans every file in the module and fails on any `control_state ===` or `enforcement_mode ===` comparison, because either would be a second opinion about what ACT can do — in the one surface a human acts from. Proven per state: an **OBSERVED/DISCOVERED** agent shows **no containment affordance**, a **GATEWAY_ENFORCED** agent is labelled **"ACT authorizes this agent's capability calls that route through ACT's gateway"** and never "governed", and a **NATIVE** agent shows full containment (confirmation-gated, dispatching to 5.6's real endpoint).
- **An absent affordance explains itself.** Containment is not rendered greyed-out and unexplained; it is absent, and the server's own `limits` sentence says why. A missing control with no reason reads as a broken feature — the explanation is what makes it read as the truth it is.
- **"You cannot see this" is never rendered as zero.** Every estate section is probed against its own domain permission through the real `AuthorizationGateway`, and a section the caller lacks returns `visible: false`, rendered as "hidden from you — this is not a count of zero". A `0` in a security dashboard reads as "all clear", which would be the most comfortable possible lie.
- **The rest of the truthful-state rules carried through.** Shadow shows **why** — 5.5's conditions (rule, severity, reason), never a bare badge, because shadow is a derived, disputable finding with no column anywhere. Unmeasurable cost is **counted and named**, never summed into a total that would look complete (5.7's `NOT_MEASURABLE`). `INSUFFICIENT_DATA` shows as itself. A **REFUSED** containment is surfaced *with* its refusal reason — an operator who believes an agent was contained when nothing happened is worse off than one who knows. A failed read shows the failure, never an empty state, because an empty list and a broken backend look identical and only one is safe to act on.
- **Reuses the 3.10/4.9 pattern verbatim.** `ConfirmActionDialog` and `useGuardedAction` are imported from `@/modules/operations`; the persona vocabulary from `@/modules/observability` — a second, subtly different persona list would have meant "Security" denoted one thing in Observability and another here. Asserted structurally: no second dialog and no second hook in the module. Thirteen views, each with the permission its data needs and the personas (§29) it serves.
- **Two read-only aggregation endpoints — and no migration, no table, no column.** `GET /api/v1/command-center/estate` and `GET /api/v1/command-center/agents` exist only because an estate count over tens of thousands of agents, and a per-row enforcement mode, cannot be assembled from a 500-row paginated list without doing arithmetic in the browser — precisely the narrow addition Phase 4.9 made for the same reason. Both are **GET-only** (no `post`/`put`/`patch`/`delete` anywhere in the package, asserted over the AST and the route table), tenant-scoped, and compute **no domain state**: the posture score comes from 5.5's `PostureSummaryService`, shadow from 5.5's `ShadowAgentService`, and the mode and reach from 5.7's `app.bridge.modes` — **including its `NATIVE_CONTROL_STATE` constant rather than a re-typed `"GOVERNED"` literal**, a drift a test caught mid-build and which was fixed by importing the constant. The **agent drilldown needed no endpoint at all**: it composes existing 5.1–5.7 endpoints with react-query, which is orchestration, not logic.
- **Assurance is deliberately empty and says so.** Compliance mapping is Phase 5.9; a placeholder framework grid — greyed-out SOC 2 rows, a "coming soon" score — would imply ACT produces evidence it does not yet produce, which is the same class of over-claim the rest of this center exists to avoid.
- **Head unchanged at `0060_external_gov_bridge`; 150 tables; routes 666 → 668** (+2). No new permission, no new authz, no new engine, no new design system. **13 new backend tests** + **24 new frontend tests** — the first frontend tests any M5 phase has added. Backend **2,587 passed**, 0 failed, 1 deselected (2,574 + 13); frontend **383 passed** (359 + 24).
- **A 5.7 defect of mine, fixed rather than left.** `test_ac14_concurrent_calls_under_a_revocation_race` failed this phase's baseline on a clean tree. Cause: `with TestClient(...)` inside three concurrent threads runs the app lifespan per thread, and the concurrent M4.11 key-material-canary inserts race its unique constraint — the platform-singleton-row trap. The context manager was never needed (the suite's own `client` fixture is bare). Harness corrected, assertions untouched, stable across three consecutive runs.
- **Pre-existing and left as-is:** `tsc -b` fails on Phase 4.9's `TraceDetailPage.tsx` (3 errors — `TraceContentResponse` is not exported from `@/services`), confirmed on clean `main` before any 5.8 code existed. `vitest` does not typecheck, so the 359-suite was always green and this never surfaced. Reported rather than fixed — it belongs to 4.9.

See [docs/command-center/overview.md](docs/command-center/overview.md).

## [Phase 5.7 / M5.7] — External Agent Governance Bridge

**The highest-risk phase in Milestone 5 — where ACT governs agents it does not run.** 5.6 could *observe* that an external agent's containment was truthfully absent. 5.7 makes the enforcement modes real, and bounds each one to exactly what it can deliver. The value of an external governance bridge is honesty about reach; a mode that over-claims would poison every dashboard, audit report and incident review downstream of it.

- **Four modes, each shipping its own limit.** `OBSERVED` sees (no enforcement). `ADVISORY` evaluates the real 4.3 policies and recommends (no enforcement). `GATEWAY_ENFORCED` authorizes/denies the capability calls an external agent routes **through ACT** — and reaches nothing else it does. `NATIVE_ENFORCED` is the M1–M4 platform (4.3 engine + kill switch). Every mode carries a `display` **and** a `limits` string, and both travel in every API response, so a client cannot render ACT's claim without its bound. The permitted sentence for GATEWAY is "ACT authorizes this agent's capability calls that route through ACT's gateway" — never "ACT governs this agent" (asserted per mode by `test_ac03_no_mode_claims_reach_act_lacks`).
- **`NATIVE_ENFORCED` is not a storable value.** A CHECK constraint on the new nullable `agents.external_enforcement_mode` admits only `OBSERVED`/`ADVISORY`/`GATEWAY_ENFORCED`; the strongest mode is **derived** from `control_state == 'GOVERNED'` — the same single signal Phase 5.6's truthful-containment gate reads. Full enforcement cannot be asserted by writing a column; it has to be *true*, and the two phases cannot drift apart. It also means **no backfill**: every pre-existing row gets a truthful mode the instant the column exists (native → `NATIVE_ENFORCED`; discovered/claimed/registered → `OBSERVED`).
- **The decision that mattered most: a GATEWAY_ENFORCED agent is NOT `GOVERNED`.** M5.1's own `control_state` comment anticipated external agents reaching GOVERNED "at NATIVE/GATEWAY enforcement, which is Phase 5.7". Building 5.7 showed the GATEWAY half would have been a **false claim** — `GOVERNED` is what 5.6 relies on to trip the kill switch, so promoting a gateway-mediated agent would have made `SUSPEND_AGENT` report success for an agent ACT cannot suspend: an over-claim manufactured inside the mechanism built to prevent one. So the agent stays `REGISTERED`, **5.6 is untouched and still refuses it**, and **no eighth containment action was added**. The superseded anticipations in ADR-0015, ADR-0020 and `docs/threat/truthful-containment.md` were corrected rather than left to mislead.
- **Federation rejected for external identity.** `FederationService._resolve_or_provision_user` → `_provision_user` → `_issue_session` mints a `User` row with roles and a session — exactly the internal principal this phase forbids. A bearer `AgentApiKey` was rejected too: a bearer token cannot be replay-protected, because the token is the request's only proof. Identity is a **signed request** — HMAC-SHA256 over method/path/timestamp/nonce/**digest of the exact body bytes** (never a re-serialization, which is where cross-language signing schemes break). The secret is stored only as Fernet ciphertext via M4.11 `credential_crypto` and returned exactly once. Replay is defeated by a `(grant_id, nonce)` unique constraint — the database, not a cache, so two concurrent replays cannot both commit — and the nonce is consumed **after** the HMAC verifies, so a garbage signature cannot burn a legitimate caller's nonces.
- **The boundary is a boundary, not a second authz.** `CapabilityBoundary` calls the real `AuthorizationGateway.authorize_agent`, resolves the real 4.3 `runtime_governance_policies`, and prices against the real 4.4 budgets — it holds no permission logic, no policy evaluation and no budget arithmetic of its own (AST-asserted). All three are proven *behaviourally*, not just structurally: a real published **ABAC DENY** policy stops a call; a real agent-scoped **`requires_approval`** policy denies it with `APPROVAL_REQUIRED`; a real **`HARD_LIMIT` budget** with no headroom denies it with `EXCEEDED`. A grant's scope only ever **narrows** what those authorities allow; it grants nothing.
- **Cost metered honestly rather than faked.** No reservation is taken: `ReservationService.reserve` is keyed to an `AgentExecution` a boundary call does not have, and fabricating one would corrupt the 4.4 ledger. ACT enforces the ceiling it can genuinely read and records `NOT_MEASURABLE` where no budget applies — rather than reporting a number it invented. Same reasoning for not invoking the 4.3 engine: it decides for an execution, so the boundary reads the same policy rows instead.
- **Commit-before-dispatch, proven for real this time.** Unlike 5.6 (whose authorities performed no I/O), the reference capability makes a genuine outbound HTTP call through M1's egress guard and `execute_http_tool`. No statement in `app/bridge` takes `FOR UPDATE`, and `dispatch.py` imports no session at all — both asserted **over the AST**, deliberately, because these modules *describe* the no-lock rule in their docstrings and a plain substring scan would have matched that prose and passed for the wrong reason. Proven behaviourally too: while a call is in flight, a **second real Postgres connection reads *and writes*** the already-committed gateway row.
- **Do-not-proxy-everything.** ACT is not in the path of an external agent's model calls or network traffic and does not pretend to be — a proxy-everything design is unscalable, lock-in-inducing, and could not be truthfully guaranteed, since an agent can always make a call that bypasses the proxy. **One** governed capability ships (`http_tool.invoke`); there is no catch-all route, no pass-through, and no HTTP client importable from the decision path (all asserted). A broad capability catalog and an external-agent SDK are **deferred** (§25A).
- **Fail semantics by §9 plane.** A governance decision fails **closed** — an unevaluable authorization/policy/cost decision denies, recorded with `fail_mode=FAIL_CLOSED` and `policy_outcome=UNEVALUABLE`. Telemetry ingest fails **open** — 202 even when every event drops. A downstream outage is recorded truthfully as *allowed + dispatch failed*, never faked and never retroactively rewritten into a denial (ACT did allow it). A DB CHECK (`ck_ext_calls_denied_never_dispatched`) makes "denied but dispatched" unrepresentable.
- **`external_grant.issue` is a distinct, stronger permission** — never implied by `external_governance.view`/`.manage`, the `containment.execute` precedent, because it is the one code that creates an outside party's ability to reach enterprise capability. Issuing a grant to an OBSERVED or ADVISORY agent is **refused**: it would be a credential for a boundary that, for that agent, enforces nothing. Dropping an agent below GATEWAY_ENFORCED revokes its live grants in the same transaction.
- **Three new tables, one new nullable column, no existing table changed, no data backfill.** Head `0059_threat_containment` → **`0060_external_gov_bridge`**; **150 tables**; routes **655 → 666** (+11, all under `/api/v1/bridge`). Three permissions (`external_governance.view`/`.manage`, `external_grant.issue`, group `runtime`); 17 error codes, 6 audit events. No new authz, no new scheduler, no second agent registry.
- **53 new backend tests** (`tests/bridge/test_external_governance_bridge.py` — AC-01..AC-20 + the §15 end-to-end proof). The E2E runs ACT under a **real uvicorn server on a real socket**, driven by a **real external agent in a separate OS process** whose own AST is asserted to import nothing from ACT — it establishes a scoped identity, calls a real enterprise capability through the boundary, is truthfully denied a call its grant forbids, and is refused entirely once the grant is revoked, while an OBSERVED agent's events ingest with no enforcement and an ADVISORY one gets a recommendation with no enforcement. **No pre-existing test changed.** Backend **2,574 passed**, 0 failed, 1 deselected (2,521 baseline + 53). Frontend **359 passed**, unchanged — not touched (all M5 UI deferred to Phase 5.8).
- **Pre-existing flake, verified and left as-is.** `test_agent_asset_model.py::test_ac09_existing_agents_are_backfilled_native_and_governed` fails in a full run and passes standalone — confirmed on a **clean tree before any 5.7 code existed**. Cause: `tests/posture/test_security_posture.py`'s `_insert_agent(..., control_state="DISCOVERED")` relies on the helper's default `origin_category="NATIVE"`, creating NATIVE+DISCOVERED rows that violate 5.1's invariant; the assertion samples `limit(500)` of 76k rows with **no `ORDER BY`**, so it fails only when one of the 7 such rows lands in the sample, and one more accumulates per full-suite run. A 5.5/5.1 test interaction on the shared dev DB, unrelated to this phase; left untouched per the verify-and-leave discipline.

See [docs/bridge/overview.md](docs/bridge/overview.md), [docs/bridge/enforcement-modes.md](docs/bridge/enforcement-modes.md), [docs/bridge/gateway.md](docs/bridge/gateway.md), [docs/bridge/external-identity.md](docs/bridge/external-identity.md), and [ADR-0021](docs/architecture/adr/0021-truthful-external-enforcement-modes.md).
...
- **TRUTHFUL containment — the defining honesty requirement.** `ContainmentOrchestrator.truthful_capability` is the one gate: capability derives from exactly one signal, `agents.control_state`. `GOVERNED` (ACT actually executes/enforces this agent) → every action routes to its real authority — proven with a real row: `lifecycle_status` genuinely flips to `SUSPENDED`, running executions genuinely `CANCELLED`. Anything else (`DISCOVERED`/`CLAIMED`/`REGISTERED`) → every enforcement-requiring action is **truthfully refused** (`status='REFUSED'`, a real `refusal_reason`, `authority_ref`/`result` both empty) — **never a fake success**.
- **Seven containment actions, six existing authorities, no eighth action, no new enforcer.** `TERMINATE_EXECUTION`/`SUSPEND_AGENT` → `KillSwitchService`; `DENY_TOOL` → `ToolRegistryService.revoke`; `REVOKE_CAPABILITY` → `CapabilityService.revoke`; `ISOLATE_CREDENTIAL` → `api_key_service.revoke_key`; `DISABLE_INTEGRATION` → `ConnectorService.disable`; `REQUIRE_APPROVAL` → `GovernancePolicyService.create` (a real, mandatory, agent-scoped policy the 4.3 engine's own checkpoints read). `app/threat` implements no enforcement of its own (AST-asserted: no parallel-enforcer class, no direct `UPDATE` of an enforcement column).
- **A real bug caught during the build, not after**: the planned revert for `DISABLE_INTEGRATION` called `ConnectorService.activate()` directly, but the real connector lifecycle has no `disabled → active` edge — only `disabled → configured → active`. Fixed to call `update_configuration` then `activate`, matching the actual state machine ([ADR-0020](docs/architecture/adr/0020-truthful-containment-via-existing-authorities.md)).
- **Kill-switch dominance is structural, not procedural.** `SUSPEND_AGENT`/`TERMINATE_EXECUTION` are non-reversible by construction; nothing in `app/threat` sets `lifecycle_status` back to `'ACTIVE'` or `cancel_requested` back to `False` (AST-asserted, the same proof `app.runtime.governance` gives for itself). Automated detection (`ThreatEvaluator`) never invokes a containment authority — a new HIGH/CRITICAL finding only ever creates a `RECOMMENDED` row (bounded automation, by construction). Proven under a real race: an operator-confirmed suspend survives a subsequent automated re-evaluation.
- **A dedicated `threat_findings` table**, not a discriminator on `posture_findings` — the same "new table, not a discriminator" call 4.5/4.7/5.5 each made, extended (ADR-0020): a threat is a runtime **event** ("this happened just now"), posture is a standing **state** ("this is how things stand"). The canonical example, delivered as `unapproved_mcp_tool_invoked`: an agent depending on an unapproved MCP server is posture (5.5); the agent actually *invoking* a tool that server exposes is a threat. The lifecycle *shape* is reused (`app.slo.states` imported, not re-spelled). `containment_actions` **records; it does not enforce** — every `EXECUTED` row's `authority_ref` names the real row a real authority produced.
- **Commit-before-dispatch holds by construction** — none of the six authorities perform network/model I/O, so there is no lock to hold across a dispatch call; both the AST proof (no `with_for_update`/`FOR UPDATE`) and a real-second-connection `FOR UPDATE NOWAIT` behavioral proof pass. **`containment.execute` is a distinct, stronger permission** — never implied by `threat.view`/`.manage`.
...
- **Two new tables, no existing table changed, no data backfill.** Head `0058_security_posture` → **`0059_threat_containment`**; **147 tables**; routes **643 → 655** (+12, all under `/api/v1/threat`). Three permissions (`threat.view`/`.manage`, `containment.execute`, group `runtime`); 6 error codes, 12 audit events; `threat.evaluate` registered scheduler handler (no new scheduler).
- **37 new backend tests** (`tests/threat/test_runtime_threat_containment.py` — AC-01..AC-18 + the §14 end-to-end proof, incl. real-Postgres concurrency for containment races + threat dedup, the truthful-containment proof across all three non-GOVERNED control states, and the kill-switch-dominance-survives-automation proof). **No pre-existing test changed.** One pre-existing test (`test_execution_tracing.py`'s `agent_id` filter case) failed in the full-suite run and passed standalone/unmodified — a pre-existing order-dependent flake unrelated to this phase, left as-is per the verify-and-leave discipline. Backend **2,521 passed**, 0 failed, 1 deselected (2,484 baseline + 37). Frontend **359 passed**, unchanged — not touched (all M5 UI deferred to Phase 5.8).

See [docs/threat/overview.md](docs/threat/overview.md), [docs/threat/rules.md](docs/threat/rules.md), [docs/threat/truthful-containment.md](docs/threat/truthful-containment.md), and [ADR-0020](docs/architecture/adr/0020-truthful-containment-via-existing-authorities.md).

## [Phase 5.5 / M5.5] — Security Posture & Shadow Findings

**Where the graph evidence becomes visible risk.** 5.1–5.4 built the canonical asset model, discovery, the authority graph and the dependency/blast-radius graph. 5.5 turns that evidence — plus credentials, governance policies and SLOs — into **deterministic, explainable posture findings**.

- **A deterministic posture-rule engine** over the 5.1–5.4 evidence. ~16 rules, each a **pure function of (asset state + graph evidence + policy)**: no accountable owner, unowned-with-production-access, discovered-outside-lifecycle, unmanaged external agent, production-activity-without-governance, missing runtime governance policy, missing SLO, expired/stale credential, dormant agent with active credential, unknown provenance, unapproved MCP dependency, dangerous dependency (blast-radius to a sensitive resource), prohibited model, unapproved tool, excessive tool scope. Code-defined + versioned (`POSTURE_RULESET_VERSION`).
- **No opaque score.** Every finding is a structured record — rule/control, evidence rows in 5.1–5.4, affected asset, severity, reason, first/last-seen, remediation, status, governing policy. `GET /posture/summary` returns a **deterministic weighted sum** of the open findings (`score = Σ severity_weight × open_finding_count`), carrying the formula, weights, `ruleset_version` and per-rule contributions — a CISO can recompute it by hand and, diffing two summaries, see exactly which findings moved it. No ML (`test_ac04` walks the AST and forbids the imports — the 4.5 proof).
- **Shadow is a derived finding-state, not a boolean.** There is **no `shadow` column** anywhere (`test_ac05` asserts it structurally). An agent is shadow **iff** it has an open finding whose `rule_id` is a shadow-class rule (`discovered_outside_lifecycle`, `unmanaged_external_agent`, `unowned_with_production_access`, `production_activity_without_governance`). `GET /posture/shadow-agents` is that query; each condition names itself, carries its evidence, is disputable, and clears when its finding resolves.
- **Findings are signals — 5.5 does not enforce.** `app/posture` names no `KillSwitchService`, no `RuntimeGovernanceEngine`, no execution-status mutation (`test_ac07`, the containment-by-absence proof 4.4/4.5 shipped). The 4.3 governance engine + kill switch remain the sole enforcers; Phase 5.6 wires *threat* findings to them.
- **Reuses the finding engine proved three times** (3.5 health → 4.5 behavioral → 4.7 SLO/alert). The 4.5 deterministic evaluation shape — `INSUFFICIENT_DATA` is first-class and recorded explicitly (`dangerous_dependency` with no dependency edges says "no dependency edges recorded; absence is not safety", never a false "healthy"). The 4.7 finding lifecycle — `app.slo.states` is **imported, not re-spelled**: OPEN → ACKNOWLEDGED → RESOLVED → SUPPRESSED, a partial unique index (`uq_posture_findings_active`) so one condition is one open finding, reopen-on-recurrence, suppression that is permission-gated + audited and is *not* resolution.
- **A dedicated `posture_findings` table**, not a `source='POSTURE'` discriminator on `runtime_alerts` — the same "new table, not a discriminator" call Phases 4.5/4.7 made ([ADR-0019](docs/architecture/adr/0019-posture-findings-and-derived-shadow.md)).
- **Idempotent + 3.8-schedulable.** `PostureEvaluator.evaluate_tenant` is the registered `posture.evaluate` scheduler handler — no new scheduler. A re-run over unchanged evidence opens nothing new; per rule, after it runs, any open finding it did not re-assert is auto-resolved. **Fails open** — a rule that raises is caught (`rule_errors` reported), fabricates nothing, blocks nothing.
- **Evidence-gap honesty.** "excessive privilege" (RBAC) and "excessive delegated authority" are **not** delivered as rules — an agent holds no RBAC role assignments, and 5.3's delegation edges are human↔human. The privilege-scope concern is covered by `excessive_tool_scope` + `unapproved_tool`; the gap is recorded (`docs/posture/rules.md`, ADR-0019) rather than fabricated.
- **Two new tables, no existing table changed, no data backfill.** Head `0057_dependency_graph` → **`0058_security_posture`**; **145 tables**; routes **630 → 643** (+13, all under `/api/v1/posture`). Two permissions (`posture.view`/`.manage`, group `runtime`); four error codes, seven audit events.
- **31 new backend tests** (`tests/posture/test_security_posture.py` — AC-01..AC-18 + the §14 end-to-end proof). **Two pre-existing tests updated, intent-preserving** — Phase 5.4's own `test_dependency_graph.py::test_ac01` migration-head guard → a subset check, and M5.1's `test_agent_asset_model.py::test_ac12_no_discovery_or_graph_machinery_shipped` (a "posture does not exist yet" time-capsule) → it now expects `app/posture/` + `posture_findings` as a legitimate sibling package while still enforcing the real invariant (no engine in `app/runtime/`, no parallel `agents` registry, **no `shadow` column on any table**). No existing test weakened. Backend **2,484 passed**, 0 failed, 1 deselected (2,453 baseline + 31). Frontend **359 passed**, unchanged — not touched (all M5 UI deferred to Phase 5.8).

See [docs/posture/overview.md](docs/posture/overview.md), [docs/posture/rules.md](docs/posture/rules.md), and [ADR-0019](docs/architecture/adr/0019-posture-findings-and-derived-shadow.md).

## [Phase 5.4 / M5.4] — MCP / Tool / Credential / Resource Dependency Graph

**Where the control graph becomes a security capability.** 5.3 built the relational edge substrate + authority-chain reconstruction; 5.4 extends it with **dependency edges** and delivers the **blast-radius queries** the milestone points at.

- **Dependency edges — more `edge_type` values on the one `control_graph_edges` table.** `AGENT → TOOL / CREDENTIAL / MCP_SERVER / CONNECTOR / RESOURCE`, `MCP_SERVER → TOOL`, `TOOL → CREDENTIAL / RESOURCE`, `CREDENTIAL → RESOURCE`. Each references two existing rows and **grants no authority** (the 5.3 rule, extended). No new edge table.
- **MCP is represented via the existing `Tool` domain — NOT a second tool registry** ([ADR-0018](docs/architecture/adr/0018-mcp-representation-via-tool-domain.md)). New `mcp_servers` table (identity, provenance, `trust_status`, version, endpoint ref) holds **no tools**; the tools an MCP server exposes are ordinary `tools` rows linked by **one additive nullable column, `tools.mcp_server_id`**. One tool registry, one M1 schema-validation path, one M1 gateway. MCP tool I/O stays contained by the existing egress/SSRF guard and tool-schema validation — `app/graph/` imports none of it.
- **Observed vs declared dependency is distinguished.** Each edge carries `evidence.mode` — `OBSERVED` (from `tool_calls`) vs `DECLARED` (from `agent_tools` / `tool_credentials` / `tools.mcp_server_id` / the version's model provider). Different evidence, never conflated. There is deliberately **no `agent→model` edge** — a model is a `{provider, model}` string, not a governed row (the `AGENT_DELEGATES_TO` restraint); the model-provider dependency surfaces as `DEPENDS_ON_CREDENTIAL` against the `provider_credentials` row.
- **Blast-radius queries reuse the 5.3 recursive-CTE machinery — no new traversal engine.** *which agents can reach resource X · what breaks if credential/MCP/tool Y is revoked · which agents depend on MCP server Z · which agents can reach a resource of kind K.* Every answer names its path (the exact edge chain + each edge's evidence), is **per-hop tenant-bounded** (re-proven adversarially for dependency edges — a planted cross-tenant edge does not extend any blast radius), is bounded/cycle-safe/depth-capped, and carries an explicit `incomplete` flag (never falsely "empty" — SRS §11).
- **MCP trust as evidence, not enforcement.** A server's `trust_status` other than `APPROVED` (`PENDING`/`UNKNOWN`/`REJECTED`) makes an agent's dependency on it a surfaced finding-evidence (`GET /graph/blast-radius/unapproved-mcp`). Unknown ≠ safe. The posture *finding* is Phase 5.5's job; 5.4 surfaces the evidence. `set_trust` is audited (`GRAPH_MCP_TRUST_CHANGED`, prev → new); two concurrent reviewers serialize on a row lock.
- **The §V graph-at-scale benchmark — NO materialised projection.** `test_ac08_v_blast_radius_benchmark_at_scale` builds a single-busy-tenant fixture (~12k agents, ~128k dependency edges), `ANALYZE`s it, and times the four blast-radius queries: **40–274 ms**. ADR-0017's "no projection" call holds, now backed by a real measurement at scale — exactly what ADR-0017 asked Phase 5.4 to produce. Numbers recorded in [ADR-0018](docs/architecture/adr/0018-mcp-representation-via-tool-domain.md) and [docs/graph/blast-radius.md](docs/graph/blast-radius.md).
- **Fails open.** A blast-radius / dependency query failure returns an error to its caller and can never block an execution or corrupt an edge (`app/graph/` is off every execution path — nothing under `app/runtime/` imports it).
- **One new table + one additive column, no existing column altered/dropped, no data backfill.** Head `0056_control_graph` → **`0057_dependency_graph`**; **143 tables** (142 in `Base.metadata`); routes **611 → 630** (+19, all under `/api/v1/graph`). MCP-trust management **reuses `graph.manage`** (no new permission). Three new error codes, six new audit events. The `control_graph_edges` `source_type`/`target_type`/`edge_type` CHECK constraints are **widened in place** (drop + recreate — no column altered).
- **34 new backend tests** (`tests/graph/test_dependency_graph.py` — AC-01..AC-18 + the §15 end-to-end proof + the §V scale benchmark). **One pre-existing test updated, intent-preserving** (`tests/graph/test_control_graph.py::test_ac01`'s migration-head guard → a subset check). **No existing test weakened, skipped or deleted.** Backend **2,453 passed**, 0 failed, 1 deselected (2,419 baseline + 34). Frontend **359 passed**, unchanged — not touched (all M5 UI deferred to Phase 5.8).

See [docs/graph/dependency-graph.md](docs/graph/dependency-graph.md), [docs/graph/mcp-via-tool.md](docs/graph/mcp-via-tool.md), [docs/graph/blast-radius.md](docs/graph/blast-radius.md), and [ADR-0018](docs/architecture/adr/0018-mcp-representation-via-tool-domain.md).

## [Phase 5.3 / M5.3] — Identity, Delegation & Trust Graph

**The pivotal Milestone 5 phase.** 5.1 built the canonical asset model; 5.2 built discovery; 5.3 builds the **relational control-graph substrate** — typed edges connecting humans, agents, identities, tools, credentials, resources — plus **authority-chain reconstruction** (who authorized this action, under whose delegated authority, through which identity, down to which resource) and the edge substrate 5.4 (dependency graph), 5.6 (containment attribution) and 5.7 (external identity) all build on.

- **NO graph database — the graph is relational.** One typed edge table (`control_graph_edges`) + `WITH RECURSIVE` CTEs (over that table and over `agent_executions.parent_execution_id`) + **no materialized projection** (a recorded measurement in `test_ac03` shows assembly is well within budget — the ADR-0008 discipline). No Neo4j, no second datastore. This phase proves the approved relational-graph decision holds. See [ADR-0017](docs/architecture/adr/0017-relational-control-graph-no-graph-database.md).
- **Per-hop tenant-bounded traversal — the sharpest isolation property.** Every recursion step re-applies `organization_id = :tenant`. Because every edge is validated same-tenant on *both* endpoints at creation, that predicate is a complete bound for service-created edges; for a hostile direct-inserted row the traversal additionally truncates at the first node that does not resolve in-tenant — it stops at the tenant edge. Proven adversarially: a planted cross-tenant edge does not extend a chain or a reachability result across the boundary (`test_ac06_*`).
- **The graph represents; it never creates.** A `TRUSTS` edge is created explicitly but only between two nodes that already exist in the caller's tenant, and it **grants nothing** — `AuthorizationGateway` stays authoritative. A `DELEGATES_TO` edge **mirrors** a `delegations` row that `DelegationService` owns, with `evidence` pointing back at it; the graph writes `delegations` never. `AGENT_DELEGATES_TO` is a declared edge type with **no producer** this phase — the runtime has no agent→agent invocation (`request_execution_as_agent` is self-only), so 5.3 does not invent it. The runtime-never-knows boundary is recorded in the ADR.
- **Authority-chain reconstruction generalizes the Phase 4.2 trace.** `origin → agent → (agent) → identity → tool → target`, assembled from existing execution/identity rows + edges: a recursive delegation prefix over `control_graph_edges`, a recursive replay prefix over `parent_execution_id`, and a bounded FK spine. Deterministic (same data ⇒ same chain); each hop names its evidence (the row or edge it came from). Missing evidence is **explicit** ("chain incomplete") — never rendered as "no delegation occurred".
- **Bounded / cycle-safe.** `depth < max_depth` (hard ceiling `MAX_TRAVERSAL_DEPTH = 32`; above it → `422 GRAPH_TRAVERSAL_DEPTH_EXCEEDED`); a `'type:id'` path array makes a revisit impossible, so a cycle in the data cannot run the query away.
- **Fails open.** The graph is a derived read plane, off every execution and governance path (structurally: nothing under `app/runtime/` imports `app/graph/`). A graph-query failure returns an error to its caller and cannot block an execution or mutate authority.
- **Concurrency (real separate Postgres sessions).** `uq_control_graph_edges_active` (partial unique index) makes edge create idempotent — a concurrent double-create resolves to exactly one live edge. A revoke locks the row `FOR UPDATE`; two concurrent revokers → one `200`, one `409`.
- **One new table, no existing table changed, no data backfill.** Head `0055_agent_discovery` → **`0056_control_graph`**; **142 tables** (141 in `Base.metadata`); routes **603 → 611** (+8, all under `/api/v1/graph`). Two permissions (`graph.view`/`.manage`), seven error codes, five audit events (edge create/revoke for trust + delegation, and authority-chain reconstruction — reading a chain is not a change and is not audited).
- **34 new backend tests** (`tests/graph/test_control_graph.py` — AC-01..AC-18 + the §14 end-to-end proof). **Two pre-existing tests updated, intent-preserving**: M5.2 `test_ac01`'s moving-target migration-head guard → a subset check; M5.1 `test_ac12_no_discovery_or_graph_machinery_shipped` now expects `app/graph/` + `control_graph_edges` as legitimate while still asserting no runtime-embedded graph engine and no parallel `agents` registry. **No existing test weakened, skipped or deleted.** Backend **2,419 passed**, 0 failed, 1 deselected (2,385 baseline + 34). Frontend **359 passed**, unchanged — the frontend was **not touched** (all M5 UI deferred to Phase 5.8).

See [docs/graph/control-graph.md](docs/graph/control-graph.md), [docs/graph/authority-chain.md](docs/graph/authority-chain.md), and [ADR-0017](docs/architecture/adr/0017-relational-control-graph-no-graph-database.md).

## [Phase 5.2 / M5.2] — Agent Discovery Framework

**The first Milestone 5 phase that reaches outside ACT.** Phase 5.1 laid the canonical asset model; this phase builds the vendor-neutral framework that populates it — adapters that observe agents in external systems, append-only observations (evidence, not truth), and reconciliation that *derives* canonical `agents` state from that evidence, plus one real, non-mocked reference adapter proving the whole path end to end.

- **The permanent M1 deadlock rule, extended to external I/O for the first time in this milestone.** No database lock or open transaction is held across `adapter.fetch()`. **Structural**: `DiscoveryAdapter.fetch()`'s signature accepts no `Session`/`db` parameter anywhere — an adapter has nothing to hold a lock with even if it tried (`inspect.signature`-asserted over every registered adapter). **Behavioral**: a real local HTTP server's response is held open mid-fetch while a *separate* Postgres session issues a real `SELECT ... FOR UPDATE` on the very `discovery_sources` row the sweep is using — it completes immediately, proving no lock is held (the exact M1 deadlock shape, proven absent).
- **Observations are append-only evidence, never truth.** `discovery_observations` has `UPDATE`/`DELETE` revoked from `PUBLIC` (mirroring migration `0047`'s `runtime_governance_decisions` precedent exactly) and is scrubbed of secrets (`app.observability.scrubbing.scrub`) before a row is ever built. No adapter, and no module outside `ReconciliationService`, ever writes `agents` directly.
- **Reconciliation is deterministic, explainable, and never silently merges or splits.** The sole identity signal is an exact `(organization_id, external_reference)` match; a fixed `0.75` confidence threshold; exactly three outcomes — CREATE a `DISCOVERED` agent through the Phase 5.1 `AgentProvenanceService` seam, LINK (discovery metadata only — ownership/control_state/lifecycle_status never touched), or FLAG a `discovery_findings` row for a human. A discovered identifier colliding with a NATIVE, ACT-governed agent is always a FLAG, never a link. A concurrent create race for the same external identifier resolves to exactly one agent (real separate Postgres sessions), never a duplicate.
- **Staleness is a finding, not a deletion.** An agent linked to a source but missing from enough consecutive sweeps (`DiscoverySource.missed_sweeps_before_stale`, default 1) gets an `OPEN` `STALE_AGENT` finding — `control_state` is never changed and the agent is never deleted; re-observation auto-resolves the finding. A partial unique index keeps re-raising idempotent under concurrent sweeps.
- **Discovery ≠ control**, unchanged from 5.1 and proven again here: a freshly discovered agent is `control_state=DISCOVERED`, and a direct client attempt to reach `GOVERNED` is rejected exactly as for any other unclaimed agent.
- **The reference adapter, distinguished from the deferred vendor catalog.** `HTTP_AGENT_REGISTRY` (`app/discovery/adapters/http_agent_registry.py`) is a generic, vendor-neutral paginated-HTTP-agent-list client, proven against a real local `http.server` (the same real-server convention Phase 2.2.1's REST-connector tests established) — not a vendor catalog. Azure AI Foundry / AWS Bedrock Agents / LangGraph / CrewAI / Kubernetes / MCP adapters are explicitly deferred; the adapter registry holds exactly one key today.
- **Sweeps run on the existing Phase 3.8 scheduler** — a new `discovery.sweep` handler, no new scheduler. Adapters use `GovernedHttpClient` exclusively (no ad-hoc HTTP, egress/SSRF/tenant containment inherited). Per-source credentials reuse the `ToolCredential` encrypted-storage pattern (`credential_crypto`). The manual-trigger API is `Idempotency-Key`-aware, reusing the Phase 3.1 claim-then-poll contract.
- **Four new tables, no existing table changed, no data backfill.** Head `0054_agent_asset_model` → **`0055_agent_discovery`**; **141 tables** (140 in `Base.metadata`); routes **593 → 603** (+10, all under `/api/v1/discovery`). Two permissions (`discovery.source.view`/`.manage`), seven error codes, eight audit events.
- **35 new backend tests** (`tests/discovery/test_discovery_framework.py` — AC-01..AC-21 + the §15 end-to-end proof); **no existing test weakened, skipped or deleted**. Backend **2,385 passed**, 0 failed, 1 deselected (2,350 baseline + 35). Frontend **359 passed**, unchanged — the frontend was **not touched** (all M5 UI is deferred to Phase 5.8).

See [docs/discovery/framework.md](docs/discovery/framework.md), [docs/discovery/reconciliation.md](docs/discovery/reconciliation.md), and [ADR-0016](docs/architecture/adr/0016-discovery-evidence-vs-canonical-truth.md).

## [Phase 5.1 / M5.1] — Universal Agent Asset Model + Ownership

**Milestone 5 (Universal Agent Control & Security Fabric) begins here — this is its foundation.** ACT's one canonical `agents` registry is extended *in place* so it can truthfully represent the whole spectrum — native, external, discovered, claimed, registered, governed, unknown — with provenance, a control-state dimension honest about ACT's real enforcement authority, reused ownership, and a safe, server-authoritative, audited claim workflow. No discovery, graph, MCP, posture, threat, containment, external-gateway or command-center machinery is built here — those are Phases 5.2–5.9. M5.1 only makes the model able to **say** the truth.

- **One registry, extended additively — no `/agents-v2`, no parallel table.** `agents` gains seven columns (migration `0054_agent_asset_model`): `control_state`, `origin_category`, `origin_provider`, and discovery placeholders `first_observed_at` / `last_observed_at` / `discovery_source_ref` / `discovery_confidence` (columns only — Phase 5.2 populates them). A dedicated test asserts no second canonical agent table exists.
- **`control_state` is a DISTINCT dimension from `lifecycle_status`.** `DISCOVERED → CLAIMED → REGISTERED → GOVERNED` answers *"what enforcement authority does ACT have?"* — not the operational lifecycle. The existing 13-state lifecycle machine (`_TRANSITIONS`) is byte-for-byte unchanged; every one of its ~27 consumers (deployment gating, runtime governance, kill switch, cost, SLOs) reads it identically. A native agent is `GOVERNED` (control) and may be in any `lifecycle_status`; both fields coexist on every row and are read independently (`test_ac03`).
- **Provenance without per-vendor schema churn.** `origin_category` (`NATIVE` / `EXTERNAL` / `UNKNOWN`) is `CHECK`-constrained; `origin_provider` is a **soft string**, not a DB enum — a new vendor (`MICROSOFT`, `LANGGRAPH`, `SOME_NEW_VENDOR_2027`, …) is a new value, never a migration. `origin_category` / `origin_provider` are **not** `registration_source` (`MANUAL` / `IMPORT` — *how the row was created*, orthogonal; a CSV-imported native agent is still `NATIVE`).
- **Server-authoritative — mass-assignment structurally impossible.** `control_state` and `origin_*` are absent from `AgentRegistrationCreate` and `AgentRegistryUpdate`. A client that puts `control_state: GOVERNED` in a `POST` or `PATCH` body is ignored — the field never reaches the row (`test_ac07`). `control_state` moves only through two dedicated endpoints (`POST /agents/{id}/claim`, `POST /agents/{id}/control-state`) that validate the transition against a fixed matrix; `DISCOVERED → GOVERNED` directly is rejected (`CONTROL_STATE_TRANSITION_INVALID`), and enrolling into `GOVERNED` requires an accountable owner (`CONTROL_STATE_TRANSITION_FORBIDDEN`).
- **The claim workflow.** `POST /agents/{id}/claim` — an authorized user (`runtime.agent.claim`) takes responsibility for a `DISCOVERED` agent: `→ CLAIMED` (**never** `GOVERNED` — claiming ≠ governance), sets the business owner, writes the existing `AgentOwnershipHistory` ledger, audited, tenant-scoped, `Idempotency-Key`-aware, and concurrency-safe (`SELECT … FOR UPDATE`; a second concurrent claim gets a deterministic `AGENT_CLAIM_CONFLICT`, proven with real separate Postgres sessions in `test_ac10`).
- **Ownership reuses everything.** The three existing owner columns, `AgentOwnershipHistory` (whose ledger already carries `SECURITY_OWNER` / `DATA_OWNER`), the org/user/team hierarchy — **no new ownership/people table**. The gap analysis found no genuine gap (business sponsor = `owner_id`, operating team = `team_id`, approving authority = `AgentOwnershipHistory.approved_by` + the lifecycle-event ledger).
- **Tenant isolation.** Every new field, endpoint and query is tenant-scoped through `get_or_404`: cross-tenant → **404** (no existence leak); in-tenant-unpermitted → **403** (`test_ac08`).
- **The external-agent forward-compat invariant** (`test_ac12`): the canonical model represents a real agent existing entirely outside ACT (`origin EXTERNAL/UNKNOWN`, `control_state DISCOVERED`, `lifecycle_status DRAFT` — no assumption ACT created or executes it, no governance affordance) — proven with **no discovery/adapter code**. This is what lets the post-M5 physical validation proceed without redesign.
- **Backfill.** Migration `0054_agent_asset_model` is additive, reversible, downgrade-tested. Every pre-existing agent row (58,044 live) is set idempotently to `control_state=GOVERNED, origin_category=NATIVE, origin_provider=ACT_NATIVE` — every existing agent *is* native and governed, so the backfill states a known truth, not a weak-signal inference. No existing-row behaviour change; the revision id is 22 chars (within the 32-char `alembic_version` ceiling).
- **No new table, +3 routes.** Head `0053_installation_bootstrap` → **`0054_agent_asset_model`**; still **137 tables** (136 in `Base.metadata`); routes **590 → 593**. Two new permissions (`runtime.agent.claim`, `runtime.agent.control.manage`), three new error codes (`AGENT_CLAIM_CONFLICT`, `CONTROL_STATE_TRANSITION_INVALID`, `CONTROL_STATE_TRANSITION_FORBIDDEN`), two new audit events (`RUNTIME_AGENT_CLAIMED`, `RUNTIME_AGENT_CONTROL_STATE_CHANGED`).
- **33 new backend tests** (`test_agent_asset_model.py` — AC-01..AC-17 + the §22 end-to-end proof); **no existing test weakened, skipped or deleted**. Backend **2,350 passed**, 0 failed, 1 deselected (2,317 baseline + 33). Frontend **359 passed**, unchanged — the frontend was **not touched** (all M5 UI is deferred to Phase 5.8).

See [docs/runtime/registry/asset-model.md](docs/runtime/registry/asset-model.md), the updated [domain-model.md](docs/runtime/registry/domain-model.md) / [api.md](docs/runtime/registry/api.md), and [ADR-0015](docs/architecture/adr/0015-universal-agent-asset-model-and-control-state.md).

## [Phase M4.11a] — Install-Mode Classification Hardening (Durable Bootstrap Marker)

**A corrective phase for M4.11.** M4.11 closed the silent-key-regeneration hazard for the common case, but its NEW-vs-EXISTING classification inferred "NEW installation" from the *absence of encrypted state* — which is unsafe. An established install can legitimately have organizations, agents, policies, deployments and audit history while holding **zero encrypted credential rows**; under the as-built logic, if it lost its key, `detect_install_mode` saw no ciphertext, classified it NEW, and let bootstrap silently mint a fresh cryptographic identity. That is the same silent-identity-loss M4.11 existed to kill, surviving in the one path the build didn't harden.

- **NEW is now a positive durable fact, not an inference from absence.** A single-row `installation_bootstrap` table (migration `0053`, additive/reversible/tenant-neutral) records **once** that this installation completed key bootstrap — a timestamp, the active key's non-secret fingerprint, the provider, a schema tag. No key, no secret.
- **Corrected `detect_install_mode`:** EXISTING iff **the marker is present OR any encrypted/signed state exists**; NEW iff **the marker is absent AND there is no such state**. Ciphertext / a signed attestation stays a *sufficient* fast-path — it is no longer the *only* signal. A missing key file never by itself implies NEW.
- **The edge case is closed:** an established install with domain state but zero encrypted rows, marker present, key lost ⇒ **fails loud (EXISTING), never bootstraps** — even with `ENCRYPTION_KEY_ALLOW_BOOTSTRAP` set. Proven both ways in `test_m411a_ac03` (the old `has_encrypted_state` signal would have said NEW; the marker says EXISTING).
- **The five-state key taxonomy** (`test_m411a_ac05`): `KEY_ABSENT`, `KEY_MALFORMED`, `KEY_PRESENT_BUT_WRONG`, `KEY_PROVIDER_UNAVAILABLE`, `INSTALLATION_NEVER_BOOTSTRAPPED` — each a deterministic, distinct, operator-actionable error code with no key/secret/ciphertext in the message. `KEY_PROVIDER_UNAVAILABLE` (a KMS/Vault outage, via a new `EncryptionKeyProvider.check_available()` seam) is explicitly **not** an install-mode signal — a provider outage never makes an established install look NEW and never falls back to generating a key.
- **The one-time backfill:** on startup, once an existing key is *verified* (canary match or trial-decrypt) and the marker is absent, it is written with `recorded_via="backfill"` — so an M4.11-bootstrapped install becomes correctly EXISTING with no window in which it reads as NEW. Idempotent; a wrong key never gets a marker (`test_m411a_ac09`).
- **Half-init guards:** a key present with no marker and nothing to verify it against ⇒ `INSTALLATION_BOOTSTRAP_INCOMPLETE`; established with a key but no canary and no ciphertext ⇒ `ENCRYPTION_KEY_UNVERIFIED_ESTABLISHED_INSTALL`. Both fail loud.
- **`bootstrap_key_material`** now records the marker atomically with the canary + signing identity, *adopts* an operator-supplied key instead of refusing it, and refuses any EXISTING install (`BOOTSTRAP_REFUSED_MARKER_PRESENT` / `BOOTSTRAP_REFUSED_EXISTING_INSTALL`).
- **Every M4.11 guarantee preserved** — fail-loud, backup/restore, continuity, canary, provider seam — and the M4.11 end-to-end proof (`test_ac18`) is **extended, not weakened**, with the zero-ciphertext-loses-key leg.
- **One additive table, no route.** Head `0052_key_material_canary` → **`0053_installation_bootstrap`**; **137 tables** (136 in `Base.metadata`); reversible, downgrade-tested; **changes no decrypt behaviour for any existing row**. The classification only *tightens* — nothing that was EXISTING becomes NEW.
- **14 new backend tests** (`test_m411a_*`); two existing M4.11 tests adjusted intent-preserving (`test_ac06` renamed error-code assertion; `test_ac18` extended, never weakened). Backend **2,317 passed**, 0 failed, 1 deselected (2,303 baseline + 14). Frontend **359 passed**, unchanged.

See [docs/security/key-management.md](docs/security/key-management.md), the amended [RECOVERY.md](RECOVERY.md), and the **M4.11a amendment** to [ADR-0014](docs/architecture/adr/0014-key-material-recovery-and-fail-loud-integrity.md).

## [Phase M4.11] — Production Integrity Closure (Key Material Recovery & Fail-Loud Integrity)

**A bounded integrity/recovery phase between Milestone 4 and Milestone 5.** It builds no new product capability. It closes the one verified production hazard that blocked M5: ACT's encryption and signing key material was in no backup, and **if a required key was absent at startup the platform silently generated a replacement** — after which new secrets encrypted and decrypted normally while every pre-existing ciphertext became permanently undecryptable, with nothing failing loudly. That silent data-loss is now a loud, deterministic, recoverable failure.

- **Fail loud, never regenerate.** `credential_crypto` and `LocalKeyProvider` no longer generate key material when a key is absent. `app.main`'s lifespan calls `verify_key_material(db)` before serving any request: if the database holds encrypted or signed state but the required key is missing or wrong, startup aborts with a deterministic `KeyMaterialError` — the message names the key, the expected location and the remediation, and **contains no key, secret or ciphertext**.
- **NEW vs EXISTING is the presence of encrypted state, not the absence of a file.** `detect_install_mode` probes every ciphertext-bearing table (`provider_credentials`, `tool_credentials`, `connector_credentials`, `connector_oauth_tokens`, `identity_federation_configs`) and the signed-attestation tables. Any row ⇒ EXISTING ⇒ a missing/wrong key fails loud. A restored database with rows and no key is therefore **always EXISTING and can never be bootstrapped** (the negative-proof anchor).
- **A wrong key fails too, not only an absent one.** New table `key_material_canary` (migration `0052`) stores `Fernet(key).encrypt(<public constant>)` keyed by the key's non-secret fingerprint; startup decrypts it and asserts the constant. On first upgrade (valid key, no canary yet) the canary is written only after a sample of real ciphertext independently decrypts.
- **Deliberate bootstrap for a genuine new install.** `python -m app.security.keys bootstrap` provisions the encryption key, the default signing identity and the canary — and **refuses to run if the database already holds encrypted or signed state**. Nothing is inferred from a missing file. `ENCRYPTION_KEY_ALLOW_BOOTSTRAP` (off by default) lets the startup check bootstrap only when there is zero encrypted state (container first-run).
- **The provider seam.** The Fernet key is obtained through an `EncryptionKeyProvider` (`app/security/encryption_provider.py`) mirroring the existing `SigningProvider`. `LocalEncryptionKeyProvider` is the recovery-safe default. A KMS/Vault adapter attaches at the registry (`ENCRYPTION_KEY_PROVIDER`) — documented as future work; **the core imports no vendor SDK** (asserted structurally).
- **Backup & recovery.** `python -m app.security.keys backup <dir>` writes the key files plus a `MANIFEST.json` that *names* the required artifacts with SHA-256 checksums and non-secret fingerprints — never their contents. `Export-ControlTowerSecrets.ps1` now includes `backend/.keys/` by default. Restore copies the files back (refusing to overwrite an existing key file), then `python -m app.security.keys verify` runs the continuity check — a half-restored state fails loud here, it does not partially serve.
- **Signing continuity.** Historical attestations stay verifiable (public keys in `signing_keys`/`signing_key_versions`, restored with the database); after an authorized restore the private key signs again under the same identity. M4.11 adds only the fail-loud check that the default identity's current private key is present and matches the recorded public key.
- **Rotation.** The key ring is a `MultiFernet` over the active key plus `MODEL_CREDENTIAL_ENCRYPTION_KEYS` (retained prior keys): new data uses the active key, old data still decrypts — no re-encrypt-everything transaction. A first-class background re-encrypt job is deferred with its architecture recorded (`docs/security/key-management.md`); nothing here makes it harder.
- **The end-to-end production-integrity proof** (`backend/tests/runtime/test_key_material_integrity.py::test_ac18`): bootstrap → store an encrypted credential → sign an artifact → back up → lose the environment → restore → the credential decrypts, the signature verifies, new signing works → then restart **without** the key ⇒ loud, no regeneration, does not serve → restart with the **wrong** key ⇒ deterministic safe failure → **no key or secret in any log, message or output** at any step.
- **One additive table, no route, no re-encrypt.** Head `0051_telemetry_privacy` → **`0052_key_material_canary`**; **136 tables** (135 in `Base.metadata`); reversible, downgrade-tested; **changes no decrypt behaviour for any existing row**. Routes stay **590** (the operator surface is a CLI). New settings: `ENCRYPTION_KEY_PROVIDER`, `MODEL_CREDENTIAL_ENCRYPTION_KEYS`, `KEY_MATERIAL_FAIL_LOUD`, `ENCRYPTION_KEY_ALLOW_BOOTSTRAP`.
- **23 new backend tests**; **one** existing test's fixture adapted, intent-preserving (`test_provider_credentials.py::_fresh_encryption_key` now provisions its own throwaway key file — the deliberate-bootstrap contract — instead of relying on the removed auto-generation). Backend **2,303 passed**, 0 failed, 1 deselected (2,280 + 23); frontend **359 passed**, unchanged (no frontend touched). See [docs/security/key-management.md](docs/security/key-management.md), the rewritten [RECOVERY.md](RECOVERY.md), and [ADR-0014](docs/architecture/adr/0014-key-material-recovery-and-fail-loud-integrity.md).

**This is the production-integrity prerequisite for Milestone 5 — now satisfied.**

## [Phase 4.10] — Enterprise Hardening & the Milestone 4 Proof

**Milestone 4's tenth and final sub-phase — a proof phase, not a feature phase. MILESTONE 4 IS COMPLETE (10/10).** It builds no product capability. It *demonstrates*, on one real governed execution, that runtime governance, cost truth, tracing, privacy and export all operate together — and re-proves tenant privacy, budget-race safety and telemetry-failure resilience at the milestone level. All fifteen §41 gates (A–O) are closed.

- **The §33 end-to-end proof** (`backend/tests/runtime/test_milestone_4_proof.py`, `test_ac01`–`test_ac05`). One `pytest` fixture configures a real execution — a deployed version, a real priced model, a tool, a governance cost policy, a `HARD_LIMIT` budget, a `REDACTED_CONTENT` capture policy, a planted secret — and runs it. Every assertion is a **real effect of the run, not a pre-inserted row** (`test_ac05` proves this structurally: the fixture only ever writes configuration). The execution: routes to the deployed version → a worker claims it (`execution_attempts` row, `started_at` set) → the first model call records real tokens and a real priced cost → the model requests a tool → governance evaluates mid-loop and permits it (the `tool_calls` row is the evidence — a denied call never reaches it) → a second model call begins → **as spend approaches the bound, the governance engine STOPs the loop with an explicit reason** (`GOVERNANCE_EXECUTION_STOPPED`, one `STOP` decision, `MIN_REMAINING_COST`, the mock model called exactly twice, `loop_iterations == 2`) → **the budget stays within bound** (reserved ≤ limit; spend 0.002 ≤ 0.0025) → **the trace reconstructs** (spans `execution`/`model_call`/`tool_call`, root status the real `FAILED`) → **sensitive fields are redacted** (the planted secret is absent from `trace_content` and from the OTLP wire bytes; the `prompt` field is masked) → **metrics reflect it** (`act_runtime_executions{status="FAILED"}`) → **the decision is audited** (`RUNTIME_EXECUTION_STOPPED`) → **the telemetry is OTLP-valid** (round-tripped through the encoder; every span a 16-byte trace id + 8-byte span id).
- **§34 tenant privacy** (`test_ac06`): two orgs with sensitive traces; org B can reach none of org A's metadata, content, or existence — including via 4.9's `/runtime/overview`, `/governance/decisions` and `/cost/summary` aggregation endpoints — and a known trace id returns 404, not content. Elevates the 4.2 / 4.8 phase-level checks into one milestone-level adversarial matrix.
- **§35 budget race** (`test_ac07`): twelve concurrent workers on **real separate `SessionLocal()` connections** race for a $1.00 budget at $0.25/execution — exactly four fit; `sum(reserved_amount) ≤ 1.00`, decided by `SELECT … FOR UPDATE` in the database. Composes 4.4's race proof; a companion test re-asserts `app/finops` imports no threading/multiprocessing.
- **§36 both plane directions**: telemetry **fails open** (`test_ac08` — a real execution reaches `SUCCEEDED` with a failing export sink; five cycles later the exporter is `degraded` with a visible error, the bounded buffer never exceeds its cap, and the execution row is byte-identical); governance **fails closed** (`test_ac09` — a mandatory constraint that raises at `BEFORE_FIRST_MODEL_CALL` STOPs the execution with `GOVERNANCE_CHECKPOINT_UNEVALUABLE` and `loop_iterations == 0` — it never proceeds ungoverned).
- **Hardening** (`test_ac10`–`test_ac11`): the trace explorer stays capped and `EXPLAIN`-verified free of a `Seq Scan on agent_executions` at 400 executions/tenant; `/metrics` never emits an unbounded label across 200 executions with 40 distinct error codes; a simulated restart leaves the governance policy, budget, capture policy and an `OPEN` alert all intact and effective.
- **The §41 gate-closure audit** (`test_ac12`): all fifteen gates A–O map to a named passing proof. The A–O letter↔concern mapping is reconstructed from the per-phase `Gate X` references plus the build prompt — the SRS §41 consolidated table is not carried in the repo (a reported SRS/repo observation). See [docs/runtime/milestone-4-proof.md](docs/runtime/milestone-4-proof.md).
- **The one finding, and it is not a defect.** The first end-to-end run of the §36 governance proof left the execution `QUEUED` rather than terminally `FAILED`. Reading `ExecutionWorkerService._fail_or_retry` confirmed this is **documented intentional behaviour**: `GOVERNANCE_CHECKPOINT_UNEVALUABLE` is deliberately *retryable* (a transient dependency may recover) — fail-closed is about the *attempt* (the loop never ran past the checkpoint), and the requeue is the retry policy. `test_ac09` asserts the honest property (`status ∈ {FAILED, QUEUED, BLOCKED, DEAD_LETTERED}`, never `SUCCEEDED`) rather than a weakened one. **No proof was weakened; no product code was changed.**
- **No new product capability, no migration, no new table, no new route, no new permission/error code/audit event.** Head stays `0051_telemetry_privacy`; routes stay **590**; schema stays **135 tables**. The one new file is `backend/tests/runtime/test_milestone_4_proof.py` (15 tests). No existing test changed.
- Backend **2,280 passed**, 0 failed, 1 deselected (2,265 + 15); frontend **359 passed**, unchanged (backend-only phase). **Gate O** — the entire M1/M2/M3/4.1–4.9 suite passes unchanged — is itself the proof.

### Milestone 4 — Enterprise Runtime Governance & Observability — is complete

Ten sub-phases: **4.1** trace/span context foundation · **4.2** trace assembly & explorer · **4.3** the runtime governance engine (one enforcement path, fail-closed) · **4.4** cost truth & budget reservations · **4.5** deterministic behavioral signals · **4.6** OpenTelemetry export & metrics (fail-open, bounded) · **4.7** SLOs & the alert lifecycle (signal, not notification) · **4.8** telemetry privacy, retention & access governance · **4.9** the observability center · **4.10** the end-to-end proof & hardening. The platform now **understands, governs, controls, investigates and financially manages what its agents do while they execute** — demonstrated end to end, not merely built.

## [Phase 4.9] — Enterprise Runtime Governance & Observability Center

**Milestone 4's ninth sub-phase: the operator control plane where all of Milestone 4 becomes visible.** Nine per-persona views over the 4.1–4.8 engines — runtime health, execution traces, cost and budgets, governance decisions, behavioural anomalies, SLOs, alerts, and telemetry policy. Read + trigger only; the server authorizes everything. Closes Gate M.

- **Nine operator views** (`frontend/src/modules/observability/`): Runtime Overview, Trace Explorer, Trace Detail, Cost Center, Governance Decisions, Behavior & Anomalies, SLO Dashboard, Alert Center, Telemetry Policy. Built on the 3.10 Release Operations Center pattern — same `PageHeader` / `Table` / `Card` primitives, the same `ConfirmActionDialog` / `useGuardedAction` reused verbatim from `@/modules/operations`. No new design system.
- **Per-persona assembly (§29), not one dashboard.** A persona lens (Platform Engineer / SRE / Security-CISO / Governance Officer / FinOps / Eng-Management / CIO-CTO) narrows the visible views to what a role needs; it never grants access — each view also carries the permission its data requires, the server re-authorizes every endpoint, and the selected persona is a `localStorage` convenience defaulting to "All views".
- **Content governance inherited from 4.8, in full.** The Trace Detail content pane is the one place content is rendered, and:
  - it shows content **only** to holders of `runtime.trace.content.view` — a metadata-only operator sees the timeline and a truthful gated message ("separate from and stronger than the metadata view"), never the content, never a misleading error, and the client does not even issue the request in that case;
  - it fetches content **exclusively** through 4.8's `GET /observability/traces/{trace_id}/content` — the audited endpoint that emits `RUNTIME_TRACE_CONTENT_VIEWED`. There is no bypass route; a test asserts exactly one `apiClient` call in the service ends in `/content` and it is that one. Content is fetched only on explicit click, never on page load;
  - capture mode is **truthful**: `METADATA_ONLY`/`DISABLED` → "no content was captured — the policy working, not an error"; `REDACTED_CONTENT` → masked items flagged `redacted`; `FULL_CONTENT` → scrubbed items flagged `secret scrubbed`;
  - **404 vs 403** honoured: `404` → "not found for your organization" (cross-tenant indistinguishable from missing); `403`/`TRACE_CONTENT_ACCESS_DENIED` → "your account does not hold `runtime.trace.content.view`" — a distinct state.
- **Read + trigger; every action is an existing 4.x operation.** Alert ack/resolve/suppress hit 4.7's endpoints; capture-policy edits and the retention run hit 4.8's. The server authorizes, enforces tenant isolation, honours `Idempotency-Key`, and writes its own audit. Nothing in this phase decides a governance/cost/privacy question.
- **Dangerous actions confirmation-gated** (§22): **suppress an alert** requires a reason (audited); **raising capture to `REDACTED_CONTENT`/`FULL_CONTENT`** is type-to-confirm with warnings about persisting prompts and content becoming readable; **running the retention sweep** is a deliberate confirm. Setting `METADATA_ONLY`/`DISABLED` is a plain action.
- **Truthful state** (§10, §28): a burned error budget reads "budget spent" in destructive red; `INSUFFICIENT_DATA` (SLO or behavioural) is its own warning state, never "met"/"normal"; a degraded exporter raises a banner; a `DISABLED` capture mode is shown as such. Nothing computes state client-side. A concurrency conflict surfaces ("someone else changed this…") and refreshes, never a silent retry.
- **Two new read-only backend aggregation endpoints** (`app/runtime/observability_center.py`, read-only by construction — an AST test fails on any `add`/`commit`/`delete`/`flush`):
  - `GET /api/v1/runtime/overview` (`runtime.telemetry.view`) — the fleet composite in one request; `success_rate` is `null` (not 0) below the 20-sample floor. Exporter health is deliberately **not** in it — `app/runtime` never imports the OpenTelemetry export plane (a 4.6 test enforces this); the screen fetches it from `GET /observability/export/health` directly.
  - `GET /api/v1/runtime/governance/decisions` (`runtime.execution.view`) — a tenant-wide, filterable governance-decision list; 4.3 only exposed the lineage per execution. `reason` is a platform-templated sentence, never content.
- **No migration, no new table** — this phase reads existing data. Head stays `0051_telemetry_privacy`. Routes 588 → **590** (+2); no new permission (`runtime.telemetry.view` / `runtime.cost.view` / `runtime.execution.view` / `runtime.telemetry_policy.*` / `runtime.alert.manage` / `runtime.trace.content.view` all already existed); no new error code; no new audit event (actions reuse the endpoints' own).
- 32 new frontend tests (`src/modules/observability/tests/observability.test.tsx`) — including the content-governance set — and 8 new backend tests (`tests/runtime/test_observability_center.py`). No existing test changed. Frontend **359 passed** (327 + 32); backend **2,265 passed**, 0 failed, 1 deselected (2,257 + 8). See [docs/operations/observability-center.md](docs/operations/observability-center.md).

## [Phase 4.8] — Telemetry Privacy, Retention & Access Governance

**Milestone 4's eighth sub-phase, and the last policy-heavy one: it turns the telemetry plane into a governed, security-sensitive data system.** What gets captured is now a deliberate per-tenant/env/agent/classification policy defaulting conservatively; content is scrubbed of secrets and redacted before it is ever persisted; chain-of-thought never is; and reading trace content is a distinct, stronger, audited permission that executing or seeing metadata does not grant. Closes Gate F (privacy).

- **Capture policy, four modes.** `METADATA_ONLY` / `REDACTED_CONTENT` / `FULL_CONTENT` / `DISABLED`, resolved per **tenant / environment / agent / data-classification**. Precedence is `classification > agent > environment > tenant > platform-default` — deterministic, and `GET /api/v1/runtime/telemetry/effective-mode` returns the winning policy, the scope it matched, and every candidate considered. New package `app/telemetry_privacy/` (7 modules).
- **Conservative default is the safety spine.** A production environment (or one whose policy declares a sensitive classification), with no explicit policy row, resolves to `METADATA_ONLY` — `source: "conservative-default"` — **never `FULL_CONTENT`**. A malformed stored mode coerces toward `METADATA_ONLY`. Content capture is opt-in everywhere; a misconfiguration fails toward *less* capture, not more.
- **Content-storage decision: a dedicated governed store, not govern-in-place.** New table `trace_content` holds the telemetry copy, materialised **on the first authorised content view and never before**. The domain rows it derives from (`execution_messages`, `agent_executions.input_payload`/`output_payload`, `tool_calls.*_summary`) are domain truth the tool loop and execution-detail depend on — redacting them in place would corrupt execution state and break "retention deletes telemetry, never domain truth" (M4-4.8-FR-032). This is the "`trace_content` table with a redaction/classification column and its own retention" §5 explicitly permits.
- **Scrub before persist, in every content mode.** `redact_for_capture` runs, in fixed and non-interchangeable order: `strip_reasoning` (§7) → `scrub` (§14, the 4.1 secret scrubber) → classification field-masking (`REDACTED_CONTENT` only). `FULL_CONTENT` keeps business content but the scrubber still runs — a planted `sk-live-…` key and a `Bearer` header never reach `trace_content` in **any** mode. An AST test asserts `strip_reasoning` is the first call in the pipeline regardless of mode.
- **No chain-of-thought in any mode (§7, structural).** The reasoning-strip is a property of the pipeline, not a mode toggle. A planted `thinking:` block is gone from `trace_content` under both `REDACTED_CONTENT` and `FULL_CONTENT`.
- **`DISABLED` / `METADATA_ONLY` capture no content.** The content endpoint returns `captured: false` with an explanatory note and materialises **zero** `trace_content` rows — asserted. The exact `DISABLED` boundary is stated in [privacy.md](docs/observability/privacy.md): no `trace_content` row and no new content-bearing telemetry record is ever created for the scope; 4.1's derived metadata, being computed on demand from domain rows, is not retroactively erasable.
- **Content view is a distinct, strictly stronger, audited permission.** `runtime.trace.content.view` is **registered here for the first time** — 4.1 and 4.2 named it in code without registering it so the boundary had a visible owner; 4.8 is that owner. It is **not** in the read-only bundle, **not** implied by execute or by `runtime.telemetry.view`. `GET /api/v1/observability/traces/{trace_id}/content` depends on the metadata view, resolves the trace, then checks the content permission imperatively. Every successful view emits `RUNTIME_TRACE_CONTENT_VIEWED` with actor + resource ids and **never the payload**.
- **404-vs-403 discipline.** A trace absent for the tenant → `TRACE_NOT_FOUND` (404, cross-tenant existence never leaked). A trace present for the tenant but caller lacking the content permission → `TRACE_CONTENT_ACCESS_DENIED` (403), distinct from the generic `PERMISSION_DENIED`. The trace is resolved before the content permission is checked, so the two stay distinct.
- **Retention per class + a safe, idempotent, bounded sweep.** Six classes: `trace_content` / `trace_metadata` / `metrics_aggregate` / `alert_history` are deletable (→ `trace_content` / `runtime_events` / `slo_evaluations` / terminal `runtime_alerts`); `governance_decision` / `financial_record` are **retain-only** — configurable minimum with a one-year floor, never deleted (evidence outlives payloads, §24). `RetentionSweeper.run` deletes in committed 1,000-row batches, capped at 50 batches/class/run; a second run continues where the first stopped. `POST /api/v1/runtime/telemetry/retention/run` is `Idempotency-Key`-aware, the 4.5/4.7/3.5 interim pattern — **no scheduler built**.
- **Retention spares domain truth.** A purge removes the expired `trace_content` rows while the `agent_executions` row and its `execution_messages` transcript survive — asserted directly (AC-08).
- **Non-gating; planes separate.** `app/telemetry_privacy` references no kill switch, no governance engine, no execution-state mutation (AST-asserted); the retention sweep names only telemetry tables in a `delete()`. A capture or retention operation never stops or alters an execution.
- **Migration `0051_telemetry_privacy`**: three new tables (`telemetry_capture_policies`, `telemetry_retention_policies`, `trace_content`), no change to any existing table, no backfill, reversible (downgrade/upgrade verified). `uq_telemetry_capture_policies_scope` is a COALESCE-based unique index so two "platform default" rows cannot coexist; `uq_trace_content_source` makes materialisation idempotent; `ix_trace_content_created` is the expiration-scan index.
- Routes 578 → **588** (+10); schema 132 → **135 tables** (3 added); three new permissions (`runtime.trace.content.view`, `runtime.telemetry_policy.view`, `runtime.telemetry_policy.manage`); four new error codes (`TELEMETRY_POLICY_INVALID` 422, `TELEMETRY_POLICY_NOT_FOUND` 404, `RETENTION_POLICY_INVALID` 422, `TRACE_CONTENT_ACCESS_DENIED` 403); four new audit events (`RUNTIME_TELEMETRY_POLICY_CHANGED`, `RUNTIME_TRACE_CONTENT_VIEWED`, `RUNTIME_TRACE_EXPORTED`, `RUNTIME_TELEMETRY_RETENTION_RUN`). Head `0050` → **`0051_telemetry_privacy`**.
- Recorded in [ADR-0013](docs/architecture/adr/0013-trace-content-capture-and-access-policy.md). 36 new backend tests (`tests/runtime/test_telemetry_privacy.py`), including the mandatory §31 privacy set (restricted content, secret scrubbing, cross-tenant 404, metadata-only, telemetry-disabled). Two existing tests updated, both intent-preserving: `test_ac04_the_content_permission_is_not_registered` → `_is_owned_by_phase_4_8`; `test_ac13_the_observability_prefix_does_not_collide` (the new content route added to the expected set, as 4.6 added export). Backend **2,257 passed**, 0 failed, 1 deselected (2,221 + 36); frontend **327** green, unchanged — a Telemetry Policy admin view assembles in 4.9. See [docs/observability/privacy.md](docs/observability/privacy.md) and [docs/observability/retention.md](docs/observability/retention.md).

## [Phase 4.7] — SLOs, Alert Rules & Incident Signals

**Milestone 4's seventh sub-phase, and a discipline-of-restraint phase: the line is one sentence — build the signal, not the notification platform.** An enterprise can now express runtime reliability as SLOs and every breach becomes a first-class, durable, auditable alert with a real lifecycle. Nothing pages anyone.

- **No notification delivery was built, and the restraint is enforceable.** `app/slo` imports no `requests` / `httpx` / `smtplib` / `slack` / `pagerduty` / `boto3` / email service, and defines no method named `send` / `notify` / `deliver` / `page` — two AST tests fail the build on any of them. Alert *creation* and external *notification* are separate concepts (§4.7); this phase owns creation. The alert **record** is the product; a future integration (4.9's center, a Slack app, a PagerDuty bridge) consumes it. Recorded in [ADR-0012](docs/architecture/adr/0012-alerts-as-signal-creation-not-notification.md).
- **SLOs**: an SLI, a target, an observation window (`1h`/`6h`/`24h`/`7d`/`30d` — a closed vocabulary), and an error budget. Six SLIs, each with an exact predicate over `agent_executions` / `tool_calls`: `success_rate`, `timeout_rate`, `provider_error_rate`, `tool_failure_rate`, `latency_p95`, `queue_delay`. **The objective direction is a fixed property of the SLI, not a stored column** — a self-contradictory "success rate below 0.99 is good" cannot be stored.
- **SLI sources**: the domain rows directly, not the 4.6 `/metrics` surface. 4.6's metrics are windowed gauges rounded to bounded-cardinality dimensions (right for a Prometheus dashboard); an SLO needs an exact count over its own window and a precise percentile. The two share a *source of truth*, not a query. The `TERMINAL` status set is asserted byte-identical to 3.5's and 4.5's.
- **Deterministic, explainable evaluation reusing the 3.5 / 4.5 shape**: veto → sufficiency → objective → budget. `INSUFFICIENT_DATA` is first-class (below 20 terminal samples, a platform constant like 3.5's/4.5's — neither MET nor BREACHED). The shared state values (`INSUFFICIENT_DATA`, `UNKNOWN`) are spelled character-for-character as 3.5 and 4.5 spell them, asserted against the live modules. A full evaluation run three times is byte-identical; no `random`/`numpy`/ML importable in the package.
- **Error-budget consumption is tracked and observable**: `budget_consumed = bad_fraction / error_budget`; above 1.0 the window's budget is spent and `budget_remaining` is 0. Defaulted on create (`1 − target` for success rate; `target` for the "lower is better" rates; `0.05` for the latency SLIs), overridable.
- **One alert lifecycle over two evidence sources (the §18 model decision)**: `runtime_alerts` is a *distinct* table that references `slo_evaluations` / `behavioral_findings` by `(source, source_id)` — **not** a shared `runtime_findings` re-homing 4.5's table. 4.5's `behavioral_findings` is untouched and gained no alert columns. The 4.5 model docstring already anticipated this ("consumed by an operator (and, in Phase 4.7, by an alert lifecycle)").
- **Escalation is explicit and threshold-defined, never automatic (§18)**: an SLO evaluation becomes an alert only when `BREACHED`; a behavioral finding only when `ANOMALOUS` — **a `DEGRADED` finding stays a finding** (a named constant, `_SIGNIFICANT_FINDING_STATE`, not a scattered `if`). There is no "incident" concept — the model is a first-class finding/alert with a lifecycle and nothing more.
- **Lifecycle OPEN → ACKNOWLEDGED → RESOLVED → SUPPRESSED**, every transition audited (`RUNTIME_ALERT_CREATED` / `_ACKNOWLEDGED` / `_RESOLVED` / `_SUPPRESSED`) with the actor — or no actor and `meta.auto` when a later evaluation reports the objective `MET` and the alert auto-resolves. An illegal transition is `ALERT_TRANSITION_INVALID` (409); an identical transition converges silently (two operators acking the same alert both succeed).
- **One ongoing condition is one active alert, DB-enforced**: `uq_runtime_alerts_active_dedup`, a **partial** unique index over `(organization_id, dedup_key) WHERE status IN ('OPEN','ACKNOWLEDGED')` — the same primitive 3.7 used for `uq_rollback_events_dedup`. A `RESOLVED` alert **re-opens** on recurrence (same row, `recurrence_count` increments); a `SUPPRESSED` one does not. Proven under a real race: eight threads on eight Postgres connections against one breach produce one evaluation row and one alert.
- **Signal, not enforcement**: an SLO breach / alert never stops or alters an execution — asserted structurally (`app/slo` references no kill switch, no governance engine, no execution-state mutation; `app/runtime` and `app/workers` import nothing from `app.slo`) and behaviourally (evaluating a wildly breached SLO leaves every execution row byte-identical). Non-gating: a failure produces no evaluation and no alert.
- **The interim idempotent evaluate op** (`POST /api/v1/runtime/slos/evaluate`, `Idempotency-Key`-aware) for Phase 3.8's scheduler to adopt — the 4.5/3.7/3.5 pattern. `uq_slo_evaluations_window` on `(slo_id, window_start, window_end)` makes a re-run over the same window a no-op. **No scheduler was built.**
- **Migration `0050_slos_and_alerts`**: three new tables (`slo_definitions`, append-only `slo_evaluations`, `runtime_alerts`), no change to any existing table, no backfill, reversible (downgrade/upgrade verified). **No index added to `agent_executions` / `tool_calls`** — SLI aggregation reuses the exact tenant-leading shape 3.5/4.5 already run.
- Routes 566 → **578**; schema 129 → **132 tables**; two new permissions (`runtime.slo.manage`, `runtime.alert.manage` — reads reuse `runtime.telemetry.view`); four new error codes (`SLO_DEFINITION_INVALID` 422, `SLO_NOT_FOUND` 404, `ALERT_NOT_FOUND` 404, `ALERT_TRANSITION_INVALID` 409); five new audit events. Head `0049` → **`0050_slos_and_alerts`**.
- 41 new backend tests (`tests/runtime/test_slos_and_alerts.py`). Backend **2,221 passed**, 0 failed, 1 deselected (2,180 + 41); frontend **327** green, unchanged — the SLO Dashboard + Alert Center assemble in 4.9. See [docs/operations/slos.md](docs/operations/slos.md) and [docs/operations/alerts.md](docs/operations/alerts.md).

## [Phase 4.6] — OpenTelemetry & Metrics Interoperability
