...
| | |
|---|---|
| Backend tests | **2,350 passed**, 0 failed, 1 deselected |
| Frontend tests | **359 passed** |
| Live schema | **137 tables**, migration head `0054_agent_asset_model` |
| HTTP routes | **593** |
...
| **Milestone 5** — Universal Agent Control & Security Fabric | **In progress (1/9)** | **5.1**: the foundation. ACT's one canonical `agents` registry is extended *in place* (never a second registry) so it can truthfully describe native, external, discovered, claimed, registered, governed and unknown agents — seven additive columns (`control_state`, `origin_category`, `origin_provider`, and four discovery placeholders 5.2 will populate). **`control_state` (`DISCOVERED → CLAIMED → REGISTERED → GOVERNED`) is a distinct dimension from the operational `lifecycle_status`** — the 13-state lifecycle machine and its ~27 consumers are unchanged. It is server-authoritative (absent from every write schema — a client cannot mass-assign it), moved only through a `FOR UPDATE`-locked, audited, tenant-scoped `claim` (→ `CLAIMED`, never `GOVERNED` — claiming ≠ governance) and a `control-state` transition endpoint that validates a fixed matrix and requires an owner to reach `GOVERNED`. Provenance uses a soft `origin_provider` string, not a per-vendor enum. Ownership reuses the existing owner columns + `AgentOwnershipHistory` + org hierarchy — no new people/ownership table. Every pre-existing agent is backfilled to native + governed (a known truth). No discovery, graph, MCP, posture, threat, gateway or UI machinery — those are 5.2–5.9. No new table; +3 routes. |
