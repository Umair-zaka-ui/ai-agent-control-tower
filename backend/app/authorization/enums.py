...
class AuthorizationAuditEvent(str, enum.Enum):
    ...
    # Runtime Threat Detection & Containment (Phase 5.6 / M5.6). A threat
    # finding's lifecycle mirrors posture's; every containment action -- and
    # every state it moves through (recommended/queued/refused/executed/
    # failed/reverted) -- is audited, naming the authority (§9). A REFUSED
    # containment is audited exactly like an executed one: truthful refusal
    # is not a non-event.
    THREAT_FINDING_OPENED = "THREAT_FINDING_OPENED"
    THREAT_FINDING_REOPENED = "THREAT_FINDING_REOPENED"
    THREAT_FINDING_ACKNOWLEDGED = "THREAT_FINDING_ACKNOWLEDGED"
    THREAT_FINDING_RESOLVED = "THREAT_FINDING_RESOLVED"
    THREAT_FINDING_SUPPRESSED = "THREAT_FINDING_SUPPRESSED"
    THREAT_EVALUATED = "THREAT_EVALUATED"
    CONTAINMENT_ACTION_RECOMMENDED = "CONTAINMENT_ACTION_RECOMMENDED"
    CONTAINMENT_ACTION_QUEUED = "CONTAINMENT_ACTION_QUEUED"
    CONTAINMENT_ACTION_REFUSED = "CONTAINMENT_ACTION_REFUSED"
