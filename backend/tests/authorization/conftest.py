"""Fixtures for the Phase 4.3.1 authorization tests."""
...
def _seed_authorization_once() -> None:
    """The built-in role taxonomy + groups are global; ensure they exist so the
    endpoint tests can see system roles regardless of DB state."""
    db = SessionLocal()
    try:
...
def admin(client: TestClient) -> dict:
    ...
    reg = client.post(
        "/auth/register",
        json={"organization_name": "Authz Org", "name": "Owner", "email": email, "password": PASSWORD},
    )
