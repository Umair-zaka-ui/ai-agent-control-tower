"""Fixtures for the Phase 5.5 (M5.5) Security Posture & Shadow Findings tests.

Mirrors ``tests/graph/conftest.py`` -- a new top-level domain gets its own
...
def register_org(client: TestClient, org_name: str) -> dict:
    email = f"posture_{uuid.uuid4().hex[:10]}@example.com"
    reg = client.post(
        "/auth/register",
...
def admin(client: TestClient) -> dict:
    return register_org(client, "Posture Org")


@pytest.fixture()
def other_org_admin(client: TestClient) -> dict:
    return register_org(client, "Other Posture Org")
