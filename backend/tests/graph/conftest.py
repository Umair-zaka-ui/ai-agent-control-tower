"""Fixtures for the Phase 5.3 (M5.3) Identity, Delegation & Trust Graph tests.

Mirrors ``tests/discovery/conftest.py`` -- a new top-level domain gets its own
test directory and its own minimal ``client`` / ``admin`` / ``db_session``
...
def register_org(client: TestClient, org_name: str) -> dict:
    email = f"graph_{uuid.uuid4().hex[:10]}@example.com"
    reg = client.post(
        "/auth/register",
...
def admin(client: TestClient) -> dict:
    return register_org(client, "Graph Org")


@pytest.fixture()
def other_org_admin(client: TestClient) -> dict:
    return register_org(client, "Other Graph Org")


@pytest.fixture()
def viewer(client: TestClient, admin: dict) -> dict:
    """A VIEWER-role member of ``admin``'s org -- VIEWER holds neither
    ``graph.view`` nor ``graph.manage`` (mirrors the discovery precedent), so
    this exercises "authenticated but not permitted" (403)."""
    email = f"graphv_{uuid.uuid4().hex[:10]}@example.com"
    r = client.post(
