"""Fixtures for the Phase 2.1.1 connector-core tests.

Mirrors ``tests/authorization/conftest.py`` and ``tests/runtime/conftest.py``
exactly — a new top-level domain gets its own test directory and its own
minimal ``client``/``admin``/``db_session`` fixtures, the established
convention for every prior domain in this codebase."""
...
def _register_org(client: TestClient, org_name: str) -> dict:
    email = f"conn_{uuid.uuid4().hex[:10]}@example.com"
    reg = client.post(
        "/auth/register",
...
def admin(client: TestClient) -> dict:
    ...
    return _register_org(client, "Connector Org")


@pytest.fixture()
def other_org_admin(client: TestClient) -> dict:
    """A second, independent org — for cross-tenant isolation tests."""
    return _register_org(client, "Other Connector Org")


@pytest.fixture()
def viewer(client: TestClient, admin: dict) -> dict:
    """A VIEWER-role member of ``admin``'s org — VIEWER has none of the new
    ``integration.connector.*`` permissions (they are not in
    ``rbac_service._READ_ONLY``), so this fixture exercises "authenticated
    but not permitted" (403), distinct from "not authenticated at all" (401)."""
    email = f"connv_{uuid.uuid4().hex[:10]}@example.com"
