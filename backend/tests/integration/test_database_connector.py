...
def test_ac26_migration_head_unchanged_no_new_migration_needed():
    """AC-26 — every table this connector touches already exists; **this
    ...
    **Rewritten in Phase 4.1, and strengthened rather than relaxed.** This
    ...
    any later phase adds one and which says nothing at all about Phase 2.2.2.
    ...
    belongs to Phase 2.2.2** — which stays true forever and is strictly
    stronger, because it also catches a Phase 2.2.2 migration inserted
    *before* the head, a case the newest-file check could never have seen."""
    ...
    for revision in revisions:
        ...
        assert "Phase 2.2.2" not in header, (
            f"{revision.name} is a Phase 2.2.2 migration; this sub-phase must add none."
        )
